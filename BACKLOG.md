...
### v0.5.2 candidates (queued for next session)

Minor follow-ups identified during the v0.5.1 ship cycle. None are release-blocking — defer to a focused v0.5.2 milestone.

- **Retire `detect_coverage_tool` / `get_coverage_gradle_task`** (~3h) — Phase 4 D1 explicitly kept the legacy chain as a fallback for one release cycle. Now that v0.5.1 is in production, audit field reports for any model fast-path miss. If clean, delete the legacy helpers; sh + ps1 then go straight to `pm_get_coverage_task` with no fallback. Saves ~80 LoC of dead detection code.
- **Unify `gate_jdk_mismatch` (sh) + `Invoke-JdkMismatchGate` (ps1) walkers via the model** (~2h) — Phase 4 D4 deferred this for the same reason. Both walkers currently have their own exclusion lists (sh:4 dirs, ps1:5 dirs) that drift from the JS canonical (9 dirs + depth=12). Migrate to delegate via `pm_get_jdk_requirement` with the existing legacy walker as fallback.
- **Cache-key SHA byte-for-byte parity sh ↔ JS** (~2h) — `lib/project-model.js#computeCacheKey` and `scripts/sh/lib/gradle-tasks-probe.sh:_kmp_compute_cache_key` produce identical SHAs on ASCII fixtures but diverge by 50-200 bytes of concat input on real-world projects (likely due to subtle differences in trailing-CRLF stripping between bash's `$(cat)` and JS `replace(/\n+$/)`). Each walker correctly invalidates its own cache so this is cosmetic — but coexistence costs one extra ~1KB file per cache generation. Worth aligning if it ever produces a user-visible issue.
- **bash-side `summary.json` counter audit** (~1h) — PR #54 fixed the PowerShell single-item-pipeline collapse (`Where-Object` → hashtable instead of array). Bash side uses `wc -l` + array-iteration explicit increments which should be immune by inspection, but a brief audit on the same `--module-filter '*single*'` scenario to confirm bash output matches PS1 post-fix is recommended.
- **Android `--test-filter` method-level filtering** (~30min-1h) — `kmp-test android --test-filter "<pattern>"` today maps **class-level only**: the wildcard or FQN gets resolved to a class FQN that becomes `-Pandroid.testInstrumentationRunnerArguments.class=<FQN>` for AndroidJUnitRunner. JVM-side (parallel/changed/coverage/benchmark JVM) already supports method-level via gradle's native `--tests "FQN.method"` matcher. Gap: support `--test-filter "FQN#testMethod"` or `"FQN.testMethod"` on android by detecting the method portion, splitting class+method, and emitting BOTH `-Pandroid.testInstrumentationRunnerArguments.class=<FQN>` AND `-Pandroid.testInstrumentationRunnerArguments.method=<method>` (AndroidJUnitRunner accepts both flags together). Same gap on `kmp-test benchmark --platform android`. Surfaced 2026-04-29 while answering "can I run a single benchmark/test method?" — JVM yes, Android only class-level.
