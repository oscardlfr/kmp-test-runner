...
- **Audit 2026-06-09 — LOW-tier findings (IDEAs; fix directions validated in-session, milestone unassigned — user's call).** The HIGH+MEDIUM tier shipped as the audit PR train (maxBuffer/spawn_error, dispatcher envelope, atomic lockfile, toml cache-key, artifact sweep + `clean`, diagnostics warnings). The 6 LOW findings below were verified with file:line evidence and their fix shapes design-reviewed; each is a small standalone change:
  - ✅ **L1 — dangling `--isolated-cache-dir` should be `invalid_flag_value` — SHIPPED 2026-06-10 (this PR).** Implementation upgraded the original two-site shape: (1) **three sites, not two** — `changed-orchestrator.js`'s inline `--isolated-cache-dir` case was a third divergent behavior (isolation silently ON with auto dir) and got the same guard; (2) both parsers return an additive `errors: []` (existing `invalid_flag_value` entry shape) instead of a `dangling:true` boolean, so the cli.js gate and the orchestrator `invalid_*` gates reuse the established contract — no new envelope codes; (3) `parseIsolatedArgs` now expands `=`-forms internally — pre-fix `--isolated-cache-dir=/x` was parsed by NOBODY on runner.js-direct routes (isolation silently no-opped) and `--isolated-cache-dir=` slipped past an undefined-only check. `runAndroid` gained its first invalid-args gate. cli.js rejects via `peekIsolatedFlags().errors` BEFORE the acquireLock decision. +14 vitest (1830→1844, incl. 1 characterization snapshot). Wet on a reference composite: dangling via bin → exit 2 + no lockfile left behind; direct `runner.js parallel` → exit 2; `changed` → exit 2; happy path still isolates with `cache_dir` override.
  - **L2 — `junit-xml.js` readFileSync without size guard.** `lib/parsers/junit-xml.js#forEachJunitXml` reads every `TEST-*.xml` whole; a pathological multi-hundred-MB XML (huge `<system-out>` CDATA) spikes RAM. Cheap `statSync` guard (skip + count files > ~32 MB). The regex shape itself is fine (lazy + literal anchor — linear in practice; the "catastrophic backtracking" hypothesis was checked and discarded).
  - **L3 — `lib/user-config.js:23` precedence comment names an "env var" layer that doesn't exist in `loadMergedConfig`.** Env vars influence specific features (JDK selection, skip-lists) outside the merge. Reword to match `lib/project-config.js`'s accurate description. Comment-only.
  - **L4 — `~` in `org.gradle.java.home` is not expanded by the jdk-preflight gate** (`lib/project/jdk-preflight.js:66-67`: `existsSync('~/jdk')` is false → gate silently passes). **Do NOT "fix" by expanding** — Gradle itself rejects `~` in that property, so expansion would suppress kmp-test's gate for a build Gradle will reject anyway. Right shape: detect the leading `~`, warn on stderr + add a `doctor` check row ("Gradle does not expand `~` here").
  - **L5 — `computeCacheKey` hashes symlinked build files by walked path (no `realpathSync`).** Two worktrees sharing sources via symlinks can collide on one cache entry. Document-only (symlinks in gradle project source are rare; realpath adds I/O per file).
  - **L6 — `cascade-retry.js` `critical[]`/`statusNoise[]` arrays uncapped** (`lib/orchestrators/parallel/cascade-retry.js:253-270`). Bounded de facto by leg maxBuffer; cap defensively following the existing `tasksRun.slice(0, 30)` pattern (keep first N critical + suppressed-count line).
  - **L7 — `benchmark --test-filter` on the jvm leg emits gradle `--tests`, which kotlinx-benchmark tasks reject** (wet 2026-06-10 on the reference composite: `Problem configuring task :<mod>:desktopSmokeBenchmark ... Unknown command-line option '--tests'` → `module_failed`). `buildFilterArgs` should map the jvm benchmark leg to kotlinx-benchmark's own filter mechanism (`-P` include pattern) or warn-and-drop the flag for benchmark tasks instead of passing a JVM-Test-only option through. The android leg's `-P` instrumentation args are unaffected. Flags-reference currently advertises `--test-filter` ✓ for benchmark — doc or behavior must converge.
