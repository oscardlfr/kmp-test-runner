...
6. **macOS validation gate** — ✅ DONE 2026-05-18 (v0.10 #6). Probe-mode evidence captured in `MACOS-GATE-V0.10-SUMMARY.md` at repo root: 45 cells, 28 PASS / 1 benign DRIFT (`android_none_KaMPKit`, snapshot over-rich vs live `errors: []` + `coverage.module_buckets.*` not emitted by android subcommand; parity vitest 64/64 PASS confirms no contract regression) / 16 SKIP (15 `project-absent` for the maintainer-private `private-lib` placeholder, 1 `no-instrumented-target` for the in-repo fixture). Scoped phase intentionally skipped (system disk at 6.3 GB free, below the 8 GB pre-flight floor; full mode requires 20 GB). Bundled tooling change: `tools/macos-validation-gate.mjs` gained `--label <vX.Y>` flag (default `v0.9` backward-compat) parameterizing the summary title + `.smoke/macos-gate-<label>/` subdir for future cycles. Follow-up: parity snapshot refresh against live `android --list-only` envelope to clear the benign DRIFT.
