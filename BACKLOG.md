...
- **`parseTestCounts` miscount on AGP 9.2.x connected tests** (wet-finding 2026-06-06, S22 / JetNews / AGP 9.2.1). A single failing `connectedDebugAndroidTest` reported `tests: {total:1, passed:1, failed:0}` in the envelope — the count is cosmetic (gradle exit code is the authority, so `exit_code:1` + `errors[].module_failed` were correct), but `parseTestCounts` (`lib/orchestrators/android-orchestrator.js`) doesn't recognise the AGP 9.2 device-test reporter's failure line, so it grades the run as passed. Add an AGP-9.2-format arm (or fall back to the gradle exit code when the parse finds a total but no failure marker on a non-zero exit). Low priority — surfaced during capture-on-fail wet validation, not a capture bug.
- **`parallel` instrumented `module_failed` over-reports `setup_failed:true` + `individual_total:0`** (wet-finding 2026-06-07, S22 / shared lib / `connectedDebugAndroidTest`). A genuine on-device test failure (assertion threw — confirmed by the captured screenshot) was tagged `errors[].setup_failed:true` and `tests.individual_total:0`. Root cause: `recordLegResults` (`lib/orchestrators/parallel/result-rollup.js`) walks `<module>/build/test-results/<task>/TEST-*.xml` via `junitTestCountFor` / `junitTestFailuresFor`, but a `connected*AndroidTest` writes its results to the instrumented tree (`<module>/build/outputs/androidTest-results/connected/…`), which the walker never reads — so instrumented failures always look like "never ran" (setup) with zero individual tests. `exit_code:1` + `errors[].module_failed` are correct (gradle exit is the authority); only the `setup_failed` discriminator + per-test count are wrong. Fix: teach the junit walker the connected-results path for instrumented tasks (or suppress `setup_failed` when the task is a `connected*AndroidTest` / `androidConnectedCheck`). Same family as the `parseTestCounts` item above (instrumented result parsing), distinct file + mechanism. Low priority — surfaced during the `--capture-on-fail` parallel wet validation, NOT a capture bug (the capture fields themselves were correct). Milestone unassigned — user's call.
