...
4. **Google `android` skills system viability** — ✅ RESEARCH DONE 2026-05-16 (`INVESTIGATE-ANDROID-SKILLS.md` at repo root, gitignored). Decision matrix 7/7 → SHIP authorized. Skill is being published as part of the `agentskills.io` open standard (multi-vendor — 35+ adopters: Claude Code, Gemini CLI, Cursor, GitHub Copilot, OpenAI Codex, etc.). **Multi-PR ship in progress**: PR 1 (foundation: `SKILL.md` + `references/cli/` + `skills-validate` CI gate + README + BACKLOG) → PR 2 (non-instrumented workflow refs) → PR 3 (instrumented workflows with `android`-CLI-presence dual branch) → PR 4 (convenience scripts) → PR 5 (Claude Code Plugin packaging). Estimated 9-12h total across the 5 PRs.
4.5. **Cross-tool output alignment audit — `kmp-test` vs `android` CLI** — surfaced during v0.10 #4 (skill shipping). Two pairs of overlapping commands emit JSON that an agent might switch between:

    | kmp-test                            | android CLI       | Overlap                                  |
    |-------------------------------------|-------------------|------------------------------------------|
    | `kmp-test parallel --dry-run --json` | `android describe` | Project metadata, modules, build targets |
    | `kmp-test doctor --json`             | `android info`     | Environment paths, SDK location, JDK info |

    Open questions: (1) are the JSON shapes compatible enough that an agent can fall back from one to the other? (2) should we converge schemas where reasonable so the SKILL.md body doesn't need to teach two shapes? (3) or stay independent + document the diff in the skill's `references/cli/envelope-schema.md`?

    Scope: lightweight audit + skill documentation update ~2h. If convergence is chosen: ~4-6h CLI-side schema rework + tests + envelope `schema_version: 2→3` bump.

    **Ship-when**: AFTER v0.10 #4 PRs 1-3 land — the skill content depends on the current state of the CLI, and we want real wet feedback from instrumented runs before reshaping schemas.
