...
- 🚨 **PRE-RELEASE — add Opus 4.8 (`claude-opus-4-8`) to the token-cost measurements + README/docs** (requested 2026-06-07). Opus 4.8 is now the latest Claude model; the committed measurements were taken with **`claude-opus-4-7`**. Before the next release, re-run the cross-model token-cost measurement with `claude-opus-4-8` and refresh every opus-labelled number/claim. **Touchpoints:** `tools/measure-token-cost.js` (the `--anthropic-models` list — bump examples/defaults `claude-opus-4-7` → `claude-opus-4-8`); `README.md` "Why this exists" — the opus columns in the 4 drill-down tables (lines ~41–91), the **"`claude-opus-4-7` ships a new tokenizer"** note (line ~34 — confirm whether 4.8 shares 4.7's tokenizer → identical counts, or differs → new numbers), the opus figures in the headline + "Practical impact" callout (lines ~7, ~31, ~122), and the `--anthropic-models` example flags (lines ~113/119); `docs/token-cost-measurement.md`; the committed evidence `tools/runs/cross-model-results-<feature>.txt`. **Needs:** `ANTHROPIC_API_KEY` + the `private-large-A` reference composite (local-only) + the OSS sample. Measurement session — same methodology as v0.10.1. Honors the cross-project metric-labelling rule (CLAUDE.md). Milestone unassigned — user's call; gate before tagging.
