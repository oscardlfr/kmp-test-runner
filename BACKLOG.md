...
- **README / tools-usage audit (next wave).** Surfaced 2026-06-06 from the `--capture-on-fail` work: a UI teammate added the Gradle plugin to a Compose-UI-only project and saw "no reports" because the default auto-detect runs the unit-test leg and **silently skips instrumented-only modules** — with no hint to use `--test-type androidInstrumented` / `kmp-test android`. Audit the README + `--help` to explain crisply which subcommand / `--test-type` to use per scenario (unit vs instrumented vs Compose UI; KMP vs Android-only) so this confusion doesn't recur. Candidate sub-item: a **discoverability hint** — when the unit/auto leg skips modules *solely* because they only have instrumented test source sets, emit a `warnings[]` / stderr pointer at `--test-type androidInstrumented`. Own PR — do NOT fold into capture-on-fail.
- **`--capture-on-fail` on `parallel --test-type androidInstrumented`.** The shared helper `lib/orchestrators/android-capture.js` already exists (shipped with the `kmp-test android` capture work); wire it into `parallel/result-rollup.js`'s `module_failed` branch (gated to `testType === 'androidInstrumented'`, reusing `resolvedDeviceSerial` + the `.kmp-test-runner/logs/android/<runId>/` dir) and propagate `captureOnFail` / `captureDir` through `ParallelTestsTask`. Deferred from the android-subcommand PR to keep it focused.
