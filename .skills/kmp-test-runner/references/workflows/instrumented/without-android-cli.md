# Instrumented tests (without android CLI) — `kmp-test android`

The canonical workflow for "run instrumented tests on a connected device or emulator" **when Google's `android` CLI is NOT installed** (probe: `which android || echo "NO_ANDROID_CLI"`). Dispatches `<module>:connectedAndroidTest` (or `<module>:androidConnectedCheck` for KMP `androidLibrary{}` DSL) across instrumented-capable modules, surfaces a single JSON envelope summarising per-module pass/fail and device attribution. Adb + the emulator binary are the only environment-introspection verbs available — same diagnostic coverage as the with-CLI branch ([`with-android-cli.md`](with-android-cli.md)), lower-level surface. The JSON envelope is **byte-identical across both branches**.
...
```bash
# Branch check
which android || echo "NO_ANDROID_CLI"

# Verify a device is bootable
emulator -list-avds                              # AVDs (offline) — from $ANDROID_HOME/emulator/
...
| `--capture-on-fail` | off | On per-module failure, auto-capture a device screenshot + UI-hierarchy dump via `adb` — automates the manual `adb exec-out screencap` / `uiautomator dump` steps below. Paths on `errors[].screenshot_file` / `.ui_hierarchy_file`; `capture_error` when adb can't oblige. Forensic-only — never changes the exit code. Same flag on `parallel --test-type androidInstrumented`. |
...
## ADB-only diagnostics

Without the `android` CLI, the agent uses `adb` + the `emulator` binary directly. Same diagnostic coverage as the with-CLI branch, lower-level surface. All commands assume the Android SDK is installed and `adb` + `emulator` are reachable (either on PATH or via absolute path under `$ANDROID_HOME/platform-tools/` and `$ANDROID_HOME/emulator/`).

### Device discovery + AVD lifecycle

```bash
# List AVDs (replaces `android emulator list`)
emulator -list-avds                          # from $ANDROID_HOME/emulator/

# Start an AVD (replaces `android emulator start <AVD>`)
emulator -avd <AVD> &                        # background; foreground without `&`
emulator -avd <AVD> -no-snapshot-load        # equivalent to --cold (wipe snapshot)

# Stop the running emulator (replaces `android emulator stop`)
adb emu kill                                 # graceful shutdown of running emulator
adb -s <SERIAL> reboot                       # forced restart if `adb emu kill` hangs
```

On Windows, `emulator.exe` lives at `$env:ANDROID_HOME\emulator\emulator.exe` — invoke via absolute path or add the directory to PATH. The `&` background syntax does not apply on PowerShell; use `Start-Process` or accept the foreground.

### UI + visual diagnostics on instrumented failures

```bash
# Screenshot (replaces `android screen capture`)
adb shell screencap -p /sdcard/failure.png && adb pull /sdcard/failure.png && adb shell rm /sdcard/failure.png

# UI hierarchy dump (replaces `android layout`)
adb shell uiautomator dump /sdcard/dump.xml && adb pull /sdcard/dump.xml && adb shell rm /sdcard/dump.xml
```

Use these when `errors[].code: module_failed` fires on an instrumented module — the screenshot + XML hierarchy snapshot often reveals "view not visible / wrong activity / dialog dismissed mid-test" causes the JUnit XML alone won't show. The `uiautomator dump` output is XML, not JSON — pipe through `xq` or `xmlstarlet` if downstream tooling needs JSON shape.

**Compose-semantic annotation gap.** The with-CLI branch's `android screen capture --annotate` overlays Compose semantic node bounding boxes on the screenshot; there is no direct adb equivalent. Workaround: install the standalone Layout Inspector JAR (Android Studio Tools menu → "Layout Inspector standalone") or attach Android Studio's Profiler to a running app. For agent automation this is rarely blocking — `uiautomator dump` carries the same node IDs and bounds the annotated PNG would show, just in XML.

### Gradle introspection

There is no adb equivalent of `android describe`. Use `kmp-test describe --json` instead — it walks the kmp-test project model and surfaces per-module `test_tasks`, coverage plugin attribution, and the dependency graph directly. `kmp-test describe` is the canonical project-model probe regardless of branch; `android describe` is a supplementary build-output diagnostic only useful in the with-CLI branch.

### What adb does NOT cover

- **Compose-semantic annotations** (see above). Workaround: Layout Inspector standalone or Android Studio.
- **Google-published documentation knowledge base** (`android docs search` / `fetch` in the with-CLI branch). Workaround: WebFetch developer.android.com directly.
- **Project scaffolding** (`android create`). Irrelevant for instrumented testing — moot in either branch.
...
- **Cold-boot timing**: `emulator -avd <AVD>` (foreground or background) takes 15-60s to reach a wakeful state. Pair with `adb wait-for-device` before invoking `kmp-test android`, or accept that the first call will see `instrumented_setup_failed` if the emulator hasn't finished booting.
- **`--device <SERIAL>` with offline serial**: `adb devices -l` shows `offline` next to the serial; `kmp-test android --device <OFFLINE_SERIAL>` emits `instrumented_setup_failed` (exit 3) — the dispatch never spawns gradle. Recovery: `adb -s <SERIAL> reboot` (real device) or `adb emu kill && emulator -avd <AVD>` (emulator).
...
The envelope is **branch-invariant** — running this dispatch with or without the `android` CLI on PATH produces the exact same JSON. The branch only changes how the agent introspects the device before/after the run.

## Troubleshooting

Branch on `errors[].code`:

- `instrumented_setup_failed` → [`../../troubleshooting/instrumented-setup-failed/without-android-cli.md`](../../troubleshooting/instrumented-setup-failed/without-android-cli.md) (this branch)
