...
| `module_failed` | parallel, android | 1 | A gradle task failed. | `setup_failed:bool`, `module:string`; on `kmp-test android --capture-on-fail`: `screenshot_file?:string`, `ui_hierarchy_file?:string`, `capture_error?:string` |
...
- New optional fields on an existing `errors[]` entry (e.g. `module_failed` gains `screenshot_file` / `ui_hierarchy_file` / `capture_error` under `kmp-test android --capture-on-fail`).
