...
| `device_offline` | android, parallel (`androidInstrumented`), benchmark | 3 | A device is present in `adb devices` but its state is `offline` — reconnect USB or restart adb. | `device?:string` (serial, when `--device` was passed) |
| `device_unauthorized` | android, parallel (`androidInstrumented`), benchmark | 3 | A device is present but not authorized for USB debugging — accept the RSA prompt on the device. | `device?:string` |
...
| `coverage_budget_without_coverage` | parallel, coverage, changed (delegated) | 2 | A positive `--min-missed-lines` budget was combined with `--no-coverage` / `--coverage-tool none`, so the requested gate cannot run. Rejected before test dispatch or XML reads. | — |
| `coverage_threshold_exceeded` | parallel (`--min-missed-lines`), coverage, changed (delegated) | 1 | Aggregated (unfiltered) `coverage.missed_lines` exceeds the threshold. `--min-missed-lines` never removes coverage data — it only decides this gate and narrows the *markdown report's* per-class detail section; `coverage.missed_lines` / `modules_contributing` / `module_buckets` always reflect the complete project even when this error fires. | `threshold:int`, `missed_lines:int` |
| `coverage_data_unavailable` | parallel (`--min-missed-lines`), coverage, changed (delegated) | 3 | A positive coverage budget cannot be evaluated reliably, so the run fails closed instead of passing with absent or stale data. `reason` is one of `no-contributing-data`, `target-not-detected`, `target-no-xml`, `target-parse-error`, `report-dispatch-failed`, or `aggregation-failed`. Never emitted together with `coverage_threshold_exceeded`. | `threshold:number`, `reason:string` |
...
| `gradle_timeout` | parallel, benchmark | **0 \| 3** | The gradle spawn exceeded `--timeout`. Parallel and result-determining benchmark timeouts are environment failures (3). A benchmark may retain per-module timeout entries with exit 0 only when another module passed and `--strict-timeouts` is absent; `partial_timeout` then explains the graded result in `warnings[]`. Never retried. | **parallel**: `module:string`, `task:string`, `timeout_ms:number`. **benchmark**: additionally `platform:string`, `log_path:string` |
| `task_not_found` | parallel, android, benchmark, changed | 3 | A dispatched Gradle task is missing — typically a plugin/task-surface mismatch or an incorrect explicit override. Standalone `coverage` only reads XML and does not dispatch coverage tasks. | — |
| `unsupported_class_version` | any | 3 | JDK toolchain mismatch — gradle daemon ran on an older JVM than the test classes target. | — |
| `jdk_mismatch` | parallel, changed, android, benchmark, coverage | 3 | The project requires a newer JDK than the selected runtime and auto-selection could not satisfy it. `--ignore-jdk-mismatch` explicitly bypasses this pre-dispatch gate. | `required_jdk:number`, `current_jdk:number` |
| `platform_unsupported` | parallel (`ios`/`macos`) | 3 | An Apple-hosted test type was requested on Windows or Linux. | `test_type:string`, `platform:string` |
| `lock_write_error` | parallel, changed, android, benchmark, coverage, clean | 3 | The project lockfile could not be created or written, commonly because of filesystem permissions or a read-only worktree. | — |
...
| `no_project` | describe | 2 | `describe` found neither settings nor build files at `--project-root`. | — |
...
| `partial_timeout` | benchmark | At least one benchmark module timed out but at least one other passed. This warning is non-fatal; exit stays 0 by default even though per-module `gradle_timeout` entries remain in `errors[]`. Pass `--strict-timeouts` to make any timeout exit 3. | `timed_out:int`, `passed:int` |
...
If `errors[]` contains any HARD-coded entry, the `exit_code` MUST be non-zero. The CLI auto-promotes `0 → 1 (TEST_FAIL)` when this invariant would otherwise be violated. Always-soft codes (`no_summary`, `no_changed_modules`) do NOT trigger promotion. `gradle_timeout` has one context-sensitive exception: a benchmark with at least one pass and no `--strict-timeouts` keeps its timeout entries for observability, adds a `partial_timeout` warning, and exits 0. Everywhere else it is fatal and exits 3.
