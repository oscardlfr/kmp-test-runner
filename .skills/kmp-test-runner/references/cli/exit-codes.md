...
| `3` | `EXIT.ENV_ERROR` | Environment problem — missing `gradlew` / JDK / `pwsh` / `bash` / project root, `task_not_found`, `unsupported_class_version`, `instrumented_setup_failed`, `device_offline`, `device_unauthorized`, `multiple_adb_devices`, `lock_held`, or `no_test_modules` with `caused_by_filter:false`. |
