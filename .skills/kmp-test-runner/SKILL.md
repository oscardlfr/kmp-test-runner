...
license: MIT
compatibility: "Requires kmp-test CLI installed and gradlew present. Optional, recommended for instrumented tests: Google's android CLI (https://developer.android.com/tools/agents/android-cli) for emulator/screen-capture/UI-debug workflows."
metadata:
  author: oscardlfr
...
1. **Resolve the workflow first** — from what the user asked (see the table under Steps).
   ...
   before running.
2. **Known workflow, no specific module** — dispatch globally as the first action
   (`kmp-test parallel --json --project-root .`); descriptive wording ("app", "shared") isn't an
   exact module.
...
4. **Known workflow, unclear module** — run `kmp-test describe --json --project-root .` once;
   check every `modules[]` entry's task field for the test type, not the likeliest name. 1
   eligible: dispatch its exact name (strip `:` for `--coverage-modules`). 2+ eligible: dispatch
   globally if broad, else ask. 0 eligible: don't invent one.
...
7. **Stop once proven** — a non-dry-run envelope with expected outcome and coherent
   `exit_code`/`errors[]` is terminal. Report and stop: no post-success dry-run, doctor, describe,
   version, or ls/pwd/which probe. An unrelated `skipped[]` entry isn't a reason to keep
   exploring.
...
A denied exploratory command isn't worth retrying — abandon it and go straight to the next canonical
step. A denied canonical `kmp-test` command is final: stop and report the blockage; don't retry
with a different flag, subcommand, or shell wrapper.
...
exists. Once both hold, report "no applicable tests" and stop; don't switch subcommand or
...
2. `gradlew` (`gradlew.bat` on Windows) at the project root — if missing, report the prerequisite
   failure; ask the user to initialize it per the repo's process.
...
`android` CLI is a separate layer (emulator/screen-capture/UI-debug) with its own output format
(see Tool selection) — it never changes `kmp-test`'s envelope. Deep-dives:
...
carries discriminated codes (`no_test_modules`+`caused_by_filter`, `module_failed`+`setup_failed`
— full list in the reference docs).

### 3. Report failures with module attribution

Per `errors[]` entry, surface `code`, discriminators (`caused_by_filter`, `setup_failed`), and
`message`; include `module` only when present. For test failures, drill into
`modules[].test_failures[{test,cause,type}]` — `test` is the fully-qualified `ClassName.methodName`,
`cause` the message, `type` the optional exception class.

## Convenience scripts

Optional, source-checkout only — may not resolve once installed as a plugin or via agentskills.io;
prefer `kmp-test` directly. `run-tests.sh` / `run-tests.ps1` wrap the same JSON envelope;
...
- **Avoid `--no-coverage`** unless coverage doesn't apply — `coverage{}` is usually wanted.
...
- [`references/cli/flags-reference.md`](references/cli/flags-reference.md) — CLI flags table (subcommands + global options)
