...
async function ps(body, shell = 'pwsh') {
  const script = `$ErrorActionPreference = 'Stop'\n$ProgressPreference = 'SilentlyContinue'\nImport-Module ${quote(modulePath)} -Force -DisableNameChecking\n${body}`;
  const result = await execFileAsync(shell, ['-NoProfile', '-NonInteractive', '-EncodedCommand', Buffer.from(script, 'utf16le').toString('base64')], {
...
describe.skipIf(!hasPowerShell)('Evidence1 validation operations functional contract', { timeout: 45000 }, () => {
  it('keeps the worker event loop responsive while PowerShell executes', async () => {
    let timerObserved = false;
    const timer = setTimeout(() => { timerObserved = true; }, 25);
    try {
      expect(await ps('Start-Sleep -Milliseconds 150; $true | ConvertTo-Json -Compress')).toBe(true);
      expect(timerObserved).toBe(true);
    } finally { clearTimeout(timer); }
  });

  it('schema2 preserves independent product, postflight and persistence failures and actual process facts', async () => {
    const dir = mkdtempSync(resolve(tmpdir(), 'e1-validation-diagnostics-'));
    try {
      const result = (await ps(`$m=Get-Module evidence1-validation-ops
        & $m {
          param($dir)
  ...
  it('schema2 validates every failure enum and process field without coercion or unknown keys', async () => {
    const result = (await ps(`$r=New-E1Result 'wet-v2' '${commit}' '${tree}'
      $out=@()
  ...
  it('reads historical schema1 without fabricating process metrics or independent failure causes', async () => {
    ...
    expect((await ps(`ConvertTo-E1SafeResult (${json(historical)}) 'wet-v2' '${commit}' '${tree}' | ConvertTo-Json -Depth 10 -Compress`))).toEqual(historical);
  });

  it('rejects non-string report enums and refuses mutation of historical schema1 diagnostics', async () => {
    expect((await ps(`$r=New-E1Result 'wet-v2' '${commit}' '${tree}'; $out=@()
  ...
  it('records a transport error independently of an already failed product result', async () => {
    const result = (await ps(`$r=New-E1Result 'wet-v2' '${commit}' '${tree}'
  ...
  it('stops on a failed initial marker flush and classifies persistence separately from product', async () => {
  ...
  it.skipIf(process.platform !== 'win32')('rejects unexpected source files, malformed artifact names and excessive generated sets', async () => {
    const f = sourceFixture();
    try {
      const result = (await ps(`$repo=${quote(f.repo)}; $scratch=${quote(f.scratch)}
        $before=Get-E1SourceSnapshot $repo '${f.commit}' '${f.tree}' $scratch
  ...
  it.skipIf(process.platform !== 'win32')('accepts V2 artifacts in V3 but forbids any dry-run artifact mutation', async () => {
    const f = sourceFixture();
    try {
      const result = (await ps(`$repo=${quote(f.repo)}; $scratch=${quote(f.scratch)}
  ...
  it.skipIf(process.platform !== 'win32')('runs guest V3 after V2 artifacts and rechecks evidence after the Java probe before wet dispatch', async () => {
    const f = sourceFixture();
    try {
      const result = (await ps(`$m=Get-Module evidence1-validation-ops
        & $m {
  ...
  it.skipIf(process.platform !== 'win32')('never exempts tracked artifacts or index changes, including assume-unchanged files', async () => {
  ...
  it.skipIf(process.platform !== 'win32').each(['assume-unchanged', 'skip-worktree'])('rejects preexisting hidden source edits under %s without altering the index', async flag => {
    const f = sourceFixture();
    try {
      ...
      expect((await ps(`$rejected=try { $null=Get-E1SourceSnapshot ${quote(f.repo)} '${f.commit}' '${f.tree}' ${quote(f.scratch)}; $false }
  ...
  it.skipIf(process.platform !== 'win32')('accepts a 9 MiB task cache and enforces the real 64 MiB producer cap', async () => {
    const f = sourceFixture();
    try {
      expect((await ps(`$repo=${quote(f.repo)}; $scratch=${quote(f.scratch)}
        $before=Get-E1SourceSnapshot $repo '${f.commit}' '${f.tree}' $scratch
        $cache=Join-Path $repo '.kmp-test-runner/cache'; $null=New-Item -ItemType Directory -Force $cache
  ...
  it.skipIf(process.platform !== 'win32').each(['pwsh', 'powershell.exe'])('fingerprints actual source and artifact bytes without Get-FileHash autoload in %s', async shell => {
    const f = sourceFixture();
    try {
      expect((await ps(`$m=Get-Module evidence1-validation-ops
        & $m {
          param($repo,$scratch)
  ...
  it.skipIf(process.platform !== 'win32')('rejects linked runtime artifacts even when ignored by git and OS is absent', async () => {
  ...
  it('prepares and verifies JDK21 in the child environment before an attempt, then restores all scoped variables', async () => {
  ...
  it('rejects missing or wrong Java before invoking the marker-owning action', async () => {
  ...
  it('checks the real wet envelope and emits only fixed booleans', async () => {
    const checks = (await ps(`Get-E1WetChecks (${json(envelope)}) 1 299.9 | ConvertTo-Json -Compress`));
  ...
  it('rejects wrong counts, coercible strings, extra errors, wrong buckets and timeout', async () => {
    ...
    const outcomes = (await ps(`$values = ${json(mutations)}
    ...
    expect((await ps(`Get-E1WetChecks (${json(envelope)}) 1 300.001 | ConvertTo-Json -Compress`)).wall_budget).toBe(false);
    expect((await ps(`Get-E1WetChecks (${json(envelope)}) 0 1 | ConvertTo-Json -Compress`)).process_exit).toBe(false);
  });

  it('rejects escapes, root paths, ADS and ambiguous canonical segments', async () => {
    ...
    expect((await ps(`$paths = ${json(rejected)}
      @($paths | ForEach-Object { try { $null = Resolve-E1Path $_; $false } catch { $true } }) | ConvertTo-Json -Compress`)))
      .toEqual(rejected.map(() => true));
    expect((await ps(`Resolve-E1Path 'C:/kmp-eval/scratch/validation/report.json' | ConvertTo-Json -Compress`)))
      .toBe('C:\\kmp-eval\\scratch\\validation\\report.json');
  });

  it('binds guest identity to the Hyper-V VM id, host distinction and local account', async () => {
    expect((await ps(`$good = @{ ComputerName='Evidence1Runner'; HostComputerName='HOST'; ExpectedGuest='Evidence1Runner';
  ...
  it('matches canonical attestation hash to readiness and validates freshness and every source anchor', async () => {
  ...
  it('persists an exclusive attempt marker and never retries a timeout for the same target', async () => {
  ...
  it('accepts exactly the two v2 single-cell plans and rejects crossed arms and coercible fields', async () => {
    const result = (await ps(`$plans = ${json([dryPlan(true), dryPlan(false)])}
      $out = @()
      foreach ($p in $plans) {
    ...
    for (const mutation of [
      ...
      expect(Object.values((await ps(`Get-E1DryChecks (${json(plan)}) 'claude-product-canary-v1' '${hash}' '${profileHash}' | ConvertTo-Json -Compress`)))).toContain(false);
    }
  }, 40_000);

  it('computes the profile hash from the real registry exactly as the harness does', async () => {
    expect((await ps(`Get-E1ProfileHash (${json(registry)}) | ConvertTo-Json -Compress`))).toBe(profileHash);
    ...
    expect((await ps(`$ok = try { $null=Get-E1ProfileHash (${json(drift)}); $false } catch { $true }; $ok | ConvertTo-Json -Compress`))).toBe(true);
  });

  it('dispatches exactly one dry plan per real design, validates both complete outputs and refuses a repeat', async () => {
  ...
  it('refuses open live handoffs even when no live process has started yet', async () => {
    expect((await ps(`$r = ${json(evidence.readiness)}
  ...
  it('never publishes a passing terminal before postflight and keeps snapshot failures unknown', async () => {
  ...
  it('counts record additions by key even when deletion hides them in a net file count', async () => {
    expect((await ps(`$r = New-E1Result 'dry-v3' '${commit}' '${tree}'
  ...
  it('sets the no-daemon override only for the product call and restores it after an exception', async () => {
  ...
  it('requires every wet success check and hash, rejects null counters and preserves safe error codes', async () => {
    expect((await ps(`$r = New-E1Result 'wet-v2' '${commit}' '${tree}'
  ...
  it('checks the received module bytes before evaluating any code', async () => {
    expect((await ps(`$receiver=Get-E1ReceiverScript
  ...
  it('rejects guest identity before any guest filesystem or process mutation', async () => {
    expect((await ps(`$m=Get-Module evidence1-validation-ops
      & $m {
  ...
  it.skipIf(process.platform !== 'win32')('contains timeout descendants in its own Windows job without killing unrelated processes', async () => {
  ...
  it.skipIf(process.platform !== 'win32')('captures exit 1 privately and cleans descendants after the original process exits normally', async () => {
    ...
    try {
      expect((await ps(`$r = Invoke-E1OwnedProcess ${quote(process.execPath)} @(${quote(fixture)}) ${quote(dir)} ${quote(resolve(dir, 'stdout.json'))} ${quote(resolve(dir, 'stderr.txt'))} 5
  ...
  it.skipIf(process.platform !== 'win32')('parses the entrypoints and compiles the native helper in Windows PowerShell 5.1', async () => {
    ...
    expect((await ps(`Initialize-E1ProcessJob
      $count=0
