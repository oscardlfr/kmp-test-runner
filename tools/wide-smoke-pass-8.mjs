...
// tools/wide-smoke-pass-8.mjs — wide-smoke pass-8 sweep, the post-PR6
// release-validation gate before tagging v0.8.0. Walks the same hardcoded list
// of 30 gradle projects under <workspace>/ that pass-7 used, runs
// `kmp-test parallel --test-type all --json` against each, classifies the
// result into GREEN/RED-orchestrator/RED-repo/SKIP, emits WIDE-SMOKE-PASS-8.md
// at repo root.
//
// Pass-8 covers the v0.8.0 PR6 surface: `.kmp-test-runner/` artifacts subdir,
// `.kmp-test-runner.json` project config, and cache schema bump 6 → 7. Goal:
// identical bucket counts vs pass-7 (3 GREEN / 14 SKIP / 13 RED-repo /
// 0 cascade) — any regression blocks the v0.8.0 release tag.
//
// Usage:
//   node tools/wide-smoke-pass-8.mjs                              # full sweep
//   node tools/wide-smoke-pass-8.mjs --output X.md --timeout 900
//   node tools/wide-smoke-pass-8.mjs --only local-app-3,local-app-1          # subset re-run
//   node tools/wide-smoke-pass-8.mjs --reclassify                 # re-classify cached envelopes
//
// Per-project artifacts (gitignored): .smoke/pass-8/<safe-name>.{out,err,json}
...
const SMOKE_DIR  = path.join(REPO_ROOT, '.smoke', 'pass-8');
...
function parseArgs(argv) {
  const out = { output: 'WIDE-SMOKE-PASS-8.md', timeout: 900, only: null, reclassify: false };
...
// Reads .smoke/pass-8/<safe>.{json,err,meta.json} and emits a fresh markdown.
function reclassifyOneProject(proj, elapsedMap) {
  ...
  if (!existsSync(metaFile) && !existsSync(jsonFile)) {
    console.log(`[MISSING] ${proj.name} (no captured artifacts at .smoke/pass-8/)`);
    return { proj, missing: true };
  }
...
function emitMarkdown(results, outputPath) {
  ...
  // Pass-7 baseline for direct comparison (frozen 2026-05-03 from
  // WIDE-SMOKE-PASS-7.md). Pass-8 must match these counts exactly to
  // unblock the v0.8.0 release tag.
  const PASS7 = { GREEN: 3, SKIP: 14, 'RED-repo': 13, 'RED-orchestrator-cascade': 0, 'RED-orchestrator': 0, MISSING: 0 };

  const lines = [];
  lines.push('# Wide-smoke pass-8 — post-PR6 release-validation baseline');
  lines.push('');
  lines.push(`Generated: ${new Date().toISOString()}`);
  lines.push('');
  lines.push('Orchestrator HEAD: `63a292b` (v0.8.0 PR6 — `.kmp-test-runner/` artifacts subdir + `.kmp-test-runner.json` project config + cache schema 6 → 7).');
  lines.push('');
  lines.push('Goal: zero regression vs pass-7 baseline (3 GREEN / 14 SKIP / 13 RED-repo / 0 cascade). Any project flipping GREEN/SKIP → RED-orchestrator-cascade or RED-orchestrator blocks the v0.8.0 tag.');
  lines.push('');
  ...
  if (cascadeProjects.length > 0) {
    lines.push(`1. **${cascadeProjects.length} cascade-isolation cases (REGRESSION vs pass-7 baseline of 0)** — orchestrator bug or PR6 path migration regression. Affected: ${cascadeProjects.join(', ')}. **Blocks v0.8.0 tag.**`);
  } else {
    lines.push('1. **0 cascade-isolation cases** — matches pass-7 baseline. PR5\'s execution-summary cascade signature continues to gate the retry path correctly post-PR6.');
  }
  lines.push('');
  lines.push(`2. **${realRedProjects.length} legitimate RED-repo cases** — actual project test failures, out of scope for v0.8.0 docs PR. Affected: ${realRedProjects.join(', ') || 'none'}.`);
  lines.push('');
  ...
  if (orchProjects.length > 0) {
    ...
    lines.push('4. **0 RED-orchestrator (other)** — every non-cascade orchestrator path is healthy post-PR6 path migration.');
  }
  lines.push('');
  lines.push('5. **PR6 surface verified**: `.kmp-test-runner/` subdir creation, dual-read fallback for legacy `.kmp-test-runner-cache/`, project-config loading, cache schema 6 → 7 — none introduced a regression.');
  lines.push('');
  lines.push('## Bucket counts');
  lines.push('');
  lines.push('| Bucket | Pass-7 baseline | Pass-8 actual | Δ |');
  lines.push('|---|---|---|---|');
  for (const [b, c] of Object.entries(counts)) {
    const baseline = PASS7[b] ?? 0;
  ...
  for (const r of results) {
    ...
    if (r.envelope) {
      ...
      lines.push('_No envelope emitted — see `.smoke/pass-8/' + safeName(r.proj.name) + '.err` for stderr._');
    }
    lines.push('');
  }
  lines.push('## Retrospective vs pass-7 baseline');
  lines.push('');
  lines.push('Per-project regression check on the 8 PR3-validated projects. Pass-7 results frozen 2026-05-03; pass-8 must match.');
  lines.push('');
  lines.push('| Project | Pass-7 bucket | Pass-8 bucket | Notes |');
  lines.push('|---|---|---|---|');
  // Pass-7 buckets for the 8 PR3 projects (from WIDE-SMOKE-PASS-7.md).
  const PASS7_PR3 = {
  ...
  for (const r of results.filter(x => x.proj.category === 'PR3')) {
    if (r.missing) { lines.push(`| ${r.proj.name} | ${PASS7_PR3[r.proj.name] || '?'} | MISSING | – |`); continue; }
    const pass7 = PASS7_PR3[r.proj.name] || '?';
    const flagDelta = pass7 !== r.bucket ? ' ⚠️ DELTA' : '';
    lines.push(`| ${r.proj.name} | ${pass7} | ${r.bucket}${flagDelta} | see \`.smoke/pass-8/${safeName(r.proj.name)}.err\` |`);
  }
  lines.push('');
  lines.push('## Per-project artifacts');
  lines.push('');
  lines.push('Forensic captures live in `.smoke/pass-8/` (gitignored — same `.smoke/` rule as pass-7):');
  lines.push('');
  ...
  console.log('Bucket counts (pass-8 vs pass-7):');
  for (const [b, c] of Object.entries(counts)) {
    const baseline = PASS7[b] ?? 0;
    const delta    = c - baseline;
...
function main() {
  ...
  if (opts.reclassify) {
    console.log(`Reclassifying ${targets.length} projects from .smoke/pass-8/ artifacts (no gradle spawn)`);
    console.log('');
