...
// tools/wide-smoke-pass-9.mjs — wide-smoke pass-9 sweep, the cumulative
// validation gate after fix-PR-A (#125 e8fd69c), fix-PR-D (#126 873ed44),
// fix-PR-B (#127 049828a), fix-PR-C (#128 0cd5432) all landed on develop
// 2026-05-04. Walks the same 30-project matrix pass-7/pass-8 used, runs
// `kmp-test parallel --test-type all --json` against each, classifies the
// result into GREEN/RED-orchestrator/RED-repo/SKIP, emits WIDE-SMOKE-PASS-9.md
// at repo root.
//
// Pass-9 covers the cumulative effect of the four release-blocker fix-PRs:
// source-set gate for androidUnit/androidInstrumented (PR-A), KMP
// `androidLibrary{}` plugin opt-in detection (PR-D), JDK preserve-host when
// AGP floor met (PR-B), `ANDROID_HOME` export for JVM legs of AGP-applying
// projects (PR-C). Goal: identical bucket counts vs pass-8 actuals
// (3 GREEN / 14 SKIP / 13 RED-repo / 0 cascade) plus expected
// GREEN/SKIP improvements on Bug A/D repro projects. Any project flipping
// GREEN/SKIP → RED-orchestrator-cascade or RED-orchestrator blocks v0.8.0.
//
// Usage:
//   node tools/wide-smoke-pass-9.mjs                              # full sweep
//   node tools/wide-smoke-pass-9.mjs --output X.md --timeout 900
//   node tools/wide-smoke-pass-9.mjs --only local-app-3,local-app-1          # subset re-run
//   node tools/wide-smoke-pass-9.mjs --reclassify                 # re-classify cached envelopes
//
// Per-project artifacts (gitignored): .smoke/pass-9/<safe-name>.{out,err,json}
...
const SMOKE_DIR  = path.join(REPO_ROOT, '.smoke', 'pass-9');
...
// 30 gradle roots — identical set to pass-7 so bucket counts compare directly.
// Categorised by PR3 sweep status. Stable ordering — alphabetical within each
// category. (8 PR3 + 8 INTERESTING + 14 NEW)
const PROJECTS = [
...
function parseArgs(argv) {
  const out = { output: 'WIDE-SMOKE-PASS-9.md', timeout: 900, only: null, reclassify: false };
...
// Reads .smoke/pass-9/<safe>.{json,err,meta.json} and emits a fresh markdown.
function reclassifyOneProject(proj, elapsedMap) {
  ...
  if (!existsSync(metaFile) && !existsSync(jsonFile)) {
    console.log(`[MISSING] ${proj.name} (no captured artifacts at .smoke/pass-9/)`);
    return { proj, missing: true };
  }
...
function emitMarkdown(results, outputPath) {
  ...
  // Pass-8 baseline for direct comparison (frozen 2026-05-04 from
  // WIDE-SMOKE-PASS-8.md, post-PR6 actuals which matched pass-7 exactly).
  // Pass-9 must match these counts (or improve on Bug A/D repro projects)
  // to unblock the v0.8.0 release tag.
  const PASS8 = { GREEN: 3, SKIP: 14, 'RED-repo': 13, 'RED-orchestrator-cascade': 0, 'RED-orchestrator': 0, MISSING: 0 };

  const lines = [];
  lines.push('# Wide-smoke pass-9 — post-fix-PR-A/D/B/C cumulative validation');
  lines.push('');
  lines.push(`Generated: ${new Date().toISOString()}`);
  lines.push('');
  lines.push('Orchestrator HEAD: `0cd5432` (v0.8.0 fix-PR-C tip — A+D+B+C cumulative: source-set gate, KMP `androidLibrary{}` plugin opt-in, JDK preserve-host, `ANDROID_HOME` export for JVM legs).');
  lines.push('');
  lines.push('Goal: zero regression vs pass-8 baseline (3 GREEN / 14 SKIP / 13 RED-repo / 0 cascade). Improvements expected on Bug A repros (orchestrator over-dispatch — private-lib, di-sample, FileKit-main, private-android-app variants), Bug D repros (KMP `androidLibrary{}` plugin — private-lib/:sample-firebase-mod), Bug B repros (JDK preserve-host notice — local-app-3, Confetti), Bug C repros (`ANDROID_HOME` notice — Nav3Guide-scenes). Any flip from GREEN/SKIP → RED-orchestrator-cascade or RED-orchestrator blocks the v0.8.0 tag.');
  lines.push('');
  ...
  if (cascadeProjects.length > 0) {
    lines.push(`1. **${cascadeProjects.length} cascade-isolation cases (REGRESSION vs pass-8 baseline of 0)** — orchestrator bug or fix-PR-A/D/B/C cumulative regression. Affected: ${cascadeProjects.join(', ')}. **Blocks v0.8.0 tag.**`);
  } else {
    lines.push('1. **0 cascade-isolation cases** — matches pass-8 baseline. PR5\'s execution-summary cascade signature continues to gate the retry path correctly post-fix-PR-A/D/B/C.');
  }
  lines.push('');
  ...
  if (orchProjects.length > 0) {
    ...
    lines.push('4. **0 RED-orchestrator (other)** — every non-cascade orchestrator path is healthy post-fix-PR-A/D/B/C cumulative.');
  }
  lines.push('');
  lines.push('5. **fix-PR-A/D/B/C surface verified**: source-set gate (PR-A: androidUnit/androidInstrumented dispatch), KMP `androidLibrary{}` plugin opt-in detection (PR-D: testAndroidHostTest dispatch), JDK preserve-host when AGP floor met (PR-B), `ANDROID_HOME` export for JVM legs of AGP-applying projects (PR-C) — cumulative effect against the 30-project matrix.');
  lines.push('');
  lines.push('## Bucket counts');
  lines.push('');
  lines.push('| Bucket | Pass-8 baseline | Pass-9 actual | Δ |');
  lines.push('|---|---|---|---|');
  for (const [b, c] of Object.entries(counts)) {
    const baseline = PASS8[b] ?? 0;
  ...
  for (const r of results) {
    ...
    if (r.envelope) {
      ...
      lines.push('_No envelope emitted — see `.smoke/pass-9/' + safeName(r.proj.name) + '.err` for stderr._');
    }
    lines.push('');
  }
  lines.push('## Retrospective vs pass-8 baseline');
  lines.push('');
  ...
  lines.push('');
  lines.push('| Project | Pass-8 bucket | Pass-9 bucket | Notes |');
  lines.push('|---|---|---|---|');
  // Pass-8 buckets for all 30 projects (from WIDE-SMOKE-PASS-8.md L37-L66).
  const PASS8_PROJECTS = {
    ...
    'kotlinconf-app-main':            'GREEN',
    'Nav3Guide-master':               'SKIP',
    'PeopleInSpace-main':             'RED-repo',
  };
  for (const r of results) {
    if (r.missing) { lines.push(`| ${r.proj.name} | ${PASS8_PROJECTS[r.proj.name] || '?'} | MISSING | – |`); continue; }
    const pass8 = PASS8_PROJECTS[r.proj.name] || '?';
    let flagDelta = '';
    if (pass8 !== r.bucket) {
      // Improvement: RED-* → GREEN/SKIP. Regression: GREEN/SKIP → RED-*.
      const wasRed   = pass8.startsWith('RED');
      const nowGreen = r.bucket === 'GREEN' || r.bucket === 'SKIP';
      flagDelta = (wasRed && nowGreen) ? ' ✅ IMPROVEMENT' : ' ⚠️ DELTA';
    }
    lines.push(`| ${r.proj.name} | ${pass8} | ${r.bucket}${flagDelta} | see \`.smoke/pass-9/${safeName(r.proj.name)}.err\` |`);
  }
  lines.push('');
  lines.push('## Per-project artifacts');
  lines.push('');
  lines.push('Forensic captures live in `.smoke/pass-9/` (gitignored — same `.smoke/` rule as pass-7/pass-8):');
  lines.push('');
  ...
  console.log('Bucket counts (pass-9 vs pass-8):');
  for (const [b, c] of Object.entries(counts)) {
    const baseline = PASS8[b] ?? 0;
    const delta    = c - baseline;
...
function main() {
  ...
  if (opts.reclassify) {
    console.log(`Reclassifying ${targets.length} projects from .smoke/pass-9/ artifacts (no gradle spawn)`);
    console.log('');
