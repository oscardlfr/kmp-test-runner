...
// THEY BOTH HASH. One exception (PR-28b): build-logic/**/*.kt sources are
// hashed only here, not in the sh/ps1 siblings — see the SCHEMA_VERSION 8→9
// comment below for why that's safe to leave scoped.
//
...
//
// PR-28b addition (JS-model-path only, NOT mirrored in the sh/ps1 script
// above — see the SCHEMA_VERSION 8→9 comment for the scope rationale):
// build-logic/**/*.kt convention-plugin sources are hashed by RELATIVE PATH
// + content (not content alone), appended after the build-file walk. Path is
// included because a convention plugin's class name comes from its
// filename — analyzeModule resolves `coveragePlugin` from that class name via
// parseBuildLogicPluginDescriptors, so a rename can change model output even
// when total file content is unchanged. Additive: a project with no
// build-logic/ directory (or none containing .kt files) hashes identically
...
// Walk build-logic/**/*.kt (convention-plugin Kotlin sources) for the cache
// key — see the PR-28b comment above computeCacheKey for why. Returns []
// when build-logic/ is absent, so the loop that consumes this is a no-op for
// non-convention-plugin projects (purely additive to the pre-existing key).
function collectBuildLogicKotlinFiles(projectRoot) {
  ...
  function walk(dir, depth) {
    ...
    try { entries = readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const e of entries) {
      if (e.isFile() && e.name.endsWith('.kt')) out.push(path.join(dir, e.name));
    }
    for (const e of entries) {
...
export function computeCacheKey(projectRoot) {
  ...
  // build-logic convention-plugin Kotlin sources — hashed last, by relative
  // path + content. See the PR-28b comment above for why path is included.
  for (const f of collectBuildLogicKotlinFiles(projectRoot)) {
