...
//   - Groovy slashy (`/.../`) and dollar-slashy (`$/.../$`) string literals are NOT recognized
//     — a bare `/` is only ever treated as the start of `//`/`/*`, never as a string delimiter.
//     A JDK/AGP-signal-shaped substring inside one of these (e.g. `def note = /jvmToolchain(21)/`
//     in a `.gradle` file) is scanned as ordinary code and can still produce a false positive.
//     Deliberately out of scope: unlike `'`/`"`, a bare `/` is genuinely ambiguous between
//     "string literal start" and "division operator" without real expression-context tracking
//     (Groovy's own parser resolves this positionally) — a naive heuristic risks the opposite,
//     worse failure mode of mistaking a division for a string open and swallowing real code
//     up to the next unrelated `/` in the file. Tracked as a residual in BACKLOG.md rather than
//     approximated here.
