...
export function extractIncludeModuleNames(content) {
  ...
  for (const m of content.matchAll(/\binclude\b/g)) {
    let j = m.index + m[0].length;
    // Skip horizontal whitespace between 'include' and '('
    while (j < content.length && (content[j] === ' ' || content[j] === '\t')) j++;
    if (content[j] !== '(') continue; // bare form — handled by pass 2
    const { inner } = scanToClosingParen(content, j);
  ...
  for (const bm of content.matchAll(/\binclude\b(?!\s*\()[ \t]*([^\n]+(?:\n[ \t]+['"]:[^\n]*)*)/g)) {
    for (const im of bm[1].matchAll(/['"]:([\w\-:]+)['"]/g)) add(im[1]);
  }
