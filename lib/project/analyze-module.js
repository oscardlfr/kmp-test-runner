...
export function resolveTasksFor(_moduleName, gradleTasks, analysis = null) {
  ...
  function pickFirst(candidates) {
    for (const c of candidates) {
      ...
      // only exist when their source set does, so they need no gate. Falls
      // through to current behavior when source-set info is absent.
  ...
  return {
    ...
    flavors:             flavorsFromTasks(gradleTasks),
