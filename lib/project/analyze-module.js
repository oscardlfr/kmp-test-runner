...
// Groovy short plugin ids (legacy `apply plugin: '...'` and the short
// `id '...'` aliases) → canonical plugin id, so a Groovy build file classifies
// the same as its Kotlin-DSL `id("org.jetbrains.kotlin.*")` twin. Ids not in
// the map pass through unchanged (com.android.*, already-canonical
// org.jetbrains.* forms, etc.).
const GROOVY_SHORT_PLUGIN_IDS = new Map([
  ['kotlin',               'org.jetbrains.kotlin.jvm'],          // legacy 'kotlin' == kotlin.jvm
  ['kotlin-android',       'org.jetbrains.kotlin.android'],
  ['kotlin-multiplatform', 'org.jetbrains.kotlin.multiplatform'],
  ['kotlin-kapt',          'org.jetbrains.kotlin.kapt'],
]);

function canonicalGroovyPluginId(id) {
  return GROOVY_SHORT_PLUGIN_IDS.get(id) ?? id;
}
...
export function analyzeModule(projectRoot, moduleName, buildLogicHints = null, catalog = null, buildLogicDescriptors = null) {
  ...
  const buildFile = resolveGradleFile(modulePath, 'build.gradle');
  let content = '';
  if (buildFile) {
  ...
  // Comment-stripped source, computed once and reused by every downstream
  // signal scan below (JVM targets, Groovy plugin detection, coverage,
  // testBuildType). Stripping first keeps commented-out declarations inert.
  const codeOnly = stripGradleComments(content);
  const namedJvmTargets = [];
  for (const m of codeOnly.matchAll(/\bjvm\s*\(\s*['"]([^'"]+)['"]\s*\)/g)) {
  ...
  const hasDefaultJvm = /\bjvm\s*\(\s*\)/.test(codeOnly);
  ...
  const hierarchyBlocks = codeOnly.matchAll(/applyDefaultHierarchyTemplate\s*\{([\s\S]*?)\n\s*\}/g);
  ...
  // Groovy DSL plugin application — legacy `apply plugin: 'x'` plus the short
  // `id 'x'` aliases inside a Groovy `plugins {}` block. Modern Groovy
  // `id 'full.id'` forms already match the paren-optional `id(...)` patterns
  // above; this layer adds the apply-plugin form and the short-id remaps, then
  // feeds the type decision through `...ViaGroovy` booleans that mirror the
  // `...ViaAlias` mechanism, so a Groovy module classifies like its Kotlin twin.
  const groovyPluginIds = [];
  for (const m of codeOnly.matchAll(/\bapply\s+plugin\s*:\s*['"]([^'"]+)['"]/g)) {
    groovyPluginIds.push(canonicalGroovyPluginId(m[1]));
  }
  for (const m of codeOnly.matchAll(/\bid\s*\(?\s*['"]([^'"]+)['"]/g)) {
    const canonical = canonicalGroovyPluginId(m[1]);
    if (canonical !== m[1]) groovyPluginIds.push(canonical); // only short-id remaps; full ids handled above
  }
  const hasKmpViaGroovy             = groovyPluginIds.includes('org.jetbrains.kotlin.multiplatform');
  const hasAndroidViaGroovy         = groovyPluginIds.some(id => /^com\.android\.(library|application|test)$/.test(id));
  const hasKotlinAndroidViaGroovy   = groovyPluginIds.includes('org.jetbrains.kotlin.android');
  const hasJvmKotlinPluginViaGroovy = groovyPluginIds.includes('org.jetbrains.kotlin.jvm');
  const hasNewKmpAndroidViaGroovy   = groovyPluginIds.includes('com.android.kotlin.multiplatform.library');
  ...
  const hasNewKmpAndroidPlugin = hasNewKmpAndroidLiteralId || hasNewKmpAndroidViaAlias || hasNewKmpAndroidViaGroovy;

  let type = 'unknown';
  let androidDsl = null;
  let androidDslVariant = null;
  if (kmpPluginIdPattern.test(content) || hasAndroidLibraryDsl || hasAndroidTargetCall || hasKmpViaAlias || hasNewKmpAndroidPlugin || hasKmpViaGroovy) {
    type = 'kmp';
  ...
  } else if (hasAndroidPlugin || hasKotlinAndroidPlugin || hasAndroidViaAlias || hasKotlinAndroidViaAlias || hasAndroidViaGroovy || hasKotlinAndroidViaGroovy) {
    type = 'android';
  } else if (hasJvmKotlinPlugin || hasJvmKotlinPluginViaAlias || hasJvmKotlinPluginViaGroovy) {
    type = 'jvm';
  }
  ...
  if (androidDslVariant === 'kmpAndroidLibrary') {
    hasHostTestOptIn = /\bwithHostTestBuilder\s*\{/.test(codeOnly);
    hasDeviceTestOptIn = /\bwithDeviceTestBuilder\s*\{/.test(codeOnly);
  ...
  const tbtMatch = codeOnly.match(/\btestBuildType\s*=\s*['"]([^'"]+)['"]/);
  if (tbtMatch) {
    testBuildType = tbtMatch[1];
  } else {
    const tbtVarMatch = codeOnly.match(/\btestBuildType\s*=\s*([A-Za-z_][\w]*)\b/);
    if (tbtVarMatch) {
      const varName = tbtVarMatch[1];
      const escaped = varName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Try `?:` default first (e.g. `val x = (...) ?: "release"` —
      // di-sample :benchmark shape).
      let valLookup = codeOnly.match(new RegExp(`\\bval\\s+${escaped}\\s*=[^\\n]*?\\?:\\s*['\"]([^'\"]+)['\"]`));
      // Fall back to direct literal (e.g. `val x = "release"`).
      if (!valLookup) {
        valLookup = codeOnly.match(new RegExp(`\\bval\\s+${escaped}\\s*=\\s*['\"]([^'\"]+)['\"]`));
      }
...
export function resolveTasksFor(_moduleName, gradleTasks, analysis = null) {
  ...
  const primaryUnitTask = pickFirst(['desktopTest', 'jvmTest', 'test', 'jsTest', 'wasmJsTest']);
  // kmpAndroidLibrary (com.android.kotlin.multiplatform.library) modules with no
  // co-declared jvm()/js()/wasmJs() target have no candidate in the chain above,
  // even though AGP genuinely creates testAndroidHostTest once withHostTestBuilder
  // {} is declared — analysis.sourceSets.androidUnitTest already mirrors that
  // opt-in (analyzeModule overrides it from hasHostTestOptIn). Lowest-priority
  // fallback only: a module that ALSO declares jvm()/js() keeps jvmTest/etc as
  // unitTestTask (see cross-platform-fixture.test.js, whose real fixture
  // legitimately declares both). Mirrors dispatch.js's own auto-pick fallback so
  // `describe` and `parallel` report the same task for the same module.
  const kmpAndroidHostUnitTask =
    !primaryUnitTask &&
    analysis && analysis.androidDslVariant === 'kmpAndroidLibrary' &&
    analysis.sourceSets && analysis.sourceSets.androidUnitTest &&
    set.has('testAndroidHostTest')
      ? 'testAndroidHostTest'
      : null;
  return {
    unitTestTask:   primaryUnitTask ?? kmpAndroidHostUnitTask,
