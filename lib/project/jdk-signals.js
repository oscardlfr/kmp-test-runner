...
import { stripGradleComments } from './kotlin-dsl.js';
...
export function aggregateJdkSignals(projectRoot) {
  ...
  function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      let content;
      // Strip comments before scanning — a `// TODO: bump to jvmToolchain(21)` note next
      // to a live jvmToolchain(17) call must not inflate the detected requirement (MAX
      // wins below). String literals aren't stripped; that's a known, separate gap.
      try { content = stripGradleComments(readFileSync(full, 'utf8')); } catch { continue; }
