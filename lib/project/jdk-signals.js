...
//
// Walks the project for build scripts (`*.gradle.kts`, `*.gradle`, `*.kt` in
// build-logic), detects JDK version signals (the jvmToolchain / JvmTarget /
// JavaVersion patterns are identical in Kotlin and Groovy DSL), and
...
import { stripGradleComments, stripGradleCommentsAndStrings } from './kotlin-dsl.js';
...
// `jvmTarget=11 + AGP 8.x` projects (local-app-3 case) this picked JDK 11 →
...
export function detectAgpVersion(projectRoot) {
  ...
  //
  //    NOTE (PR-28d): this branch is deliberately NOT routed through stripGradleComments.
  //    The direct analogue of the bug fixed below (a commented-out key=value line) is
  //    already a non-issue here today: TOML comments start with `#`, and the regex below
  //    requires the key to be the first non-whitespace token on the line (`^\s*<key>\s*=`),
  //    which a leading `#` structurally cannot satisfy. Routing this content through the
  //    Kotlin/Groovy stripper would be actively harmful, not just unhelpful — an ordinary
  //    TOML value like `exclude = "**/*.kt"` contains the literal substring `/*`, which the
  //    block-comment rule would treat as an open and eat everything up to the next `*/`
  //    anywhere later in the file. There IS a separate, unrelated, low-probability residual
  //    issue in the `[versions]` section-boundary regex just below (no comment-awareness of
  //    its own, so a stray commented-out `# [versions]` example header ahead of the real
  //    section could make it latch onto the wrong occurrence) — that's a section-header
  //    matching bug, not a key-value-comment bug, and needs its own quote-aware TOML lexer
  //    to close properly. Tracked in BACKLOG.md, not fixed here.
  ...
  for (const f of candidates) {
    ...
    // Strip comments before matching (PR-28d) — a commented-out declaration positioned
    // textually before a live one (a common pattern while bumping AGP: the old
    // `id(...) version "X"` line left commented "for reference") otherwise wins outright,
    // since String.prototype.match (no /g flag) returns the FIRST match in source order.
    // Preserve-mode stripGradleComments leaves the live declaration's string-literal version
    // number fully intact.
    const stripped = stripGradleComments(content);
    // plugins { id("com.android.application") version "X.Y.Z" }
    let m = stripped.match(/id\s*\(\s*["']com\.android\.(?:application|library|test)["']\s*\)\s*version\s*["']([^"']+)["']/);
    if (m) return m[1];
    // buildscript { dependencies { classpath("com.android.tools.build:gradle:X.Y.Z") } }
    m = stripped.match(/com\.android\.tools\.build:gradle:([^"'\s)]+)/);
    if (m) return m[1];
  }
  return null;
}
...
// `min` (i.e., AGP raised the floor; jvmToolchain didn't override it). Callers
// use this to decide whether the "preserving host JDK" notice fires:
...
export function aggregateJdkSignals(projectRoot) {
  ...
  function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      let content;
      // Strip comments AND blank string-literal content before scanning (PR-28d) — a
      // `// TODO: bump to jvmToolchain(21)` note next to a live jvmToolchain(17) call must
      // not inflate the detected requirement (MAX wins below), and neither must a phantom
      // `println("jvmToolchain(21)")`-shaped string literal. Uses stripGradleCommentsAndStrings
      // (blank-string-content mode) — the ONE call site in this codebase that doesn't use plain
      // stripGradleComments; see that function's doc comment in kotlin-dsl.js for why this is
      // safe here specifically (a real JDK signal is always bare DSL code, never a string).
      try { content = stripGradleCommentsAndStrings(readFileSync(full, 'utf8')); } catch { continue; }
