...
// Delegates to lib/project-model.js#aggregateJdkSignals — the pure-Node
// walker that previously lived here is the canonical implementation there
// (same exclusion list, same depth=12 cap, same regex patterns).
function findRequiredJdkVersion(projectRoot) {
  return aggregateJdkSignals(projectRoot).min;
}

// Read `org.gradle.java.home` from <projectRoot>/gradle.properties. Returns
// null when the file or the property is absent/unreadable, else
// { raw, exists, hasTilde }:
//   raw      — the verbatim property value
//   exists   — existsSync(raw), NO expansion. Gradle itself rejects `~` and
//              shell-isms in this property, so kmp-test must judge the
//              literal path exactly like gradle will.
//   hasTilde — value starts with `~`. Gradle does NOT expand it; the
//              preflight warns and `kmp-test doctor` surfaces a WARN row.
// Single reader: preflightJdkCheck and lib/commands/doctor.js both consume
// this so the gradle.properties parse lives in one place.
export function readGradleJavaHome(projectRoot) {
  const gradleProps = path.join(projectRoot, 'gradle.properties');
  if (!existsSync(gradleProps)) return null;
  let txt;
  try { txt = readFileSync(gradleProps, 'utf8'); } catch { return null; }
  const m = txt.match(/^[ \t]*org\.gradle\.java\.home[ \t]*=[ \t]*(.+?)[ \t\r]*$/m);
  if (!m) return null;
  const raw = m[1];
  return { raw, exists: existsSync(raw), hasTilde: raw.startsWith('~') };
}
...
function preflightJdkCheck(projectRoot) {
  const javaHome = readGradleJavaHome(projectRoot);
  if (javaHome) {
    if (javaHome.exists) return null;
    if (javaHome.hasTilde) {
      // Gradle does NOT expand `~` in org.gradle.java.home — the build will
      // fail to launch with this value. Deliberately NOT expanded here
      // either: expanding would make THIS gate pass for a build gradle
      // rejects anyway. Warn, then fall through to the signal-based gate.
      process.stderr.write(
        `[WARN] gradle.properties org.gradle.java.home starts with '~' (${javaHome.raw}) — `
        + `Gradle does not expand it; use an absolute path\n`,
      );
    }
    // Set-but-missing (non-tilde) also falls through to the signal gate —
    // same as the pre-existing existsSync(false) behavior.
  }
