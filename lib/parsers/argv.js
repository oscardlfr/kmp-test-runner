...
// Strip --test-filter <pattern> from args; return cleaned args + pattern (or
// null). A dangling `--test-filter` (last token) is stripped AND reported via
// `errors` — pre-fix the flag was left in the args and leaked downstream to
// the wrapper, where bash silently dropped it and PowerShell died with an
// unstructured parameter-binding error.
export function consumeTestFilter(args) {
    ...
    if (args[i] === '--test-filter') {
      if (args[i + 1] === undefined) {
        errors.push({
          ...
          flag: '--test-filter',
          value: null,
          message: '--test-filter: missing required value',
        });
        continue;
      }
      pattern = args[i + 1];
  ...
  return { args: out, pattern, errors };
}
