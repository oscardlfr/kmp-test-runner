...
// opts.childExecutionModes maps exact task paths to the final attempt's modes;
// opts.childSinceMs is that attempt's freshness cutoff for non-reused children.
function junitTestCountFor(projectRoot, taskColonPath, sinceMs = 0, anomalies = null, opts = {}) {
  let count = 0;
...
function forEachJunitXml(projectRoot, taskColonPath, sinceMs, visit, opts = {}) {
  ...
  const candidateDirs = [
    ...
    { dir: path.join(modPath, 'build', 'test-results', taskShort), sinceMs },
  ];
  ...
  // The per-child cutoff below scopes these to fresh or confirmed reused XML. JVM/KMP `test`
    ...
    if (existsSync(trRoot)) {
      ...
      for (const ent of trEntries) {
        if (ent.isDirectory() && /UnitTest$/.test(ent.name)) {
          let childSinceMs = sinceMs;
          if (opts.childExecutionModes) {
            const mode = opts.childExecutionModes.get(`:${segs.join(':')}:${ent.name}`);
            // Lifecycle-task reuse cannot vouch for an unknown or freshly run child.
            childSinceMs = mode === 'up_to_date' || mode === 'from_cache'
              ? 0 : (opts.childSinceMs ?? sinceMs);
          }
          candidateDirs.push({ dir: path.join(trRoot, ent.name), sinceMs: childSinceMs });
        }
      }
    }
  }
  ...
  if (existsSync(agpRoot)) {
    ...
    for (const ent of agpEntries) {
      if (ent.isDirectory()) candidateDirs.push({ dir: path.join(agpRoot, ent.name), sinceMs });
    }
  }

  for (const { dir, sinceMs: cutoff } of candidateDirs) {
...
function junitTestFailuresFor(projectRoot, taskColonPath, sinceMs = 0, anomalies = null, opts = {}) {
  const out = [];
  forEachJunitXml(projectRoot, taskColonPath, sinceMs, (xml) => {
