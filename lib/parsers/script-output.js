...
//
// `{ useCache: false, skipProbe: true }` is required, not optional: a bare
// buildProjectModel(projectRoot) call defaults useCache to true, which on a
// cold cache still spawns `gradlew tasks --all --quiet` (skipProbe defaults
// falsy) AND unconditionally writes `.kmp-test-runner/cache/model-<sha>.json`
// once the model is assembled, regardless of whether the probe itself
// succeeded. skipProbe alone doesn't fix this — that write is gated on
// useCache, not skipProbe. This is the same no-spawn/no-write combination
// describe-orchestrator.js already uses for its own --skip-probe/--no-cache
// flags. A pre-existing tasks-<sha>.txt is still read (bonus fidelity), just
// never probed for or written on a miss.
//
...
function resolveDryRunModules(subcommand, projectRoot, finalArgs) {
  ...
  try {
    const projectModel = buildProjectModel(projectRoot, { useCache: false, skipProbe: true });
