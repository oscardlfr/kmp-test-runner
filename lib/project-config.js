...
export function loadProjectConfig(projectRoot) {
  ...
  try {
    ...
    warn(`read failed: ${e.message} — ignoring`);
    return null;
  }
  let parsed;
  try {
    ...
    warn(`parse failed: ${e.message} — ignoring`);
    return null;
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    warn('top-level must be a JSON object — ignoring');
    return null;
  }
...
function validateConfig(cfg) {
  ...
  for (const k of Object.keys(cfg)) {
    if (!['sharedProject', 'defaults', 'skip'].includes(k)) out[k] = cfg[k];
  }
  return out;
}
...
//
// v0.10 #3: also injects `--java-home <path>` when the resolved config (after
// user-global + project-local merge) carries `java_home` and the flag is not
...
// loadMergedConfig — v0.10 #3 — load both layers and return the resolved view.
//
...
export function loadMergedConfig(projectRoot, opts = {}) {
  const userPreset = loadUserGlobalPresetForProject(projectRoot, opts);
  const projectLocal = loadProjectConfig(projectRoot);
  return mergeConfigs(userPreset, projectLocal);
}
