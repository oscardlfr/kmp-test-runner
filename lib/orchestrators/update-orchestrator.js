...
// lib/update-orchestrator.js — Node-side `kmp-test update` orchestrator (v0.9 step 3).
//
...
export async function runUpdate({
  ...
  const probe = await probeLatestVersion({ prerelease: opts.prerelease, fetchImpl });
  if (!probe) {
    const envelope = envErrorJson({
      subcommand: 'update',
      projectRoot: root,
