...
function parseArgs(argv) {
  const out = {
    ...
    prefix: '',
    prerelease: false,
    // Collected validation errors (dangling value-bearing flags). runUpdate
    // gates on them before any version probe.
    errors: [],
  };
  // Accept POSIX `--name=value` form. Orchestrator-only subs return from
  // cli.js BEFORE its global equals-form expansion, so `--prefix=/x` reached
  // this parser unexpanded and silently fell to the default-drop case.
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      // Shared path flag remains in argv so commands/update.js can resolve the
      // project root before this parser handles update-specific options.
      case '--project-root': requireFlagValue(a, argv[++i], out.errors); break;
      case '--prefix':     out.prefix = requireFlagValue(a, argv[++i], out.errors) || ''; break;
...
export async function runUpdate({
  ...
  // Exit 2 BEFORE any version probe when parseArgs collected
  // validation errors (dangling value-bearing flags).
  const invalidArgs = (opts.errors || []).filter(
    e => e.code && (e.code.startsWith('invalid_') || e.code === 'unknown_flag'),
  );
  ...
  // In JSON mode use stdio:'pipe' so the installer's stdout/stderr are captured
  // rather than inherited — prevents contamination of the single-JSON-object
  // contract on stdout. Captured output is forwarded to stderr after the spawn.
  const spawnStdio = jsonMode ? 'pipe' : 'inherit';
  ...
  // Forward any captured installer output to stderr so the user still sees it.
  if (jsonMode) {
