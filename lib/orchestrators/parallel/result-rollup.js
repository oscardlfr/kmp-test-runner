...
function recordLegResults({
  ...
  // errors instead. No state.tests.failed increment — no test cases ran.
  timedOut = false,
  timeoutMs = 0,
}) {
  if (timedOut) {
...
// pre-PR-10 parallel-orchestrator.js). Exit-code policy in priority order
// (first match wins):
//   - CONFIG_ERROR (2): CLI usage errors — flavor_unused, OR no_test_modules
//     with caused_by_filter:true (user supplied a filter that matched
//     nothing).
//   - ENV_ERROR (3): environmental failures — no_test_modules (project
//     genuinely empty), platform_unsupported, instrumented_setup_failed.
//   - TEST_FAIL (1): module_failed / task_not_found /
//     unsupported_class_version / coverage_threshold_exceeded / any
//     state.tests.failed > 0.
//   - SUCCESS (0) otherwise.
function decideExitCode(state) {
  // `flavor_unused` is a USAGE error: user
  ...
  // `no_test_modules` with `caused_by_filter:
  ...
  const ENV_CODES = new Set(['no_test_modules', 'platform_unsupported',
    'instrumented_setup_failed', 'gradle_timeout']);
  ...
  const hasTestFailure = state.tests.failed > 0
    || state.errors.some(e => e && (e.code === 'module_failed'
        ...
        // Coverage gate breach is a test-fail
  ...
  if (hasConfigError) return EXIT.CONFIG_ERROR;
  if (hasEnvError)    return EXIT.ENV_ERROR;
  if (hasTestFailure) return EXIT.TEST_FAIL;
  return EXIT.SUCCESS;
}
