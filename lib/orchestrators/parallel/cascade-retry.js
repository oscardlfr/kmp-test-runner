...
async function executeLeg({
  ...
  if (taskList.length === 0) {
    if (modules.length > 0 && opts.testType !== 'all') {
      ...
      return { stdout: '', stderr: '', exit: 0, taskOwners: [] };
    }
  ...
  const initialJunitAttempt = {
    childExecutionModes: classifyTaskExecutionMode(stdout, stderr),
    childSinceMs: state.runStartMs,
  };
  const junitAttempts = new Map(taskList.map(task => [task, initialJunitAttempt]));
  ...
  if (cascadeDetected) {
    ...
    for (let i = 0; i < taskList.length; i++) {
      ...
      // Never borrow reuse evidence from the first attempt or another module's retry.
      junitAttempts.set(task, {
        childExecutionModes: classifyTaskExecutionMode(r.stdout, r.stderr),
        childSinceMs: retryStartMs,
      });
  ...
  let inFailureCause = false;
  for (const line of allLines) {
    // Gradle's cause may be plain prose, not a '> ...' continuation. Retain
    // this section under the same cap, stopping at the next section header.
    if (/^\* /.test(line)) inFailureCause = /^\* What went wrong:/.test(line);
    ...
    if (STATUS_RE.test(line)) {
      ...
      statusNoiseCount++;
    } else if (CRITICAL_SPECIFIC_RE.test(line) || (inFailureCause && line.trim())) {
      pushCritical(line);
  ...
  execSummary = recordLegResults({
    ...
    modules, state, projectRoot, log, resolutionFailed, resolutionFailedTasks, junitAttempts,
  ...
  return {
    ...
    // The modules that actually got a task dispatched THIS leg, after
    // SKIP_*_MODULES partitioning (step 1) and the per-module task-resolution
    // guard (step 2) — a strict subset of the leg's input `modules` whenever
    // either mechanism excluded one. The coverage-budget availability check
    // must never require a module that was never actually tested this run.
    taskOwners,
  };
}
