...
function parseArgs(argv) {
  ...
  for (let i = 0; i < expanded.length; i++) {
    ...
    switch (a) {
      ...
      case '--module-filter':      out.moduleFilter = requireFlagValue(a, expanded[++i], out.errors) || '*'; break;
      case '--test-filter':        out.testFilter = requireFlagValue(a, expanded[++i], out.errors) || ''; break;
      case '--max-workers': {
      ...
      case '--exclude-modules':    out.excludeModules = requireFlagValue(a, expanded[++i], out.errors) || ''; break;
      ...
      case '--benchmark-config':   out.benchmarkConfig = requireFlagValue(a, expanded[++i], out.errors) || 'smoke'; break;
      case '--variant':
      case '--android-variant':    out.androidVariant = (requireFlagValue(a, expanded[++i], out.errors) || 'auto').toLowerCase(); break;
      // Parity-gap flags. All five are
      // androidInstrumented-only; no-op for other test types.
      case '--device':             out.device = requireFlagValue(a, expanded[++i], out.errors) || ''; break;
      case '--device-task':        out.deviceTaskOverride = requireFlagValue(a, expanded[++i], out.errors) || ''; break;
      ...
      case '--capture-dir':        out.captureDir = requireFlagValue(a, expanded[++i], out.errors) || ''; out.captureOnFail = true; break;
      // Global gradle-args escape hatch. Value is opaque: it may itself start
      // with `--` (e.g. `--gradle-args --no-parallel`).
      case '--gradle-args':
        out.gradleArgs.push(...splitGradleArgs(requireFlagValue(a, expanded[++i], out.errors, { opaque: true })));
        break;
      default:
...
function pickGradleTaskFor(mod, testType, opts = {}) {
  ...
  switch (testType) {
    case 'common':
    case 'desktop':
    // 'jvm' is an accepted TEST_TYPE_VALUES value with no case of its own —
    // same JVM-side chain as common/desktop, including the
    // testAndroidHostTest exclusion below. Falling through to default would
    // have re-exposed the --test-type all double-dispatch this file guards
    // against for common/desktop.
    case 'jvm': {
      ...
      // jsTest/wasmJsTest appear in the unitTestTask candidate chain but are not
      // JVM/desktop runners. An explicit common/desktop request must not dispatch them.
      if (t === 'jsTest' || t === 'wasmJsTest') {
        return { task: null, reason: `no ${testType} target` };
      }
      // testAndroidHostTest is resolveTasksFor's kmpAndroidLibrary fallback
      // candidate — it belongs to the dedicated androidUnit leg, not
      // common/desktop. Without this exclusion, --test-type all would
      // dispatch it twice (once per leg) for host-test-only modules with no
      // jvm()/desktop() target.
      if (t === 'testAndroidHostTest') {
        return { task: null, reason: `no ${testType} target` };
      }
    ...
    case 'android':
    // 'android' is an accepted TEST_TYPE_VALUES value with no case of its
    // own. For every module shape below, default's own fallback already
    // produced the same result androidUnit does here — making it explicit
    // removes the reliance on that coincidence.
    case 'androidUnit': {
    ...
    case 'js':
    case 'wasmJs':
    // TEST_TYPE_VALUES' accepted CLI value is 'wasm', not 'wasmJs' — the
    // 'wasmJs' label is exercised directly by existing unit tests calling
    // pickGradleTaskFor with that literal string, so it stays; 'wasm' is
    // added alongside it so --test-type wasm (the real CLI value) no longer
...
function dispatchLeg({
  ...
  return { exit, stdout, stderr, gradleArgs, signal, spawnError };
}
