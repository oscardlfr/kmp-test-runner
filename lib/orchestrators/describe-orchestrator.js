...
// lib/describe-orchestrator.js — Node-side `kmp-test describe` orchestrator.
//
...
// spliced (--project-root, --java-home, --ignore-jdk-mismatch) remain in
// argv — we skip them explicitly so their values don't bind as positionals
// (see note below). The set mirrors `CLI_GLOBAL_FLAGS` in
// tests/vitest/_parity-helpers.js so the flag-matrix audit stays green.
//
// A positional argument now binds to
// --module-filter as a shorthand. Pre-fix, `kmp-test describe :foo-bar`
...
// `kmp-test describe --module-filter :foo-bar`. Extra positionals
// after the first warn on stderr and are ignored.
const GLOBAL_VALUE_FLAGS = new Set(['--project-root', '--java-home']);
const GLOBAL_BOOL_FLAGS = new Set(['--ignore-jdk-mismatch']);
...
function parseArgs(argv) {
  const out = {
    ...
    skipProbe: false,
    noCache: false,
    // Collected validation errors (dangling value-bearing flags). runDescribe
    // folds them into the same CONFIG_ERROR gate as invalid_regex.
    errors: [],
  };
  // Accept POSIX `--name=value` form. Splits to
  // `[--name, value]` so the if-chain (which matches the bare flag head)
  // sees the canonical shape. Must run BEFORE positional binding (drift #5)
  // so `--module-filter=:foo` doesn't bind as a positional via the
  // `!a.startsWith('--')` branch.
  argv = expandPosixEqualsForm(argv);
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--module-filter') { out.moduleFilter = requireFlagValue(a, argv[++i], out.errors) || ''; continue; }
...
//   includeBuild "../shared-lib"      // groovy-style
...
//      modules — covers the case where a project's `:core-foo`
//      reported `["android"]` from static analysis despite gradle exposing
...
function platformsFromAnalysis(analysis, gradleTasks = null) {
  ...
  // Probed signals. Conservative regex match — only accepts
...
function buildModuleEntry(modKey, entry) {
  ...
  return {
    ...
    // Pass `entry.gradleTasks` so platforms[] is
    ...
    // Probe-backed: a root `subprojects {}` / `allprojects {}` jacoco
    // application surfaces via resolved.coverageTask even when the static
    // coveragePlugin regex saw nothing. aggregateCoverageTool reads this.
    coverage_plugin: effectiveCoveragePlugin(entry),
    test_build_type: entry.testBuildType ?? null,
    // Probe-aware: convention-applied flavors (build-logic plugin) report
    // static hasFlavor=false but surface here via resolved.flavors from the
    // `gradlew tasks --all` probe. `flavors` carries the recovered names.
    has_flavor: effectiveHasFlavor(entry),
...
/**
 * Run `kmp-test describe` — emit per-module breakdown envelope.
 * @param {object} [opts]
...
export function runDescribe({
  ...
  // Exit 2 BEFORE any probe when parseArgs collected validation
  // errors (dangling value-bearing flags). Mirrors the other orchestrators.
  ...
  // Compile the --module-filter regex eagerly so
  // invalid patterns surface as a `invalid_regex` error + exit 2, NOT as a
  // silent fall-through to "match all" (which hid the user's mistake).
  let filterRegex = null;
  if (opts.moduleFilter) {
    try {
      filterRegex = new RegExp(opts.moduleFilter);
    } catch (err) {
      const envelope = buildInvalidArgsEnvelope({
        subcommand: 'describe',
        projectRoot: root,
        ...
        errors: [{
          code: 'invalid_regex',
          flag: '--module-filter',
          value: opts.moduleFilter,
          message: `--module-filter: invalid regular expression '${opts.moduleFilter}' — ${err && err.message ? err.message : String(err)}`,
        }],
      });
      return { envelope, exitCode: EXIT.CONFIG_ERROR };
    }
  ...
  if (!hasSettings && !hasBuild) {
    ...
    // v0.9 wet-audit F-1 — pass exitCode to envErrorJson so envelope.exit_code
    // matches the orchestrator's process exit (CONFIG_ERROR / 2). Pre-fix the
    // envelope hardcoded ENV_ERROR / 3 while the caller exited 2.
  ...
  // Describe accepts a regex (power-user form). Also
  // dual-tests against the colon-stripped form so `^:foo-bar$` and
  // `^foo-bar$` both match a module whose `m.name` is `:foo-bar`.
  // Regex compilation moved to runDescribe top — already validated.
  ...
  const describe = {
    ...
    jdk_requirement: model?.jdkRequirement
      ? { min: model.jdkRequirement.min ?? null, agp: model.jdkRequirement.agpVersion ?? null }
      : null,
    dependency_graph: {
