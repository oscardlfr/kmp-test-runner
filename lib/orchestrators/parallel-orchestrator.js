...
async function runCoverageInProcess(projectRoot, opts, env, log, runCoverageInjection) {
  ...
  const { envelope } = await runCoverage({
    ...
    env,
    log,
    // PR 3.4 A3 — flag so the report header reads "Yes (via parallel)" instead
    // of the hardcoded "No (--skip-tests)" inherited from the skip-tests path.
    testsRan: true,
...
export async function runParallel({
  ...
  if (opts.skipTests) {
    ...
    // PR 3.4 A3 — originatingSubcommand='parallel' so the report says
    // "No (--skip-tests)" instead of the standalone-coverage default.
    return runCoverage({
  ...
  // v0.10 #2 — auto-respect org.gradle.parallel=false from project gradle.properties.
