...
// Structurally complete fallback coverage block for the one path that cannot
// ask coverage-orchestrator.js to construct its own (an exception thrown
// from inside it, so nothing it could have returned is trustworthy).
// Deliberately does NOT claim any module is in no_xml — that bucket means
// specifically "XML missing on disk" (docs/envelope-contract.md), a fact an
// arbitrary exception never establishes. modules_with_kover_plugin /
// modules_with_jacoco_plugin are left untouched (a real, already-known
// project-model fact, independent of the exception); module_buckets is left
// at all-empty instead of forcing a false cardinality match. The resulting
// plugin-list/bucket accounting gap is exactly what the EXISTING
// coverage_aggregation_drift warning already exists to surface (see
// coverage-orchestrator.js's own identical-shaped guard) — reused here
// rather than inventing a new signal.
function buildUnavailableCoverageBlock(coverage) {
  return {
    ...coverage,
    missed_lines: null,
...
// Companion to buildUnavailableCoverageBlock: the coverage_aggregation_drift
// warning coverage-orchestrator.js's own accounting-invariant guard would
// have raised, had it gotten far enough to compute module_buckets at all.
function buildAggregationExceptionDriftWarning(coverage) {
  const detected = (coverage.modules_with_kover_plugin || []).length
    + (coverage.modules_with_jacoco_plugin || []).length;
  return {
    code: 'coverage_aggregation_drift',
    message: `Coverage module accounting drift: ${detected} detected, 0 accounted `
      + `(${detected} unaccounted — coverage aggregation threw before any module could be classified)`,
    detected,
    accounted: 0,
...
// ---------------------------------------------------------------------------
async function runCoverageInProcess(
  projectRoot, opts, env, log, runCoverageInjection, runStartTime, requiredCoverageModules,
  reportDispatchFailed,
) {
  // Allow vitest to inject a stubbed runCoverage (coverage-orchestrator.js's
  // own parser is in-process too, per PR-17 — no subprocess to avoid here).
  ...
  const { envelope } = await runCoverage({
    ...
    // Internal-only (never CLI, never --coverage-modules): the exact modules
    // THIS run already resolved and dispatched for tests (applyModuleFilters,
    // above) — used solely to decide whether a positive coverage budget is
    // evaluable. Must never change discoverCoverageModules's own project-wide
    // dispatch/bucketing.
    requiredCoverageModules,
    // Internal-only: the jacoco/kover report-generation task itself exited
    // non-zero this run. Tells the aggregator to never trust any on-disk XML
    // as fresh proof (it may be stale from an earlier run) — every dispatched
    // module is treated as though its XML is absent, and the budget
    // unavailability reason is reported precisely, instead of coverage-
    // orchestrator.js re-deriving a misleading target-no-xml from a read it
    // was never entitled to trust.
    reportDispatchFailed,
  });
  ...
  // gate signal silently. PR-17: warnings[] gets the identical treatment —
  // pre-fix aggregation warnings (e.g. coverage_parse_failed) were silently
  // dropped on this path even though errors[] had already been fixed.
  return {
...
export async function runParallel({
  ...
  // isolated-flag strip above) collected validation errors. Mirrors
  // EXIT.CONFIG_ERROR contract: bad CLI input is a usage error, not an
  // environment failure.
  const invalidArgs = [...isolatedFlags.errors, ...opts.errors]
    .filter(e => e.code && (e.code.startsWith('invalid_') || e.code === 'unknown_flag'));
  ...
  // `--isolated` isolates Gradle's `--project-cache-dir` + lockfile but cannot
  // protect a SHARED runtime resource (host-wide iOS simulator, single ADB
  // device). Reject those combos early with a structured CONFIG_ERROR. The rule
  // + rationale live in one place — assessIsolatedRuntimeRace — so the
  // dispatcher's `--dry-run` short-circuit enforces the IDENTICAL verdict
  // (dry-run must surface the rejection a real run produces, else --dry-run
  // validation gives a false all-clear).
  const isolatedRace = assessIsolatedRuntimeRace({
    enabled: isolatedFlags.enabled,
    testType: opts.testType,
    hasDevice: !!opts.device,
  });
  if (isolatedRace) {
    const envelope = buildInvalidArgsEnvelope({
      subcommand: 'parallel',
  ...
  // outright is a known usage contradiction — fail before any gradle spawn
  // (test legs OR the --skip-tests coverage delegate below), never silently
  // downgrade to the coverage_aggregation_skipped warning path.
  ...
  //    Reachability: only DIRECT `node lib/runner.js parallel --skip-tests`
  //    invocations land here. The standard CLI route never does — the shared
  //    run-parallel-coverage-suite wrapper rewrites `parallel --skip-tests`
  //    into a `coverage` invocation before runner.js runs; that route
  //    carries the user-typed sub via KMP_ORIGINATING_SUBCOMMAND instead
  //    (script-dispatcher → runner.js coverage branch). Kept as the correct
  //    handler for the runner.js-direct surface.
  //
  ...
  if (opts.gradleArgs.length > 0) log(`Gradle args: ${opts.gradleArgs.length} token(s)`);
  log('');
  ...
  if (hasInstrumentedLeg) {
    ...
    if (opts.device) {
      // --device strict path: full state validation via resolveAdbDevice.
      const { deviceSerial: resolved, error: adbErr } = resolveAdbDevice(devices, { device: opts.device });
      if (adbErr) {
        log(`[ERROR] ${adbErr.message}`);
        state.errors.push({ code: adbErr.code, message: adbErr.message, device: opts.device });
        const envelope = buildJsonReport({
          subcommand: 'parallel',
          projectRoot,
      ...
      resolvedDeviceSerial = resolved;
      dispatchEnv = { ...dispatchEnv, ANDROID_SERIAL: resolvedDeviceSerial };
      log(`Device: ${resolvedDeviceSerial}`);
    } else if (opts.clearData) {
      // --clear-data alone: warn if no devices (pm clear will be skipped, best-effort);
      // when devices are present, validate state before picking one.
      if (!haveDevices) {
        ...
        const { deviceSerial: resolved, error: adbErr } = resolveAdbDevice(devices, {});
        if (adbErr) {
          log(`[ERROR] ${adbErr.message}`);
          state.errors.push({ code: adbErr.code, message: adbErr.message });
          const envelope = buildJsonReport({
            subcommand: 'parallel',
            projectRoot,
            exitCode: EXIT.ENV_ERROR,
      ...
      // No --device, no --clear-data: validate state (catches offline, unauthorized,
      // multiple usable, and no devices → instrumented_setup_failed / exit 3).
      const { deviceSerial: resolved, error: adbErr } = resolveAdbDevice(devices, {});
      if (adbErr) {
        log(`[ERROR] ${adbErr.message}`);
        state.errors.push({ code: adbErr.code, message: adbErr.message });
      ...
      // Surface for envelope only (no ANDROID_SERIAL injection —
      // gradle picks its own default device).
      resolvedDeviceSerial = resolved;
  ...
  let allStderr = '';
  const legResults = [];
  // jacocoTestReport / koverXmlReport{,Debug,Desktop} aggregate the UNIT test
  // task only — androidInstrumented/ios/macos legs never produce coverage
  // report data no matter what they dispatch. Defined here (not just at the
  // report-dispatch step below) so the SAME set also gates which legs may
  // contribute to dispatchedModuleNames, below.
  const UNIT_COVERAGE_LEGS = new Set(['', 'common', 'desktop', 'androidUnit']);
  // Union of every UNIT-COVERAGE-BEARING leg's ACTUALLY-dispatched module
  // names (executeLeg's own taskOwners, post SKIP_*_MODULES + per-module
  // task-resolution) — the coverage-budget availability check, and the
  // report-generation dispatch below, must only ever require/target a module
  // some UNIT-COVERAGE leg genuinely tested this run: never the pre-leg-
  // filtering `modules` set (which can still contain a module SKIP_*_MODULES
  // or a missing per-leg task target excluded before any gradle dispatch),
  // and never a module whose ONLY leg this run was androidInstrumented/ios/
  // macos (real dispatch, but never coverage-relevant).
  ...
  for (const leg of legs) {
    ...
    const r = await executeLeg({
      ...
      state, deviceSerial: resolvedDeviceSerial, isolatedDir,
      capture: captureFn,
    });
    if (UNIT_COVERAGE_LEGS.has(leg)) {
      for (const name of r.taskOwners || []) dispatchedModuleNames.add(name);
    }
    allStdout += r.stdout + '\n';
    allStderr += r.stderr + '\n';
    const legEntry = {
  ...
  // Set when the report-generation task itself exited non-zero this run —
  // threaded into step 11 so coverage-orchestrator.js never trusts possibly-
  // stale on-disk XML as fresh proof (rule 11). coverage-orchestrator.js is
  // the SOLE source of the resulting coverage_data_unavailable error (via the
  // errors-merge below) so the envelope's module_buckets/modules_contributing
  // stay well-formed instead of being left at step 7's incomplete placeholder.
  let reportDispatchFailed = false;
  if (opts.coverageTool !== 'none' && !opts.skipTests && ranUnitCoverageLeg) {
    // Scoped to modules a UNIT-COVERAGE leg actually dispatched this run
    // (dispatchedModuleNames, above) — never the pre-leg-filtering `modules`
    // set, which can still include a module SKIP_*_MODULES excluded, one with
    // no compatible task this leg, or one whose only leg was
    // androidInstrumented/ios/macos. Generating a report for a module that
    // was never actually unit-tested this run would be both wasteful and
    // could feed a stale/misleading XML into the aggregation step below.
    const coverageEligibleModules = modules.filter((m) => dispatchedModuleNames.has(m.name));
    const coverageTasks = buildCoverageReportTasks(coverageEligibleModules, opts);
  ...
  //     Skipped only when --no-coverage / --coverage-tool none. A report-
  //     dispatch failure above still runs this step (with reportDispatchFailed
  //     threaded through) so the envelope's coverage block stays complete and
  //     well-formed — never skipped outright, which would leave step 7's
  //     incomplete placeholder (no module_buckets/modules_contributing) in
  //     the final envelope.
  // ---------------------------------------------------------------
  if (opts.coverageTool !== 'none') {
    try {
      const requiredCoverageModules = [...dispatchedModuleNames].sort();
      // Never trusting on-disk XML as fresh proof is a GATE-only hardening —
      // with no budget requested, there is no gate to protect, and forcing
      // every dispatched module to no_xml would silently zero out
      // missed_lines/modules_contributing/module_buckets for the plain,
      // informational `kmp-test parallel` (no --min-missed-lines) case,
      // changing behavior threshold 0 must keep byte-identical to pre-PR-A.
      const suppressStaleXml = reportDispatchFailed && +opts.minMissedLines > 0;
      const cov = await runCoverageInProcess(
        projectRoot, opts, env, log, runCoverageInjection, startTime, requiredCoverageModules,
        suppressStaleXml,
      );
      ...
      // PR-17: mirror the errors-merge above for warnings — pre-fix this was
      // silently dropped (only errors[] got the same treatment).
      if (cov && cov.warnings && cov.warnings.length > 0) {
        for (const w of cov.warnings) state.warnings.push(w);
      }
    } catch (e) {
      state.warnings.push({
      ...
      // A positive budget can never silently succeed just because aggregation
      // itself blew up — additive to the warning above, never a replacement
      // for it.
      if (+opts.minMissedLines > 0) {
        state.errors.push({
          code: 'coverage_data_unavailable',
          threshold: +opts.minMissedLines,
          reason: 'aggregation-failed',
          message: `Coverage budget cannot be evaluated (aggregation-failed); `
            + `--min-missed-lines ${opts.minMissedLines} requires reliable, fresh coverage data`,
        });
      }
      // The exception means coverage-orchestrator.js never returned, so
      // state.coverage is still step 7's incomplete placeholder (no
      // module_buckets/modules_contributing at all). Replace it with a
      // structurally complete block that never fabricates a no_xml claim an
      // arbitrary exception can't back up (see buildUnavailableCoverageBlock's
      // own doc comment) — surfacing the resulting plugin-list/bucket
      // accounting gap via the existing coverage_aggregation_drift warning
      // instead, exactly as coverage-orchestrator.js's own guard would have.
      const driftWarning = buildAggregationExceptionDriftWarning(state.coverage);
      if (driftWarning.detected > 0) state.warnings.push(driftWarning);
      state.coverage = buildUnavailableCoverageBlock(state.coverage);
    }
  } else {
    state.warnings.push({
