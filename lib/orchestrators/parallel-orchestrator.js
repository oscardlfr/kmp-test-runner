...
// ---------------------------------------------------------------------------
async function runCoverageInProcess(projectRoot, opts, env, log, runCoverageInjection, runStartTime, requiredCoverageModules) {
  ...
  const { envelope } = await runCoverage({
    ...
    // evaluable (Evidence1 success-recovery runbook Section 8.4). Must never
    // change discoverCoverageModules's own project-wide dispatch/bucketing.
...
export async function runParallel({
  ...
  // downgrade to the coverage_aggregation_skipped warning path (Evidence1
  // success-recovery runbook Section 8.3).
  ...
  // Set when a report-dispatch failure has already invalidated a positive
  // budget (rule 11 below) — gates step 11 off so a possibly-stale on-disk
  // XML can never ALSO fire a second, conflicting terminal error (e.g.
  // coverage_threshold_exceeded) alongside the coverage_data_unavailable
  // already pushed here.
  let reportDispatchInvalidatedBudget = false;
  if (opts.coverageTool !== 'none' && !opts.skipTests && ranUnitCoverageLeg) {
    ...
    if (coverageTasks.length > 0) {
      ...
      if (cr.exit !== 0) {
        ...
        // A failed report dispatch invalidates a positive budget even if
        // stale XML from a prior run still sits on disk — never treat
        // possibly-stale coverage as fresh proof (Evidence1 success-recovery
        // runbook Section 8.4, rule 11). Additive to the warning above.
        if (+opts.minMissedLines > 0) {
          state.errors.push({
            code: 'coverage_data_unavailable',
            threshold: +opts.minMissedLines,
            reason: 'report-dispatch-failed',
            message: `Coverage budget cannot be evaluated (report-dispatch-failed); `
              + `--min-missed-lines ${opts.minMissedLines} requires reliable, fresh coverage data`,
          });
          reportDispatchInvalidatedBudget = true;
        }
      }
    }
  }
  ...
  //     Skipped when --no-coverage / --coverage-tool none, or when a report
  //     dispatch failure already invalidated a positive budget above — never
  //     let coverage-orchestrator.js read possibly-stale on-disk XML and
  //     stand it in for fresh proof.
  // ---------------------------------------------------------------
  if (opts.coverageTool !== 'none' && !reportDispatchInvalidatedBudget) {
    try {
      const requiredCoverageModules = modules.map(m => m.name);
      ...
      // itself blew up (Evidence1 success-recovery runbook Section 8.4, rule
      // 12) — additive to the warning above, never a replacement for it.
      if (+opts.minMissedLines > 0) {
        state.errors.push({
          code: 'coverage_data_unavailable',
          threshold: +opts.minMissedLines,
  ...
  } else if (opts.coverageTool === 'none') {
    state.warnings.push({
      code: 'coverage_aggregation_skipped',
      message: '--coverage-tool none: coverage aggregation skipped',
    });
  }
  // else: reportDispatchInvalidatedBudget — the coverage_report_dispatch_failed
  // warning and coverage_data_unavailable error above already fully describe
  // what happened; no additional warning is needed here.
