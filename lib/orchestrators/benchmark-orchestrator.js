...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--project-root') { i++; }  // read by runner.js, not stripped; consume value
    else if (a === '--timeout') {
...
export async function runBenchmark({
  ...
  if (requestedPlatforms.includes('android')) {
    if (skipAdb) {
      ...
      adbDevices = adbProbe();
      const { error: adbErr } = resolveAdbDevice(adbDevices, {});
      androidAvailable = !adbErr;
      // If android was the ONLY platform requested, any device-state error is
      // fatal (no silent pass for missing platform deps).
      if (adbErr && opts.platform === 'android') {
  ...
  if (totalAttempted === 0 && totalTimedOut === 0 && skippedCount > 0 && !haveCapableModules) {
    ...
    // task_not_found / unsupported_class_version discriminate as
    // environment/toolchain problems, not test assertions — classifyExitCode
    // promotes those over the generic "a benchmark failed" TEST_FAIL default.
    scriptStatus = classifyExitCode(state.errors, { testsFailed: totalFail });
  } else {
    scriptStatus = EXIT.SUCCESS;
  }
