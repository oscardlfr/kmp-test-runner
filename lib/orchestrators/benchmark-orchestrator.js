...
function parseArgs(argv) {
  const out = {
    ...
    variant: 'auto',
    // PR 3.2 / A10 — opt-in restore of pre-graded behavior: any gradle
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--config')           { out.config = argv[++i]; }
      ...
      out.variant = (argv[++i] || 'auto').toLowerCase();
    }
...
// v0.9 step 9.3 (Bug #3) — unified filter normalization. Pre-fix used
...
//
// 2026-05-05 v0.9 step 3 — `variant` parameter for Android benchmarks.
...
// testBuildType-aware resolution (fix-PR-F-bis).
...
export async function runBenchmark({
  ...
  // PR 3.2 / A9 — test-only override: deterministic run-id for log-path
  // assertions. Mirrors android-orchestrator.js.
  runId = defaultRunId(),
}) {
  const startTime = Date.now();

  // v0.9 step 4 — strip + parse `--isolated*` flags BEFORE parseArgs. Each
  ...
  // PR 3.2 / A9 — per-task gradle logs at
  ...
  for (const mod of modules) {
    for (const plat of requestedPlatforms) {
      ...
      // PR 3.2 / A11 — default `--no-configuration-cache` for benchmark
      ...
      // flags (gradle last-wins). PR 3.2 / A11: user `--gradle-args
      ...
      // PR 3.2 / A9 — persist per-(module, platform) gradle stdout+stderr
  ...
  if (totalAttempted === 0 && totalTimedOut === 0 && skippedCount > 0 && !haveCapableModules) {
    ...
    // PR 3.2 / A10 — graded partial-timeout. When at least one benchmark
    ...
    // BACKLOG #5 line 468 decision. PR 3.2 / A10: also fires when
  ...
  const benchmarkField = {
    ...
    // PR 3.2 / A9 — per-(module, platform) log file paths keyed by
