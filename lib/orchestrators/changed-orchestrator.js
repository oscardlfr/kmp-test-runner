...
function parseArgs(argv) {
  const out = {
    ...
    dryRun: false,
    // Global escape hatch. Forwarded to runParallel
    // via buildParallelArgs so per-module gradle dispatches receive the tokens.
    gradleArgs: [],
    // Global Android variant selector. Default
    ...
    // buildParallelArgs.
    variant: 'auto',
    // Concurrency Tier 3 isolated cache dir. Same
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case '--exclude-coverage':     out.excludeCoverage = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--test-filter':          out.testFilter = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--exclude-modules':      out.excludeModules = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      ...
      case '--isolated-cache-dir': {
        ...
        if (v === undefined || v === '') {
          ...
          // pre-expands to ['--isolated-cache-dir', '']). Pre-fix this
          // silently enabled isolation with an auto-generated dir — a third
          // divergent behavior vs the parseIsolatedArgs sites. The invalid_*
          // gate in runChanged rejects before any git/gradle work.
          out.errors.push({
...
// Build a git_error env-error result. Used by the three call sites where a git
// command fails at probing time. Sanitizes stderr: collapses CR/LF to spaces,
// trims, caps at 300 chars, omits when empty.
function buildGitErrorResult({ subcommand, projectRoot, startTime, message, gitCommand, exitStatus, stderr, stagedOnly }) {
  ...
  if (envelope.errors.length > 0) {
    envelope.errors[0].git_command = gitCommand;
    envelope.errors[0].exit_status = exitStatus;
    const cleaned = stderr ? stderr.replace(/\r?\n/g, ' ').trim().slice(0, 300) : '';
    if (cleaned) envelope.errors[0].stderr_summary = cleaned;
  }
  envelope.changed = {
    detected_modules: [],
...
function buildParallelArgs(opts, moduleFilter) {
  ...
  if (opts.maxFailures > 0)   a.push('--max-failures', String(opts.maxFailures));
  ...
  // Forward each --gradle-args token verbatim. One emit per
  ...
  // Forward --variant verbatim when non-default. parallel-
  ...
  // Forward isolated flags so parallel-orchestrator's
...
export async function runChanged({
  ...
  // Exit 2 BEFORE git/gradle work when parseArgs
  ...
  // `isolated:` envelope field for changed paths that don't
  ...
  if (gitProbe.status !== 0) {
    ...
    return buildGitErrorResult({
      ...
      exitStatus: gitProbe.status,
      stderr: gitProbe.stderr,
  ...
  if (opts.stagedOnly) {
    ...
    if (r.status !== 0) {
      const msg = `git diff --cached failed (exit ${r.status})`;
      log(`[ERROR] ${msg}`);
      return buildGitErrorResult({
        subcommand: 'changed',
        projectRoot,
        startTime,
        message: msg,
        gitCommand: 'diff --cached --name-only',
        exitStatus: r.status,
        stderr: r.stderr,
        stagedOnly: opts.stagedOnly,
      });
    ...
    if (r.status !== 0) {
      const msg = `git status failed (exit ${r.status})`;
      log(`[ERROR] ${msg}`);
      return buildGitErrorResult({
        subcommand: 'changed',
        projectRoot,
  ...
  const envelope = buildJsonReport({
    ...
    parsed: {
      ...
      // Forward the parallel-orchestrator-owned isolated state.
