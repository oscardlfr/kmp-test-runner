...
// Implementation lives in ./module-filter.js — a dependency-free leaf module,
// so a consumer that only needs the matching contract (tools/agentic-eval's
// grading harness) can import it directly without pulling in this file's
// fs/child_process/console-mode surface. Re-exported here so every existing
// orchestrator-utils.js import site keeps working unchanged.
export { splitCsv, globToRegex, matchModuleFilter } from './module-filter.js';
...
// Returns array of { serial, type, model, state }; empty when no devices or no adb.
// Shared across benchmark + android orchestrators (sub-entries 1 + 3).
// `state` is the raw second token from each `adb devices -l` line:
//   'device' = usable, 'offline' = disconnected/rebooting, 'unauthorized' = needs RSA prompt.
export function parseAdbDevicesOutput(stdout) {
  ...
  for (const line of (stdout || '').split(/\r?\n/)) {
    ...
    out.push({ serial, type, model: modelMatch ? modelMatch[1] : 'unknown', state });
  }
  return out;
}
...
// Classify an adbProbe() result and resolve which device to use.
// Single source of truth shared by android, parallel, and benchmark orchestrators.
// Returns { deviceSerial, error: null } on success, or { deviceSerial: null, error: {code,message,device?} }.
export function resolveAdbDevice(devices, opts = {}) {
  // Missing state (legacy stubs / defaultAdbProbe before state field existed)
  // is treated as 'device' (usable) for backward compatibility.
  const effectiveState = d => d.state ?? 'device';
  const usable   = devices.filter(d => effectiveState(d) === 'device');
  const unusable = devices.filter(d => effectiveState(d) !== 'device');

  if (devices.length === 0) {
    return { deviceSerial: null, error: {
  ...
  if (opts.device) {
    ...
    if (!match) {
      return { deviceSerial: null, error: {
        code: 'instrumented_setup_failed',
        message: `Requested device "${opts.device}" not found in adb devices output. `
          + `Available: ${devices.map(d => d.serial).join(', ') || '(none)'}.`,
        device: opts.device,
      }};
    }
    const mState = effectiveState(match);
    if (mState === 'offline') {
      return { deviceSerial: null, error: {
        code: 'device_offline',
        message: `Device "${opts.device}" is offline. Reconnect the USB cable or restart adb.`,
        device: opts.device,
      }};
    }
    if (mState === 'unauthorized') {
      return { deviceSerial: null, error: {
        code: 'device_unauthorized',
        message: `Device "${opts.device}" needs USB-debugging authorization. Accept the RSA prompt on the device.`,
        device: opts.device,
      }};
    }
    if (mState !== 'device') {
      return { deviceSerial: null, error: {
        code: 'instrumented_setup_failed',
        message: `Device "${opts.device}" is not ready (state: ${match.state}).`,
        device: opts.device,
      }};
    }
    return { deviceSerial: opts.device, error: null };
  }

  // No explicit --device selection
  if (usable.length === 0) {
    const hasUnauth  = unusable.some(d => d.state === 'unauthorized');
    const hasOffline = unusable.some(d => d.state === 'offline');
    if (hasUnauth) {
      return { deviceSerial: null, error: {
        code: 'device_unauthorized',
        message: `No usable adb device — ${unusable.map(d => `${d.serial} (${d.state})`).join(', ')}. Accept the USB-debugging prompt on the device.`,
      }};
    }
    if (hasOffline) {
      return { deviceSerial: null, error: {
        code: 'device_offline',
        message: `No usable adb device — ${unusable.map(d => `${d.serial} (${d.state})`).join(', ')}. Reconnect the USB cable or restart adb.`,
      }};
    }
    return { deviceSerial: null, error: {
      code: 'instrumented_setup_failed',
      message: `No usable adb device. Connected: ${unusable.map(d => `${d.serial} (${d.state})`).join(', ') || '(none)'}.`,
    }};
  }

  if (usable.length > 1) {
    return { deviceSerial: null, error: {
      code: 'multiple_adb_devices',
      message: `Multiple usable adb devices without --device: ${usable.map(d => d.serial).join(', ')}. Pass --device <serial> to select one.`,
    }};
  }

  return { deviceSerial: usable[0].serial, error: null };
}
