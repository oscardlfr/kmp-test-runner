...
export function spawnGradle(spawn, gradlewPath, gradleArgs, opts) {
  // v0.10 #1 — defensive `--console=plain` injection when stdout isn't a
  // TTY (or NO_COLOR is set, or --color=never was passed). Idempotent:
...
export function parseIsolatedArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    // v0.9 session 2 Bug-F — `--isolated-no-lock` implies `--isolated` so the
...
// ---------------------------------------------------------------------------
// POSIX `--flag=value` argv normalization (v0.9 session 2 Bug-A.2)
// ---------------------------------------------------------------------------
//
...
// ---------------------------------------------------------------------------
// Input validation helpers (v0.9 session 2 Bug-B/Bug-D)
// ---------------------------------------------------------------------------
//
