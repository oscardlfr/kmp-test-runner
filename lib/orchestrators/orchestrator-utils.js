...
import { resolveGradleFile } from '../project/kotlin-dsl.js';
...
// (`["foo", "core:domain"]`). Comments stripped before parsing so commented-out
// includes don't surface as phantom modules.
export function discoverIncludedModules(projectRoot) {
  ...
  const out = [];
  // Scan each `include` statement (up to newline/`;`) and pull every quoted
  // ":name". One pass covers both DSLs and both single- and multi-arg forms,
  // independent of parens or quote style. The capture group excludes the
  // leading colon, so `out` holds bare names; dedup preserves first-seen order.
  for (const stmt of content.matchAll(/\binclude\b([^\n;]*)/g)) {
    for (const im of stmt[1].matchAll(/['"]:([\w\-:]+)['"]/g)) {
      const name = im[1];
...
export function spawnGradle(spawn, gradlewPath, gradleArgs, opts) {
  ...
  if (wrapperJar) {
    const javaExe = _resolveJavaExe((opts && opts.env) || process.env);
...
export function requireFlagValue(flag, value, errors) {
