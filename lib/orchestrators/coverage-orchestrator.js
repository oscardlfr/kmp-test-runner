...
function writeMarkdownReport({
  ...
  // PR 3.4 A3 — label reflects actual execution path:
  //   - `coverage` subcommand    → tests never run by design
  //   - `parallel --skip-tests`  → tests skipped on purpose
  //   - `parallel` (full)        → tests ran, then coverage aggregated
  ...
  // v0.8.0 — clean break from project-root output. Reports live under
  // .kmp-test-runner/reports/coverage/, with `latest.md` as the stable alias.
  // The legacy `coverage-full-report*.md` files at project root are GONE
  // (BREAKING change documented in CHANGELOG). The `outputFile` parameter is
  // retained on the CLI surface for back-compat but is no longer read here —
  // path is fixed inside the subdir.
  void outputFile; // intentionally unused — clean-break migration
  const reportsDir = path.join(projectRoot, '.kmp-test-runner', 'reports', 'coverage');
  const versionedPath = path.join(reportsDir, `${runId}.md`);
 ...
 * @param {boolean} [opts.testsRan=false] - PR 3.4 A3: true when called from
 *   parallel-orchestrator AFTER it ran tests; false for standalone `coverage`
 *   or `parallel --skip-tests`. Controls the "Tests Run" report header.
 * @param {string} [opts.originatingSubcommand='coverage'] - PR 3.4 A3: which
 *   subcommand invoked us. 'coverage' (standalone) or 'parallel' (delegated
 *   via --skip-tests OR via in-process aggregation after a full parallel run).
