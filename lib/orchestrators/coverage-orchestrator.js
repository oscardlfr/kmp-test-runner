...
function parseArgs(argv) {
  const out = {
    ...
    dryRun: false,
    // v0.9 session 2 Bug-B/Bug-D — collected validation errors.
    errors: [],
  };
...
function aggregateClassRows(rows, minMissedLines) {
  ...
  const filteredRows = [];
  // wet-audit-v0.9-part2 BUG-2 — track *unfiltered* totals so the gate (and
...
export async function runCoverage({
  ...
  // v0.9 session 2 Bug-B/Bug-D — exit 2 BEFORE any project-model work when
  ...
  // wet-audit-v0.9-part2 BUG-2 — fail-gate. Help text has always promised
