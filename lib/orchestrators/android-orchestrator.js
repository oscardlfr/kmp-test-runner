...
// are stripped by lib/runner.js. --project-root is read there but NOT removed,
// so the switch below must consume it to avoid the unknown_flag path.
function parseArgs(argv) {
  const out = {
    ...
    variant: 'auto',
    // Collected validation errors (dangling value-bearing flags). runAndroid
    // inspects them together with the isolated-flag strip errors and emits
    // an EXIT.CONFIG_ERROR envelope before any adb/gradle work.
    errors: [],
  };
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    switch (a) {
      case '--device':         out.device = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--module-filter':  out.moduleFilter = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--skip-app':       out.skipApp = true; break;
      case '--verbose':        out.verbose = true; break;
      case '--flavor':         out.flavor = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      ...
      case '--capture-dir':    out.captureDir = requireFlagValue(a, argv[++i], out.errors) || ''; out.captureOnFail = true; break;
      ...
      case '--test-filter':    out.testFilter = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--device-task':    out.deviceTaskOverride = requireFlagValue(a, argv[++i], out.errors) || ''; break;
      case '--dry-run':        out.dryRun = true; break;
      case '--project-root':   i++; break;  // read by runner.js, not stripped; consume value
      case '--gradle-args':
        out.gradleArgs.push(...splitGradleArgs(requireFlagValue(a, argv[++i], out.errors, { opaque: true })));
        break;
      case '--variant':
      case '--android-variant':  out.variant = (requireFlagValue(a, argv[++i], out.errors) || 'auto').toLowerCase(); break;
      default:
...
// failure marker that parseTestCounts recognises on a failing run — first
// observed on AGP 9.2.1 `connectedDebugAndroidTest`, but NOT 9.2.1-exclusive:
// wet-confirmed on AGP 9.0.1 too (the reporter prints `Finished N tests` +
// `Tests on <device> failed: There was 1 failure(s).`, which parseTestCounts'
// `Tests M/N completed. (S skipped) (F failed)` progress regex doesn't match,
// so a single failing test surfaced as `tests:{total:1, passed:1, failed:0}`
// even though the task exited non-zero). The gradle exit code is the authority (the PASS/FAIL +
...
export async function runAndroid({
  ...
  // Exit 2 BEFORE any adb probe / gradle work when the isolated-flag strip
  // or parseArgs collected validation errors. Mirrors the parallel / changed
  // / benchmark gates; placed before resolveIsolatedDir so no cache dir is
  // created.
  ...
  const { deviceSerial: resolvedSerial, error: adbErr } = resolveAdbDevice(devices, { device: opts.device });
  if (adbErr) {
    ...
    const envelope = envErrorJson({
      ...
      projectRoot,
      durationMs: Date.now() - startTime,
      message: adbErr.message,
      code: adbErr.code,
    });
    envelope.android = {
      device_serial: adbErr.device || opts.device || '',
      device_task: opts.deviceTaskOverride || '',
      flavor: opts.flavor || '',
      instrumented_modules: instrumentedModules,
    };
  ...
  for (const mod of modules) {
    ...
    let spawnError = (result && result.error) || null;
    let retried = false;
    // finalStdout/finalStderr track the last attempt only; overwritten if retry
    // runs. Used for count/failure parsing so the retry verdict wins over
    // first-attempt noise. Combined stdout/stderr (with both attempts appended)
    // is kept for the .log artifact only.
    ...
    if (exit !== 0 && opts.autoRetry) {
      ...
      finalStdout = (result && result.stdout) || '';
      finalStderr = (result && result.stderr) || '';
      stdout += '\n' + finalStdout;
      stderr += '\n' + finalStderr;
      spawnError = (result && result.error) || null;
      retried = true;
    }

    const duration = Date.now() - t0;
    // Cross-module aggregates only feed the discriminator regex pass below;
    // full per-module output is persisted to logFile. Tail-cap them so a
    // verbose multi-module run can't pin N×64 MB in memory.
    allStdout = tailTruncate(allStdout + stdout + '\n', AGGREGATE_OUTPUT_CAP);
    allStderr = tailTruncate(allStderr + stderr + '\n', AGGREGATE_OUTPUT_CAP);

    // Tee outputs into log files (best-effort; failures surface as a
    // log_write_failed warning so the envelope's log_file pointer isn't a
    // silent dead link — disk-full / read-only artifact dirs).
    const warnLogWrite = (file, e) => state.warnings.push({
      code: 'log_write_failed',
      path: file,
      message: `log write failed: ${(e && e.message) || e}`,
    });
    // Persist stdout AND stderr: gradle writes `FAILURE: ... What went wrong`
    // to STDERR, so a stdout-only log left config/install failures with no
    // cause in ANY artifact (S22 wet finding — a module_failed whose log
    // ended mid-merge-tasks with zero diagnostic). Same separator shape as
    // benchmark-orchestrator's per-task logs.
    const logContent = stdout + '\n--- STDERR ---\n' + stderr;
    try { writeFileSync(logFile, logContent, 'utf8'); } catch (e) { warnLogWrite(logFile, e); }

    // Capture per-module logcat post-run (best-effort). Explicit maxBuffer:
    // a full `logcat -d` dump can exceed spawnSync's 1 MB default (precedent:
    // SCREENCAP_MAX_BUFFER in android-capture.js).
    const logcatRes = spawn('adb', ['-s', deviceSerial, 'logcat', '-d'], { encoding: 'utf8', maxBuffer: resolveMaxBuffer(env) });
    try { writeFileSync(logcatFile, (logcatRes && logcatRes.stdout) || '', 'utf8'); } catch (e) { warnLogWrite(logcatFile, e); }

    const counts = reconcileTestCounts(parseTestCounts(finalStdout), exit);
    ...
    if (status === 'PASS') {
      ...
      const errBuckets = {
        compilationErrors: finalStderr.match(/^e:.+/gm) || [],
        testFailures: parseTestFailures(finalStdout),
        crashes: finalStdout.match(/FATAL EXCEPTION:.+/g) || [],
      };
      try { writeFileSync(errorsFile, JSON.stringify(errBuckets, null, 2), 'utf8'); } catch (e) { warnLogWrite(errorsFile, e); }
      const errEntry = {
        code: 'module_failed',
        module: mod.name,
      ...
      // The gradle child never ran to completion (spawn-layer error such as
      // ERR_CHILD_PROCESS_STDIO_MAXBUFFER or EAGAIN) — discriminate so agents
      // don't read this as an ordinary test failure. Discriminator pass below
      // only upgrades `module_failed`, so this code is final.
      if (spawnError) {
        errEntry.code = 'spawn_error';
        errEntry.errno = spawnError.code || null;
        // Overflow errno differs by platform: Node reports
        // ERR_CHILD_PROCESS_STDIO_MAXBUFFER on POSIX but ENOBUFS on Windows
        // (wet-observed on the S22 session host).
        errEntry.message = `[FAIL] ${mod.name} — gradle child process error: ${spawnError.code || spawnError.message || spawnError}`
  ...
  // 9. Build envelope
  // ---------------------------------------------------------------
  const exitCode = classifyExitCode(state.errors, { testsFailed: state.tests.failed });
  const envelope = buildJsonReport({
    subcommand: 'android',
    projectRoot,
    exitCode,
