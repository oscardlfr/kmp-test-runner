...
function discoverAndroidModules(projectModel) {
  ...
  for (const [modKey, entry] of Object.entries(projectModel.modules)) {
    ...
    out.push({
      ...
      type: entry.type,
      // v0.9 step 9.5 (Bug #5) — surface coveragePlugin so the envelope's
...
// v0.9 step 9.5 (Bug #5) — derive module-name lists keyed on coveragePlugin
...
// Pre-v0.9-step-9.3 this was a plain `m.name.includes(filter)` substring
// check that returned 0 modules on `:`-prefixed input — Bug #3 reproduced
// against di-sample `:benchmark` filter against the `android` subcommand.
...
export async function runAndroid({
  ...
  // v0.9 step 9.5 (Bug #5) — populate kover/jacoco module lists from
  ...
  if (opts.listOnly) {
    ...
    log('');
    // wet-audit-v0.9-part2 OBS-6 — populate top-level `modules[]` and echo
  ...
  for (const mod of modules) {
    ...
    // PR 3.3 / A1 — propagate device pin to the gradle subprocess. The newer
    ...
    // PR 3.3 / A1 — set ANDROID_SERIAL in the spawn env unconditionally when
    ...
    if (exit !== 0 && opts.autoRetry) {
      ...
      // PR 3.3 / A5 — refresh the adb server between attempts. Defensive
      ...
      spawn('adb', ['-s', deviceSerial, 'logcat', '-c'], { encoding: 'utf8' });
      // gradleArgs already carries --project-cache-dir + (PR 3.3 / A1)
    ...
    if (status === 'PASS') {
      ...
      //
      // v0.9 wet-audit drift #4 (PR #168, cells J2 + J2-watch): pre-fix the
