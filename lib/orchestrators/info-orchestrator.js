...
// lib/info-orchestrator.js — Node-side `kmp-test info` orchestrator.
//
...
// Mac shells frequently lack an exported JAVA_HOME — homebrew JDKs install
// under /opt/homebrew/Cellar but don't wire the env var, and Adoptium .pkgs
// only set it interactively. /usr/libexec/java_home is Apple's canonical
// resolver, so on darwin we try it before falling back to null. The fallback
// is darwin-only to preserve the existing null result on linux/windows
// where the same divergence does not exist. The optional `spawner` argument
// is for tests; production callers always use the default.
export function resolveDarwinJavaHome(versionString, spawner = spawnSync) {
  if (process.platform !== 'darwin') return null;
  if (!versionString) return null;
  const parts = String(versionString).split('.');
  const major = parts[0] === '1' ? `1.${parts[1] || '8'}` : parts[0];
  try {
    const r = spawner('/usr/libexec/java_home', ['-v', String(major)], {
      encoding: 'utf8',
      shell: false,
    });
...
function reshapeJdk(jdkCheck, catalogueCheck) {
  ...
  return {
    ...
    java_home: process.env.JAVA_HOME || resolveDarwinJavaHome(jdkCheck.value) || null,
