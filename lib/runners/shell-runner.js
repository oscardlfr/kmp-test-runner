...
//
// Pure helpers for resolving the script wrapper to invoke (sh on POSIX,
// ps1 on Windows) and for translating bash long-flags to their PowerShell
// PascalCase form before the spawn. cli.js re-exports every name through
// its existing `export {}` block so external consumers (tests, orchestrators)
// keep importing from './cli.js' unchanged via ESM live bindings — same
// pattern used by lib/commands/doctor.js.
//
...
//
// POSIX `--name=value` form: split on the FIRST `=`
...
// PowerShell-binding hazards for the new --gradle-args flag.
//
