...
// v0.10 #3 — cli.js main() owns the loadMergedConfig + applyConfigDefaults
// path now (so getJavaHomeOverride sees user-global java_home). This module
// no longer re-runs the merge here, which avoids a duplicate `java_home is
// not allowed in project-local config` warn.
...
// top-level. Same pattern v0.9 step 3 / PR-04 / PR-06 use for shell-runner.
import {
...
function dispatchScriptCommand({
  ...
  if (dryRun) {
    ...
    if (jsonMode) {
      ...
      // v0.10 #2 — surface gradle_config_applied at the dispatcher-level dry-run
      // for `parallel`. The orchestrator-level dry-run (lib/orchestrators/parallel-
      ...
      // PR 3.1 — emit the subcommand-specific block on dry-run with empty-but-
  ...
  // v0.9 step 4 — `--isolated-no-lock` opts out entirely.
