...
//
// Owns the cross-cutting concerns shared by parallel/changed/android/
// benchmark/coverage:
...
import { assessIsolatedRuntimeRace } from '../orchestrators/orchestrator-utils.js';
import {
  pickWindowsShell,
  collapseGradleArgs,
...
import { resolveMaxBuffer } from '../orchestrators/orchestrator-utils.js';
import { sweepArtifacts } from '../project/artifact-sweep.js';
import {
  preflightJdkCheck,
  jdkMismatchHint,
  parseGradleConfig,
...
// JSON stdout remains machine-only. Retain bounded human diagnostics on stderr
// before the Windows temporary captures are deleted, without echoing envelopes.
export function formatFailureDiagnostics(stdout, stderr) {
  const blocks = [];
  for (const [label, stream] of [['runner stdout', stdout], ['runner stderr', stderr]]) {
    const text = String(stream ?? '').replace(
      /__KMP_TEST_ENVELOPE_V1_BEGIN__[\s\S]*?(?:__KMP_TEST_ENVELOPE_V1_END__|$)/g, '',
    ).trim();
    if (!text) continue;
    const bytes = Buffer.from(text, 'utf8');
    const truncated = bytes.length > 32600;
    let start = truncated ? bytes.length - 32600 : 0;
    // A UTF-8 tail must start on a code-point boundary.
    while (start < bytes.length && (bytes[start] & 0xc0) === 0x80) start++;
    blocks.push(`[kmp-test ${label}${truncated ? '; truncated tail' : ''}]\n${bytes.subarray(start).toString('utf8')}\n`);
  }
  return blocks.join('');
}
...
function dispatchScriptCommand({
  ...
  cleanupConfig = null, // { auto, logsTtlDays } from resolveCleanupConfig (cli.js)
}) {
  ...
  if (dryRun) {
    // Isolation-race parity: a real `parallel` run rejects `--isolated` combined
    // with ios / all / androidInstrumented-without-device (assessIsolatedRuntimeRace
    // in runParallel). The dispatcher short-circuits --dry-run BEFORE the
    // orchestrator, so without this the same combo would pass dry-run (exit 0 +
    // plan) then fail the real run (exit 2) — a false all-clear for agents that
    // validate via --dry-run. Enforce the identical verdict here, scoped to
    // `parallel` (the only subcommand whose orchestrator gates this).
    if (sub === 'parallel') {
      const ttIdx = finalArgs.indexOf('--test-type');
      const isolatedRace = assessIsolatedRuntimeRace({
        enabled: !!isolatedFlags.enabled,
        testType: ttIdx >= 0 && ttIdx + 1 < finalArgs.length ? finalArgs[ttIdx + 1] : '',
        hasDevice: finalArgs.includes('--device'),
      });
      if (isolatedRace) {
        if (jsonMode) {
          emitJson(buildInvalidArgsEnvelope({
            subcommand: sub, projectRoot, durationMs: 0, errors: [isolatedRace],
          }));
        } else {
          process.stderr.write(`kmp-test: ${isolatedRace.message}\n`);
        }
  ...
  // Artifact lifecycle sweep (TTL-based, best-effort). Runs only
  // AFTER the lock is held — never races a concurrent run's writers — which
  // also excludes --dry-run (returned above) and bypassed locks (no lock →
  // no exclusivity guarantee). Opt-out: cleanup.auto=false in config or
  // KMP_TEST_NO_SWEEP=1 (both resolved by cli.js#resolveCleanupConfig).
  if (!bypassLock && cleanupConfig && cleanupConfig.auto) {
    try {
      sweepArtifacts(projectRoot, { logsTtlDays: cleanupConfig.logsTtlDays });
    } catch { /* never blocks the run */ }
  }
  ...
  // file to remove.
  const cleanup = () => { if (!bypassLock) releaseOwnLock(projectRoot, lockResult.ourLock); };
  ...
  try {
    ...
    const spawnOpts = useTempFileRedirect
      ...
      : jsonMode
        ? { encoding: 'utf8', maxBuffer: resolveMaxBuffer(), timeout: timeoutMs, killSignal: 'SIGTERM' }
    ...
    // Thread the user-typed subcommand through the wrapper to runner.js.
    // The shared run-parallel-coverage-suite wrapper rewrites
    // `parallel --skip-tests` into a `coverage` invocation and CANNOT
    // distinguish it from `kmp-test coverage` (the coverage prefix injects
    // the same --skip-tests — identical argv); only this layer still knows
    // which sub the user typed. runner.js's coverage branch reads the env
    // (allowlisted) so the markdown report header says "No (--skip-tests)"
    // vs "No (coverage subcommand)" correctly. NOT gated on jsonMode — the
    // header is written in human mode too. Direct wrapper invocations (env
    // absent) keep the 'coverage' default.
    if (cmd.migrated) {
      spawnOpts.env = { ...(spawnOpts.env || process.env), KMP_ORIGINATING_SUBCOMMAND: sub };
    }
    ...
    if (result.error) {
      // Every spawn-layer error emits a JSON envelope in --json mode. The
      // pre-fix code emitted one only for ENOENT — any other error (e.g.
      // maxBuffer exceeded, EAGAIN) wrote plain stderr and exited without
      // JSON, breaking the envelope contract for agents.
      const classified = classifySpawnError(result.error, { isWin, spawnCmd });
      if (jsonMode) {
        emitJson(envErrorJson({
          subcommand: sub, projectRoot, durationMs,
          message: classified.message, code: classified.code,
        }));
      } else {
        process.stderr.write(`kmp-test: ${classified.message}\n`);
      }
      // ENV_ERROR (3) for all spawn-layer failures — consistent with the
      // sibling missing_shell / gradle_timeout paths. (Pre-fix the non-ENOENT
      // branch returned TEST_FAIL (1), which mis-told agents "tests failed".)
      return EXIT.ENV_ERROR;
    }
    ...
    let envelopeParseFailed = false;
    if (cmd.migrated && jsonMode) {
      const { envelope, sentinelSeen } = extractMigratedEnvelopeDetailed(capturedStdout);
      if (envelope) {
        envelope.exit_code = resolveDispatchExitCode(capturedStdout, scriptStatus);
        if (envelope.exit_code !== EXIT.SUCCESS) {
          const diagnostics = formatFailureDiagnostics(capturedStdout, capturedStderr);
          if (diagnostics) process.stderr.write(diagnostics);
        }
        emitJson(envelope);
        return envelope.exit_code;
      }
      // No envelope → legacy parser fallback. Sentinel ABSENT is the normal
      // legacy shape (silent). Sentinel PRESENT means the orchestrator
      // emitted but the bracketed JSON didn't survive (truncated stdout,
      // interleaved writes) — tag the fallback so agents can discriminate a
      // degraded envelope from a native legacy one.
      envelopeParseFailed = sentinelSeen;
    }
    ...
    if (envelopeParseFailed) {
      if (!Array.isArray(parsed.warnings)) parsed.warnings = [];
      parsed.warnings.push({
        code: 'envelope_parse_failed',
        reason: 'json_parse_failed',
        message: 'orchestrator envelope sentinel present but its JSON did not parse — fell back to the legacy output parser (results may be coarser)',
      });
    }
...
// Resolve the canonical exit code for a migrated subcommand result.
//
// Pass capturedStdout (containing the sentinel-wrapped envelope) for the JSON
// path — the function extracts the envelope and applies the WS-5 invariant to
// envelope.exit_code, honouring SOFT_ERROR_CODES (including gradle_timeout for
// benchmark partial-timeout grading).
// Pass '' for the text path (inherited stdio, nothing captured) — the function
// falls through to enforceErrorsExitCodeInvariant(subprocessStatus, {errors:[]})
// which is always a no-op since captured errors[] is empty.
//
// Exported so the parity test suite can exercise both call shapes directly.
export function resolveDispatchExitCode(capturedStdout, subprocessStatus) {
  const { envelope } = extractMigratedEnvelopeDetailed(capturedStdout || '');
  if (envelope) {
    return enforceErrorsExitCodeInvariant(
      envelope.exit_code ?? subprocessStatus,
      { tests: envelope.tests, errors: envelope.errors }
    );
  }
  return enforceErrorsExitCodeInvariant(subprocessStatus, { errors: [] });
}

// Classify a spawnSync error into an envelope-ready { code, message } pair.
// Pure — unit-testable without spawning. ENOENT keeps its historical
// missing_shell code + message; everything else discriminates as
// spawn_error (matching the orchestrator-level code for gradle children),
// with a targeted hint when the wrapper's own output overflowed maxBuffer.
function classifySpawnError(error, { isWin = process.platform === 'win32', spawnCmd = '' } = {}) {
  if (error && error.code === 'ENOENT') {
    const missing = isWin ? 'pwsh/powershell' : 'bash';
    return {
      code: 'missing_shell',
      message: `'${missing}' not found on PATH. Install PowerShell (Windows) or bash (Unix).`,
    };
  }
  // Overflow errno differs by platform: ERR_CHILD_PROCESS_STDIO_MAXBUFFER on
  // POSIX, ENOBUFS on Windows (wet-observed on the S22 session host).
  if (error && (error.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER' || error.code === 'ENOBUFS')) {
    return {
      code: 'spawn_error',
      message: `wrapper output exceeded maxBuffer — raise KMP_GRADLE_MAXBUFFER_MB (failed to spawn '${spawnCmd}': ${error.message})`,
    };
  }
  return {
    code: 'spawn_error',
    message: `failed to spawn '${spawnCmd}': ${(error && error.message) || error}`,
  };
}

export { dispatchScriptCommand, dedupBooleanFlags, KNOWN_BOOLEAN_FLAGS, classifySpawnError };
