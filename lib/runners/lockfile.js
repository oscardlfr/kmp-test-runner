...
// Ownership-verifying release. Re-reads the current lockfile and unlinks
// only when pid + start_time still match ourLock — guards the --force
// takeover race: process B overwrites A's lock; A's cleanup must not
// delete B's fresh lock.
// pid + start_time are the ownership identity in schema v1 — no extra token needed.
// When ourLock is absent or lacks identity (bypassed-lock path), is a no-op.
function releaseOwnLock(projectRoot, ourLock) {
  if (!ourLock || typeof ourLock.pid !== 'number' || typeof ourLock.start_time !== 'string') {
    return;
  }
  const current = readLockfile(projectRoot);
  if (!current || current.invalid) return;
  if (current.pid === ourLock.pid && current.start_time === ourLock.start_time) {
    try { unlinkSync(lockfilePath(projectRoot)); } catch { /* best-effort */ }
  }
}
