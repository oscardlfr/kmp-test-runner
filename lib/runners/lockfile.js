...
// Atomic create-if-absent write ('wx' = O_CREAT|O_EXCL|O_WRONLY). When the
// lockfile already exists this throws EEXIST instead of silently overwriting
// a concurrent winner's lock — closing the read→check→write TOCTOU race the
// previous acquireLock had (two simultaneous processes could both pass the
// liveness check and both "acquire").
function writeLockfileExclusive(projectRoot, subcommand) {
  const lock = buildLockPayload(projectRoot, subcommand);
  const fd = openSync(lockfilePath(projectRoot), 'wx');
  try {
