...
// executed).
//
// Falls back to the raw comparison only if realpath resolution itself
// throws for EITHER path (e.g. deleted between argv construction and this
// check) — at that point neither path can be resolved to a canonical form,
// so a canonical comparison isn't possible either way. Known residual risk:
// if realpath throws for some other transient reason (fd exhaustion,
// permissions) on the exact symlinked-temp-dir case this guard exists for,
// the fallback reproduces the original bug. Accepted trade-off — the
// alternative (treating "can't tell" as "is the entrypoint") risks running
// main() when this file was genuinely imported as a module instead.
