...
async function main() {
  ...
  // invocation of the wrapper (bypassing cli.js, e.g. a bats test spawning
  // the wrapper script directly) still BLOCKs on toolchain mismatch. When invoked via cli.js the upstream
  // gate already ran:
...
// Guard: only invoke main() when this file is the entry point (not when
// imported by tests). Compares realpaths, not raw strings — process.argv[1]
// keeps whatever literal path form the caller passed (e.g. the Gradle
// plugin's RuntimeExtractor extracts to a JVM temp dir, which on macOS
// resolves under /var/folders/... — a symlink to /private/var/folders/...),
// while Node's ESM loader canonicalizes import.meta.url through that same
// symlink. A raw string comparison mismatches on the identical file and
// main() silently never runs (exit 0, zero output — no error, since nothing
// executed).
//
// The literal-equality fast path below already ruled out argvPath ===
// modulePath, so if realpath resolution then throws for EITHER path (e.g.
// deleted between argv construction and this check, fd exhaustion,
// permissions) neither path can be resolved to a canonical form and we
// already know they're not textually identical — treat "can't tell
// canonically, and not equal as literal strings" as not-the-entrypoint.
// Known residual risk: this is fail-closed by design, but on the exact
// symlinked-temp-dir case this guard exists for, a realpath throw here means
// main() still won't run. Accepted trade-off — the alternative (treating
// "can't tell" as "is the entrypoint") risks running main() when this file
// was genuinely imported as a module instead.
export function isRunnerEntrypoint(argv1 = process.argv[1], moduleUrl = import.meta.url) {
  if (!argv1) return false;
  const argvPath = path.resolve(argv1);
  const modulePath = fileURLToPath(moduleUrl);
  if (argvPath === modulePath) return true; // already identical — skip the realpath syscalls
  try {
    return realpathSync.native(argvPath) === realpathSync.native(modulePath);
  } catch {
    return false; // argvPath !== modulePath already established above
  }
}
