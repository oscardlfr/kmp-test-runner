...
// The CLI re-exports runDoctor / runDoctorChecks from this file via its
// export block so external consumers (info-orchestrator, tests) keep
// importing from './cli.js' through ESM live bindings.
...
import { readGradleJavaHome } from '../project/jdk-preflight.js';
...
function runDoctorChecks(projectRoot, isWin = process.platform === 'win32') {
  ...
  // gradle.properties org.gradle.java.home diagnostic. Gradle does NOT
  // expand `~` (or any shell-ism) in this property and fails to launch on a
  // non-existent path; the jdk-preflight gate also ignores unusable values
  // (it falls through to signal detection). Surface the literal-path verdict
  // so the user sees what gradle will see. No row when the property is unset
  // (the common case — keep doctor output focused).
  try {
    const gradleJavaHome = readGradleJavaHome(projectRoot);
    if (gradleJavaHome) {
      if (gradleJavaHome.hasTilde) {
        checks.push({
          name: 'gradle java.home',
          status: 'WARN',
          value: gradleJavaHome.raw,
          message: 'Gradle does not expand ~ here — use an absolute path',
        });
      } else if (!gradleJavaHome.exists) {
        checks.push({
          name: 'gradle java.home',
          status: 'WARN',
          value: gradleJavaHome.raw,
          message: 'path does not exist — gradle will fail to launch with this org.gradle.java.home',
        });
      } else {
        checks.push({
          name: 'gradle java.home',
          status: 'OK',
          value: gradleJavaHome.raw,
          message: 'org.gradle.java.home set and path exists (wins over JDK auto-select)',
        });
      }
    }
  ...
  // User-global config (~/.kmp-test/config.json) presence + matched
  ...
  if (process.env.KMP_TEST_SKIP_ADB === '1') {
    ...
    if (!adb.error && adb.status === 0) {
      ...
      // PATH miss — try <sdk.dir>/platform-tools/adb(.exe) before warning.
      // The local.properties block above already validated sdk.dir; we
      // re-call inspectLocalProperties (cheap file read) to keep this
      // check self-contained without lifting state across blocks.
      let sdkAdbHit = null;
      const lp = inspectLocalProperties(projectRoot);
      if (lp && lp.ok) {
        const adbExe = isWin ? 'adb.exe' : 'adb';
        const sdkAdbPath = path.join(lp.path, 'platform-tools', adbExe);
        const sdkAdb = spawnSync(sdkAdbPath, ['version'], { encoding: 'utf8' });
        if (!sdkAdb.error && sdkAdb.status === 0) {
          const out2 = (sdkAdb.stdout || '') + (sdkAdb.stderr || '');
          const m2 = out2.match(/Version (\S+)/);
          sdkAdbHit = { value: m2 ? m2[1] : 'available', path: sdkAdbPath };
        }
      }
      if (sdkAdbHit) {
        checks.push({
          name: 'ADB',
          status: 'OK',
          value: sdkAdbHit.value,
          message: `via SDK at ${sdkAdbHit.path}`,
        });
      } else {
        checks.push({
          name: 'ADB',
          ...
          value: 'not found',
          message: lp && lp.ok
            ? `not on PATH and not at ${path.join(lp.path, 'platform-tools', isWin ? 'adb.exe' : 'adb')}`
            : 'install Android SDK platform-tools to run android subcommand',
        });
      }
    }
  }
...
function runDoctor(args, jsonMode) {
  ...
  if (jsonMode) {
    // Unify envelope shape across subcommands.
