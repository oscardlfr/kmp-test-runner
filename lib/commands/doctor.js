...
function runDoctorChecks(projectRoot, isWin = process.platform === 'win32') {
  ...
  if (process.env.KMP_TEST_SKIP_ADB === '1') {
    ...
    if (!adb.error && adb.status === 0) {
      ...
      // PR 3.4 A7 — before warning, fall back to <sdk.dir>/platform-tools/adb
      // (or adb.exe on Windows). doctor's `local.properties` block above
      // already validated sdk.dir; re-call inspectLocalProperties so the ADB
      // check stays self-contained (cheap file read, no caching needed).
      let sdkAdbHit = null;
      const lp = inspectLocalProperties(projectRoot);
      if (lp && lp.ok) {
