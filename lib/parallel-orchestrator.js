...
import { defaultAdbProbe, spawnGradle, readPackageName } from './orchestrator-utils.js';
...
function dispatchLeg({
  spawn, gradlewPath, projectRoot, taskList, testType, opts, env, log,
}) {
...
export async function runParallel({
  ...
  for (const leg of legs) {
    ...
    const r = await executeLeg({
      ...
      state, runCoverageInjection, deviceSerial: resolvedDeviceSerial,
    });
    allStdout += r.stdout + '\n';
    allStderr += r.stderr + '\n';
    const legEntry = {
