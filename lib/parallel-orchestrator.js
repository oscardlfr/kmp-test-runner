...
function androidConnectedTask(gradlePath, variant, mod) {
  const v = (variant || 'auto').toLowerCase();
  if (v === 'debug')   return `${gradlePath}:connectedDebugAndroidTest`;
  if (v === 'release') return `${gradlePath}:connectedReleaseAndroidTest`;
  if (v === 'all')     return `${gradlePath}:connectedAndroidTest`;  // umbrella
  if (mod && mod.testBuildType === 'release') return `${gradlePath}:connectedReleaseAndroidTest`;
  return `${gradlePath}:connectedDebugAndroidTest`;
}
...
function pickGradleTaskFor(mod, testType, opts = {}) {
  ...
  switch (testType) {
    ...
    case 'androidInstrumented': {
      ...
      // set (NOT `androidInstrumentedTest`). Gate is opt-in-driven via
      // `withDeviceTestBuilder {}` declared inside `androidLibrary {}`.
      // The probe path above (r.deviceTestTask) already includes
      // `androidConnectedCheck` as a candidate (project-model.js:950) so
      // this fallback only fires when gradle ./tasks didn't run.
      if (mod.androidDslVariant === 'kmpAndroidLibrary') {
      ...
      // 2026-05-04 Bug A — source-set guard for the fallback path. The
      // `if (t) return ...` early-return above uses r.deviceTestTask which is
      // already source-set aware (project-model populates it only when
      // androidInstrumentedTest exists). The fallback below was the
      // over-dispatch path — now requires explicit source-set evidence.
      const hasInstrumentedSrc = ss.androidInstrumentedTest || ss.androidTest;
      if ((mod.type === 'android' || mod.androidDsl) && hasInstrumentedSrc) {
        // 2026-05-05 fix-PR-F — honor --variant + mod.testBuildType (mirror
        // of androidUnitTask). Repro: dipatternsdemo :benchmark's
        // `testBuildType = "release"` means AGP only generates
        // connectedReleaseAndroidTest; the previous hardcoded Debug task
        // failed with task_not_found.
        return { task: androidConnectedTask(gradlePath, opts.androidVariant, mod), reason: '' };
      }
...
async function executeLeg({
  ...
  return { stdout, stderr, exit, execution: execSummary, cascadeDetected, retryFired };
}
