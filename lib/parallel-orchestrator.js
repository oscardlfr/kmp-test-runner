...
function expandNoCoverageAlias(argv) {
  ...
  for (const a of argv) {
    ...
    // Wet audit 2026-05-08 (cell I1c) surfaced: the switch in parseArgs
    // matches the FULL token, so `--gradle-args=--info` (and any other
    // `--flag=value` POSIX form) was silently dropped. Split `--flag=value`
    // into `[--flag, value]` so callers can use either `--flag value` or
    // `--flag=value`. Restricted to tokens that *begin* with `--<name>=`
    // (no whitespace before `=`) — value strings passed via the
    // space form (`--gradle-args "--no-parallel -Pfoo=bar"`) must NOT be
    // split, since `=` inside the value is part of a gradle property.
    const eq = a.indexOf('=');
...
// Repro: dipatternsdemo :benchmark sets `testBuildType = "release"`, so
...
// Pre-fix wet runs emitted bare strings (`["core-result"]`) while list-only
...
function pickGradleTaskFor(mod, testType, opts = {}) {
  ...
  switch (testType) {
    ...
    case 'androidUnit': {
      ...
      // Repro: shared-kmp-libs `:core-firebase-native` has the dir on disk
    ...
    case 'androidInstrumented': {
      ...
      // `gradleTasks` (e.g. dipatternsdemo `:benchmark` post the
...
async function executeLeg({
  ...
  // before EOL → strict regex misses). Repro: shared-kmp-libs Win post-toolchain
  ...
  for (let i = 0; i < taskList.length; i++) {
    ...
    if (!state.modules.some(em => em.name === mod)) {
      state.modules.push(canonicalModuleEntry(moduleByName.get(mod) || { name: mod }));
    }
    ...
    state.tests.individual_total += junitTestCountFor(
      projectRoot,
      task,
  ...
  // from their original bucket. Repro envelopes: shared-kmp-libs Mac `macos`
...
export async function runParallel({
  ...
  // 2026-05-05 v0.9 step 1 (flag #4) — `--flavor <name>` warning when no
  // module on the leg declares productFlavors {} (mod.hasFlavor=false).
  // The flavor weave inside `androidConnectedTask` no-ops for flat modules,
  // but the user supplying --flavor likely expects it to take effect
  // somewhere — surface a structured warning so agents and humans can spot
  // the misconfiguration without inspecting gradle task names. Fires for
  // any androidInstrumented-touching test type (single + 'all') and lands
  // on the envelope before any early-return so dry-run / empty-module
  // paths surface it too.
  ...
  if (opts.flavor && flavorAffectsLeg
    ...
    state.warnings.push({
      code: 'flavor_unused',
      message: `--flavor "${opts.flavor}" supplied but no discovered module declares productFlavors {} (flag is a no-op)`,
      flavor: opts.flavor,
    });
  }
  ...
  const wantsAdb = (opts.device || opts.clearData) && hasInstrumentedLeg;
