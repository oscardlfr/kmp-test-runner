...
async function executeLeg({
  ...
  const CRITICAL_RE = /FAILED$|^FAILURE:|^BUILD (FAILED|SUCCESSFUL)|^\* What went wrong:|^\* Where:|^\* Try:|^>\s*Could not|UnsupportedClassVersionError|Failed to install|Cannot locate|requires Java|Android Gradle plugin requires|Plugin .* not found|Could not resolve all|^Caused by:|Compilation error|Execution failed for task|^[\d]+:\s+Task failed|There were failing tests|^\s+>\s+/;
