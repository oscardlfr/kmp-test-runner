...
async function executeLeg({
  ...
  // wide-smoke 2026-05-03). Retry each module's task in its own gradle
  // invocation to isolate culprits. The merged stdout/stderr feeds the
  // existing classifyTaskResults so per-module truth surfaces.
  ...
  //
  // New strategy: forward EVERY meaningful gradle line (any task line with
  // its status, any failure-block line). Empty lines + dependency-noise
  // (e.g. compilation progress) stay dropped. Cap at 60 lines per leg so
  // the human banner doesn't drown in a 10K-line failure dump.
  const allLines = (stdout + '\n' + stderr).split(/\r?\n/);
  const errLines = allLines.filter(line =>
    /^>\s*Task\s|FAILED$|^FAILURE:|^BUILD (FAILED|SUCCESSFUL)|^\* What went wrong:|^\* Where:|^\* Try:|^>\s*Could not|UnsupportedClassVersionError|Failed to install|Cannot locate|requires Java|Android Gradle plugin requires|Plugin .* not found|Could not resolve all|Configuration on demand|^Caused by:/.test(line));
  const capped = errLines.slice(0, 60);
  for (const line of capped) log(line);
  if (errLines.length > capped.length) {
    log(`  (… ${errLines.length - capped.length} more gradle lines suppressed; raw output in <project>/build/reports/)`);
  }

  // Step 5 — classify per-task pass/fail and record per-module results.
  const all = stdout + '\n' + stderr;
  const resolutionFailed = /Cannot locate tasks? that match/.test(all);
  const results = classifyTaskResults(stdout, stderr, taskList, exit);
  for (let i = 0; i < taskList.length; i++) {
    const task = taskList[i];
