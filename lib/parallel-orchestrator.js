...
function pickGradleTaskFor(mod, testType) {
  ...
  switch (testType) {
    case 'common': {
      ...
      // candidate chain (jvmTest > desktopTest > test > jsTest > wasmJsTest).
      const t = r.unitTestTask;
      if (!t) return { task: null, reason: 'no common target' };
      return { task: `${gradlePath}:${t}`, reason: '' };
    }
    case 'desktop': {
      // Same resolution as common; banner differentiates.
      const t = r.unitTestTask;
      if (!t) return { task: null, reason: 'no desktop target' };
      return { task: `${gradlePath}:${t}`, reason: '' };
    }
    case 'androidUnit': {
...
export async function runParallel({
  ...
  const ENV_CODES = new Set(['no_test_modules', 'platform_unsupported',
    'task_not_found', 'instrumented_setup_failed', 'unsupported_class_version']);
