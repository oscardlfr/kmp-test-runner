...
function reshapeJdkCatalogue(catalogueCheck) {
  if (!catalogueCheck) return [];
  if (catalogueCheck.status === 'WARN' && catalogueCheck.value === 'empty') return [];
  // catalogueCheck.message is "JDK 17 (Adoptium), JDK 21 (Zulu)" — parse it
  // back into structured entries when present.
  if (!catalogueCheck.message) return [];
  return catalogueCheck.message.split(',').map(entry => {
    const m = entry.trim().match(/^JDK (\d+) \(([^)]+)\)$/);
    if (!m) return null;
    return { major: parseInt(m[1], 10), vendor: m[2] };
  }).filter(Boolean);
}
...
export function runInfo({
  ...
  const info = {
    ...
    jdk_catalogue: reshapeJdkCatalogue(findCheck(checks, 'JDK catalogue')),
