...
function expandNoCoverageAlias(argv) {
  ...
  for (const a of argv) {
    ...
    // Wet audit 2026-05-08 (cell I1c) surfaced: parseArgs matches the full
    // token, so `--coverage-tool=kover` (POSIX = form) was silently dropped.
    // Split `--flag=value` so both `--flag value` and `--flag=value` work.
    // Restricted to tokens that begin with `--<name>=` (no whitespace before
    // `=`) so value strings carrying `=` internally aren't broken.
    const eq = a.indexOf('=');
