...
//
// Precedence: loadMergedConfig (lib/project-config.js) merges the CONFIG
// layers only — project-local > user-global > built-in default. CLI flags
// win over the merge result via cli.js#applyConfigDefaults (flags are
// re-injected only when absent), and env vars are NOT a merge layer: they
// influence specific features outside the merge (JDK selection via
// JAVA_HOME, skip-lists via SKIP_*_MODULES — applied env-first with
// config-fallback by the orchestrators).
//
