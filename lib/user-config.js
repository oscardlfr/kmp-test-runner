...
// lib/user-config.js — v0.10 #3 user-global config loader.
...
export function loadUserGlobalConfig(homeOverride) {
  ...
  try {
    ...
    userWarn(`read failed: ${e.message} — ignoring`);
    return null;
  }
  let parsed;
  try {
    ...
    userWarn(`parse failed: ${e.message} — ignoring`);
    return null;
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    userWarn('top-level must be a JSON object — ignoring');
    return null;
  }
  return parsed;
}
...
function validateUserPreset(cfg) {
  const out = {};
...
export function loadUserGlobalPresetForProject(projectRoot, opts = {}) {
  ...
  return validateUserPreset(preset);
}
...
export function mergeConfigs(userPreset, projectLocal) {
