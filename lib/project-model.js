...
import {
  ...
  detectAgpVersion,
  agpRequiredJdk,
} from './project/jdk-signals.js';

// Re-export for back-compat: cli.js + tests import aggregateJdkSignals
// from this module via ESM live binding (PR-03 refactor pre-v0.10).
...
// Fingerprint of THIS file's source. Surfaced by wet audit 2026-05-08:
// `cacheKey` only hashes project inputs (settings/properties/build files), so
// caches generated before a project-model change (e.g. Bug #4 fix in
// commit ae02317) silently keep returning outputs from the OLD logic until the
// project itself changes. By embedding a fingerprint of `project-model.js`
// itself, any modification to the introspector auto-invalidates existing
// caches — no manual SCHEMA_VERSION bump required. Tasks-<sha>.txt (raw
// gradle probe output) is independent of the introspector and stays valid,
// so re-building the model is Node-only (~ms), not a re-probe.
const MODEL_FINGERPRINT = (() => {
  try {
    const src = readFileSync(fileURLToPath(import.meta.url), 'utf8');
    return crypto.createHash('sha1')
      .update(src.replace(/\r/g, '').replace(/\n+$/, ''))
...
// detectAgpVersion, aggregateJdkSignals) extracted to ./project/jdk-signals.js
// in PR-03 (refactor pre-v0.10). Imported above + re-exported for back-compat.
...
//
// `appliedPlugins[]` (v0.9 step 9.2 — Bug #2 fix) records which plugin ids
...
//   - `pluginManager.apply("literal-id")` (shared-kmp-libs pattern)
...
export function parseBuildLogicPluginDescriptors(projectRoot, catalog = null) {
  ...
  // Walk all `*.gradle.kts` and `*.kt` under build-logic looking for:
  ...
  // class-name heuristic missed (e.g. shared-kmp-libs's neutrally-named
...
function extractAppliedPluginsFromConventionSource(content) {
  if (!content) return [];
  const stripped = stripGradleComments(content);
  const out = new Set();
  ...
  // 4. Precompiled-script `plugins { id("…") }` and the `id("…")` form
  // generally. Bare `id = …` (with `=`) does not match — the open-paren
  // requirement disambiguates from `register() { id = … }` in build files.
...
export function analyzeModule(projectRoot, moduleName, buildLogicHints = null, catalog = null, buildLogicDescriptors = null) {
  ...
  // v0.9 step 9.2 (Bug #2) — expand alias-resolved ids through convention-plugin
  ...
  // plugin id OR alias resolution (shared-kmp-libs uses
  // `libs.plugins.android.kotlin.multiplatform.library` alias) — and by
  // expansion through convention-plugin appliedPlugins (v0.9 step 9.2).
  ...
  if (kmpPluginIdPattern.test(content) || hasAndroidLibraryDsl || hasAndroidTargetCall || hasKmpViaAlias || hasNewKmpAndroidPlugin) {
    ...
    // direct alias OR via convention plugin (e.g. shared-kmp-libs's
    // `:benchmark-network` applies `com.grinx.shared.kmp.benchmark` which
    // internally applies the new plugin).
  ...
  if (androidDslVariant === 'kmpAndroidLibrary') {
    ...
    // shared-kmp-libs `:core-firebase-native` has `src/androidUnitTest/` on
    // disk but no `withHostTestBuilder {}` declared → AGP creates no
  ...
  // exist. Repro: dipatternsdemo `:benchmark` declares
  // `testBuildType = benchmarkBuildType` where
  // `val benchmarkBuildType = (project.findProperty(...) as? String) ?: "release"`.
  // 2026-05-05 fix-PR-F follow-up — also resolves single-file variable
  ...
  if (tbtMatch) {
    ...
    if (tbtVarMatch) {
      ...
      // dipatternsdemo :benchmark shape).
...
export function resolveTasksFor(_moduleName, gradleTasks, analysis = null) {
  ...
  // exposing BOTH variants (e.g. dipatternsdemo `:benchmark` after the
...
export { SCHEMA_VERSION, MODEL_FINGERPRINT, CACHE_DIR_NAME, LEGACY_CACHE_DIR_NAME };
