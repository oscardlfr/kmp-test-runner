...
function main() {
  ...
  // Dangling / empty `--isolated-cache-dir` is a usage error. Reject BEFORE
  // the acquireLock decision and the wrapper spawn (mirrors the --color and
  // enum/numeric gates above): pre-fix, isolation silently stayed off while
  // the Tier 1 lock was still taken and the token leaked into orchestrator
  // args.
  if (isolatedFlags.errors.length > 0) {
    if (jsonMode) {
      emitJson(buildInvalidArgsEnvelope({
        subcommand: sub,
        projectRoot: path.resolve(getProjectRoot(cleanedArgs)),
        durationMs: 0,
        errors: isolatedFlags.errors,
      }));
    } else {
      for (const err of isolatedFlags.errors) {
        process.stderr.write(`kmp-test ${sub}: ${err.message}\n`);
      }
    }
    return EXIT.CONFIG_ERROR;
