...
import { expandPosixEqualsForm, validateEnum, validateNonNegativeInt, requireFlagValue } from './orchestrators/orchestrator-utils.js';
...
import {
  EXIT, ENVELOPE_SCHEMA_VERSION, ASYNC_DEFERRED,
  classifyExitCode, CONFIG_ERROR_CODES, ENV_ERROR_CODES,
} from './envelope/exit-codes.js';
import {
  readVersion,
  buildJsonReport,
...
import { ALL_KNOWN_FLAGS, VALUE_BEARING_FLAGS } from './parsers/argv-constants.js';
...
import * as cleanCmd     from './commands/clean.js';
import { resolveCleanupConfig } from './project/artifact-sweep.js';
...
// Detailed variant for the dispatcher: distinguishes "sentinel absent"
// (normal legacy script output — silent fallback) from "sentinel present but
// the bracketed JSON didn't survive" (truncated/corrupt orchestrator
// envelope) so the legacy fallback can carry a discriminable
// envelope_parse_failed warning. extractMigratedEnvelope above stays
// byte-identical on purpose — it is hand-mirrored by the
// tools/wide-smoke-pass-*.mjs probes.
function extractMigratedEnvelopeDetailed(stdout) {
  const sentinelSeen = !!stdout && stdout.includes(MIGRATED_ENVELOPE_BEGIN);
  return { envelope: extractMigratedEnvelope(stdout), sentinelSeen };
}
...
Options:
  ...
  --test-type <type>         all | common | jvm | desktop | android | androidUnit |
  ...
  --capture-on-fail          On instrumented-module failure, capture a device
                             ...
                             (capture_error when adb can't oblige). Once per failed
                             module, post-retry. Emulators included.
  --capture-dir <path>       Override where --capture-on-fail artifacts land
                             (default: the per-run .kmp-test-runner/logs/android
                             /<runId> dir). Implies --capture-on-fail. Relative
                             paths resolve against --project-root.
...
Example:
  ...
  kmp-test parallel --test-type androidInstrumented --device <DEVICE_SERIAL> \\
  ...
  kmp-test parallel --test-type androidInstrumented --capture-on-fail --json
`,
...
Options:
  ...
  --show-modules-only        List detected modules without running tests (preview)
...
Options:
  ...
  --test-filter <pattern>    Filter Android benchmark classes. Wildcard patterns
                             (e.g. *ScaleBenchmark*) resolve to an FQN by source scan.
                             kotlinx-benchmark JVM tasks do not support this flag;
                             matching JVM legs are skipped with a
                             test_filter_unsupported warning.
  --variant | --android-variant <value>
...
`,
  coverage: `kmp-test coverage — aggregate existing coverage XML (skips tests and report tasks)

Usage: kmp-test coverage [--project-root <path>] [options]

The command may run gradlew tasks --all --quiet to discover coverage metadata
when the project-model cache is absent. It never runs tests or coverage report tasks.
...
Options:
  ...
  --output-file <path>       Custom report path. When omitted, writes
                             .kmp-test-runner/reports/coverage/<runId>.md and
                             updates latest.md. Relative paths resolve against
...
`,
  clean: `kmp-test clean — purge run artifacts under .kmp-test-runner/

Usage: kmp-test clean [--project-root <path>] [options]

Removes per-run artifacts (logs/, cache-isolated/, init-scripts/, stale
cache tmp files) regardless of age. Never touches the lockfile or
.kmp-test-runner.json. Refuses while another kmp-test run holds the
project lock. The test subcommands also auto-sweep stale artifacts on
startup (TTL-based; configure via cleanup:{auto,logsTtlDays} in
.kmp-test-runner.json or disable with KMP_TEST_NO_SWEEP=1).

Options:
  --project-root <path>      Gradle project root (default: cwd)
  --all                      Also purge the model/tasks cache (cache/) and
                             reports/ — next run re-probes gradle once
  --dry-run                  List the targets + sizes without deleting
  --force                    Proceed despite a live project lock (only when
                             the holding process is known dead)
  --json                     Emit single JSON object on stdout (agentic mode)
  --help                     Show this message

Exit codes:
  0  cleaned (or dry-run listed)
  3  refused (lock held) or 1+ target could not be removed
...
Subcommands:
  ...
  coverage   Discover metadata and aggregate existing XML (no tests/report tasks)
  ...
  clean      Purge run artifacts under .kmp-test-runner/ (--all adds caches)

Run \`kmp-test <subcommand> --help\` for subcommand-specific flags.

Cross-command options (support depends on subcommand; see each subcommand's help):
  --project-root <path>   Gradle project root (default: cwd)
  ...
  --dry-run               Preview runner-backed commands or clean without executing
                          (update rejects it; use update --check)
  --json                  Emit single JSON object on stdout (agentic mode, low-token)
  --force                 Bypass the project lock; update uses it to reinstall
                          an equal version
...
Exit codes:
  ...
  2  config error — bad CLI usage (unknown subcommand, unknown/invalid flag, missing arg)
...
function main() {
  ...
  if (!cmdModule) {
    ...
    // Stale-install hint. When the local binary predates a now-canonical
    // subcommand (e.g. an install from before `update` shipped), the help text
    // below comes from that same obsolete binary and lists only the subcommands
    // it knew — so the user has no signal that THEIR INSTALL is the problem, not
    // the requested subcommand. Point at a re-install before the (possibly
    // outdated) help. Surfaced 2026-05-25 from a user on a pre-v0.5 mac running
    // `kmp-test update` who concluded the bug was in current develop.
    ...
    process.stderr.write(
      `\nIf you expected '${sub}' to be a valid subcommand, your installed kmp-test may be out of date.\n` +
      `Update it and try again:\n` +
      `  ${reinstall}\n` +
      `  (npm: npm install -g kmp-test-runner@latest)\n\n`,
    );
    printHelp();
    return EXIT.CONFIG_ERROR;
  }
  ...
  // Orchestrator-only subs (info/describe/update/doctor/clean) take a minimal
  // call. dryRun + force ride along for `clean`, while `update` uses force for
  // an equal-version reinstall (consumed from rawArgv at main() entry, so these
  // flags are no longer in cleanedArgs); the remaining commands ignore them.
  if (!SCRIPT_BACKED_SUBS.has(sub)) {
    return cmdModule.run({ args: cleanedArgs, jsonMode, dryRun, force });
  }
  ...
  // Dangling string-valued flag as the LAST token: no value to bind. Reject
  // here, before the wrapper spawn — on Windows the ps1 wrapper's typed
  // param block otherwise dies with an unstructured parameter-binding error
  // that the legacy output parser can only classify as `no_summary`, and on
  // POSIX the bash side silently treats the flag as if omitted. Orchestrator
  // parsers carry the same guard for direct runner.js routes. Enum/numeric
  // dangling forms are already rejected by the validator loop above;
  // --test-filter / --isolated-cache-dir by their dedicated gates below.
  const lastToken = cleanedArgs[cleanedArgs.length - 1];
  if (VALUE_BEARING_FLAGS.includes(lastToken)) {
    requireFlagValue(lastToken, undefined, invalidArgErrors);
  }
  // Layer-1 unknown-flag gate: reject --flags not known to any subcommand.
  // Runs after expandPosixEqualsForm so only canonical [--flag, value] pairs
  // exist. VALUE_BEARING_FLAGS tokens advance i by 1 to skip their value so
  // that opaque values like `--gradle-args --no-parallel` are not mis-flagged.
  for (let i = 0; i < cleanedArgs.length; i++) {
    const a = cleanedArgs[i];
    ...
    invalidArgErrors.push({ code: 'unknown_flag', flag: a, message: `${a}: unknown flag` });
  }
  ...
  let cleanupConfig = resolveCleanupConfig(null);
  {
    ...
    // Artifact-sweep policy (cleanup:{auto,logsTtlDays} + KMP_TEST_NO_SWEEP)
    // resolved here where the merged config is in scope; consumed by
    // dispatchScriptCommand after lock acquisition.
    cleanupConfig = resolveCleanupConfig(_mergedCfg);
  }
  ...
  // Dangling `--test-filter` / dangling-or-empty `--isolated-cache-dir` is a
  // usage error. Reject BEFORE the acquireLock decision and the wrapper
  // spawn (mirrors the --color and enum/numeric gates above): pre-fix,
  // isolation silently stayed off while the Tier 1 lock was still taken and
  // the dangling token leaked into orchestrator args.
  const strippedFlagErrors = [...tf.errors, ...isolatedFlags.errors];
  if (strippedFlagErrors.length > 0) {
    if (jsonMode) {
      emitJson(buildInvalidArgsEnvelope({
        subcommand: sub,
        projectRoot: path.resolve(getProjectRoot(cleanedArgs)),
        durationMs: 0,
        errors: strippedFlagErrors,
      }));
    } else {
      for (const err of strippedFlagErrors) {
        process.stderr.write(`kmp-test ${sub}: ${err.message}\n`);
      }
    }
    return EXIT.CONFIG_ERROR;
