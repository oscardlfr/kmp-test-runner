...
//
// Version 2 (wet-audit v0.9 part 2 — see WET-AUDIT-V0.9-PART2-RESULTS.md):
...
function buildInvalidArgsEnvelope({ subcommand, projectRoot, durationMs, errors }) {
  return {
    tool: 'kmp-test',
    schema_version: ENVELOPE_SCHEMA_VERSION,
    subcommand,
...
function envErrorJson({ subcommand, projectRoot, durationMs, message, code, extra, exitCode = EXIT.ENV_ERROR }) {
  const err = { message };
...
function buildDryRunReport({ subcommand, projectRoot, plan, isolated = null }) {
  const out = {
    tool: 'kmp-test',
    schema_version: ENVELOPE_SCHEMA_VERSION,
    subcommand,
