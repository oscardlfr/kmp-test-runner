...
function findRequiredJdkVersion(projectRoot, maxDepth = 12) {
  const skip = new Set(['build', '.gradle', 'node_modules', '.git', '.idea', 'dist', 'out', 'target', '.vscode']);
  const patterns = [
    /jvmToolchain\s*\(\s*(\d+)\s*\)/g,
    /JvmTarget\.JVM_(\d+)\b/g,
    /JavaVersion\.VERSION_(\d+)\b/g,
  ];
...
function resolveAndroidTestFilter(pattern, projectRoot) {
  ...
  const fqn = findFirstClassFqn(projectRoot, pattern);
  return fqn || pattern;
}
...
function applySharedSignal(stdout, stderr, state) {
  ...
  // Gradle 9 deprecation notice — "[NOTICE] Gradle exited with code N but all
  // M tasks passed individually." Emitted by parallel scripts when the gradle
  // daemon exits non-zero solely due to deprecation warnings. Surface as
  // warnings[], NOT errors[], so agents can branch on real failures vs noise.
  // Match BOTH `[NOTICE]` (current) and the legacy `[!]` form.
  const deprecationMatch = all.match(/\[(?:NOTICE|!)\]\s+Gradle exited with code (\d+) but all (\d+) tasks passed individually/i);
  if (deprecationMatch) {
    state.warnings.push({
      code: 'gradle_deprecation',
      message: deprecationMatch[0].trim(),
      gradle_exit_code: +deprecationMatch[1],
      tasks_passed: +deprecationMatch[2],
    });
...
function parseScriptOutput(stdoutRaw, stderrRaw, args, subcommand) {
  ...
  if (!sawAnything) {
    state.errors.push({ message: 'no recognizable test/build summary in script output' });
  }
...
function main() {
  ...
  try {
    ...
    if (jsonMode) {
      const parsed = parseScriptOutput(result.stdout, result.stderr, finalArgs, sub);
