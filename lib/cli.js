...
Options:
  ...
  --test-type <type>         all | common | jvm | androidUnit | androidInstrumented |
...
Options:
  ...
  --test-filter <pattern>    Filter an Android benchmark class. Wildcard pattern
                             (e.g. *ScaleBenchmark*) is resolved to FQN by source scan.
                             JVM benchmark legs are skipped with
                             warnings[].code "test_filter_unsupported" because
                             kotlinx-benchmark tasks do not accept --tests.
  --variant | --android-variant <value>
...
Options:
  ...
  --output-file <path>       Report path. Default:
                             .kmp-test-runner/reports/coverage/<run-id>.md
...
function main() {
  ...
  // call. dryRun + force ride along for `clean` (consumed from rawArgv at
  // main() entry, so they're no longer in cleanedArgs); the others ignore them.
