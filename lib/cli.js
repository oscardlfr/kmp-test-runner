...
Exit codes:
  ...
  3  environment error — gradlew not found, bash/pwsh missing, JDK absent,
                         or another kmp-test is running on the same project root
                         (errors[].code: lock_held — pass --force to override)
`);
}
...
function envErrorJson({ subcommand, projectRoot, durationMs, message, code }) {
  const err = { message };
