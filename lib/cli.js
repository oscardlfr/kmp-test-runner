...
function parseScriptOutput(stdoutRaw, stderrRaw, args) {
  ...
  // Pattern 4: Gradle 9 deprecation notice — "[NOTICE] Gradle exited with
  ...
  // `[!]` form (anyone running an older script via direct invocation).
  ...
  // Skip when the only signal is the deprecation NOTICE — gradle prints
  // "BUILD FAILED" alongside the deprecation exit code, but logically it's
  // a warning, not an error.
  const buildFailedMatch = all.match(/BUILD FAILED[^\n]*/i);
  if (buildFailedMatch && !deprecationMatch) {
    errors.push({ message: buildFailedMatch[0].trim() });
  }

  // If neither test summary nor BUILD SUCCESSFUL/FAILED nor a deprecation
  // notice appeared, mark a parse gap.
  const sawSuccess = /BUILD SUCCESSFUL/i.test(all);
  if (!summaryMatch && !sawSuccess && !buildFailedMatch && !deprecationMatch) {
    errors.push({ message: 'no recognizable test/build summary in script output' });
  }

  return { tests, modules, coverage, errors, warnings };
}
