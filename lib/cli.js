...
function main() {
  ...
  // the populated tasks cache and tier 1 will engage. This costs a small
  // upfront walk of *.gradle.kts files (no subprocess) and is best-effort:
  // any failure inside buildProjectModel is swallowed so the run continues.
  try {
  ...
  try {
    ...
    // Bug H (no watchdog): spawnSync's timeout option fires SIGTERM and sets
    // result.signal. Surface a structured `gradle_timeout` error instead of a
    // generic test failure — agents need to distinguish "tests failed" from
    // "gradle hung and we killed it" (typical cause: JDK switch confusing the
    // daemon, or a misconfigured project causing infinite recursion).
