...
function applyErrorCodeDiscriminators(stdout, stderr, state) {
  ...
  // with only `composeApp` that itself has no tests) or the user's
  // `--module-filter` excluded everything. Discriminates the most common
  // parse-gap cause; surfaced from v0.6.1 stress test against Nav3Guide-scenes
  // and kmp-production-sample. Match the wrapper's exact line on stdout.
  const noModulesMatch = stdout.match(/^\[ERROR\] No modules found matching filter[^\n]*/m);
  if (noModulesMatch) {
    state.errors.push({
