...
// v0.10 #3: loadMergedConfig combines user-global (~/.kmp-test/config.json)
// with project-local. Injected here so getJavaHomeOverride downstream sees
...
// Refactor PR-05 — argv parsers (consume*/get*/peek*/expand*/extract*) extracted
// to lib/parsers/argv.js, and applyErrorCodeDiscriminators centralized in
// lib/envelope/error-codes.js. cli.js re-exports every name through its
// `export {}` block at the bottom so existing consumers (tests, orchestrators,
// commands/doctor.js) keep importing from './cli.js' unchanged via ESM live
// bindings.
import {
  getProjectRoot,
...
// Refactor PR-06 — shell/PowerShell helpers extracted to lib/runners/. cli.js
// re-exports these names through its `export {}` block at the bottom so existing
// consumers (cli.test.js) keep importing from './cli.js' unchanged via ESM live
// bindings. shell-runner.js imports COMMANDS back from this file (live binding)
// since main()'s dispatcher and resolveScript() share that registry.
import {
  pickWindowsShell,
...
// Refactor PR-09 — concurrent-invocation lockfile helpers extracted to
// lib/runners/lockfile.js. cli.js re-exports through the `export {}` block at
// the bottom so existing consumers (cli.test.js, Pester Concurrency.Tests.ps1)
// keep importing from './cli.js' unchanged via ESM live bindings.
import {
  lockfilePath,
  isPidAlive,
  readLockfile,
...
// Refactor PR-09 — `--test-filter` pattern resolution helpers extracted to
// lib/parsers/test-filter.js. cli.js re-exports through the `export {}` block
// at the bottom (live bindings) for cli.test.js + orchestrator consumers.
import {
  findFirstClassFqn,
  splitClassMethod,
...
// Refactor PR-09 — script stdout/stderr parsers extracted to
// lib/parsers/script-output.js. cli.js re-exports through the `export {}`
// block at the bottom. parseScriptOutput is the canonical entry; the
// per-subcommand helpers (parseAndroidSummary, parseBenchmarkSummary, etc.)
// stay exposed so cli.test.js + Pester tests keep working.
import {
  stripAnsi,
  applySharedSignal,
  parseAndroidSummary,
...
// Refactor PR-09 — JDK preflight + gradle.properties parser extracted to
// lib/project/jdk-preflight.js. cli.js re-exports through the `export {}`
// block at the bottom (live bindings) for cli.test.js + doctor.js consumers.
import {
...
// Refactor PR-09 — script-backed spawn pipeline + commands dispatch table.
// Each command module exposes `parse(args) + run({...})`. Script-backed subs
// (parallel/changed/android/benchmark/coverage) forward to dispatchScriptCommand.
// Orchestrator-only subs (info/describe/update/doctor) own their run() body.
...
// Refactor PR-09 — the dispatch table. Hoists global flags, resolves the
// subcommand module, normalizes argv (alias / POSIX-equals / coverage-only),
// runs pre-spawn enum/int validation, then forwards to the command's run().
// Script-backed subs (parallel/changed/android/benchmark/coverage) get the
// full pipeline context (gradlew check, JDK preflight, lockfile, spawn) via
...
function main() {
  ...
  // v0.10 #1 — global `--color={always,never,auto}` strip. Sets module-level
  // state read by spawnGradle (lib/runners/console-mode.js). Invalid values
  ...
  // v0.10 #3 — merge user-global + project-local config and inject defaults
  ...
  // v0.9 step 4 — peek for --isolated* flags. Pass-through to the orchestrator
...
export {
  ...
  COMMANDS,
  EXIT,
  // Refactor PR-06: re-exported from ./runners/shell-runner.js (do not delete —
