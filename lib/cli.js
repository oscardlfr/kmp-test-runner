...
// 2026-05-03: Android SDK auto-detection. Surfaces in `kmp-test doctor`
// and is auto-set as ANDROID_HOME by orchestrators when env doesn't carry it.
...
import { loadProjectConfig, applyConfigDefaults, CONFIG_FILE_NAME } from './project-config.js';
...
const COMMANDS = {
  parallel:  { sh: 'run-parallel-coverage-suite.sh',  ps1: 'run-parallel-coverage-suite.ps1',  prefix: [], migrated: true },
  changed:   { sh: 'run-changed-modules-tests.sh',    ps1: 'run-changed-modules-tests.ps1',    prefix: [], migrated: true },
  android:   { sh: 'run-android-tests.sh',             ps1: 'run-android-tests.ps1',             prefix: [], migrated: true },
  benchmark: { sh: 'run-benchmarks.sh',                ps1: 'run-benchmarks.ps1',                prefix: [], migrated: true },
  coverage:  { sh: 'run-parallel-coverage-suite.sh',   ps1: 'run-parallel-coverage-suite.ps1',   prefix: ['--skip-tests'], migrated: true },
};
...
Options:
  ...
  --variant <value>          Android unit-test variant: auto (default; respects
                             testBuildType="release" projects), debug, release,
                             or all (umbrella test task — runs both Debug AND
                             Release variants). Ignored for non-Android modules.
...
function preflightJdkCheck(projectRoot) {
  ...
  return { required, current, agpVersion: sig.agpVersion };
}
...
const SOFT_ERROR_CODES = new Set(['no_summary', 'no_changed_modules']);
function enforceErrorsExitCodeInvariant(scriptStatus, parsed) {
