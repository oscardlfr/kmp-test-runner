...
import { EXIT, ENVELOPE_SCHEMA_VERSION, classifyExitCode } from './exit-codes.js';
...
export function enforceErrorsExitCodeInvariant(scriptStatus, parsed) {
  ...
  if (hardErrors.length > 0 && scriptStatus === EXIT.SUCCESS) {
    // Promotion target is classifyExitCode, not a blind TEST_FAIL — a hard
    // ENV/CONFIG-tier code (task_not_found, instrumented_setup_failed, ...)
    // promotes to its documented code, not just "any non-zero". The
    // TEST_FAIL fallback below is a floor: classifyExitCode can't return
    // SUCCESS here (hardErrors is non-empty by the guard above), but keeping
    // it guards against a future classifyExitCode change reintroducing a gap.
    const testsFailed = (parsed && parsed.tests && parsed.tests.failed) || 0;
    const classified = classifyExitCode(hardErrors, { testsFailed });
    return classified === EXIT.SUCCESS ? EXIT.TEST_FAIL : classified;
  }
  return scriptStatus;
}
