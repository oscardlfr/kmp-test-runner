...
// `coverage_budget_without_coverage`: a positive --min-missed-lines budget was
// combined with --no-coverage / --coverage-tool none — a known, detectable-
// before-dispatch usage contradiction (docs/envelope-contract.md).
export const CONFIG_ERROR_CODES = new Set([
  'flavor_unused', 'isolated_runtime_race', 'coverage_budget_without_coverage',
]);
...
// `coverage_data_unavailable`: a positive coverage budget could not be
// evaluated (no contributing data, a required target lacks fresh XML or
// failed to parse, or report generation/aggregation failed this run) —
// docs/envelope-contract.md documents the closed `reason` enum.
export const ENV_ERROR_CODES = new Set([
  'task_not_found', 'unsupported_class_version', 'instrumented_setup_failed',
  'platform_unsupported', 'gradle_timeout', 'coverage_data_unavailable',
]);
