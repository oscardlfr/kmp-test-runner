# Wide-smoke pass-8 — post-PR6 release-validation baseline

Generated: 2026-05-04T11:22:36.238Z

Orchestrator HEAD: `63a292b` (v0.8.0 PR6 — `.kmp-test-runner/` artifacts subdir + `.kmp-test-runner.json` project config + cache schema 6 → 7).

Goal: zero regression vs pass-7 baseline (3 GREEN / 14 SKIP / 13 RED-repo / 0 cascade). Any project flipping GREEN/SKIP → RED-orchestrator-cascade or RED-orchestrator blocks the v0.8.0 tag.

## Key findings

1. **0 cascade-isolation cases** — matches pass-7 baseline. PR5's execution-summary cascade signature continues to gate the retry path correctly post-PR6.

2. **13 legitimate RED-repo cases** — actual project test failures, out of scope for v0.8.0 docs PR. Affected: DawSync, dipatternsdemo, gyg, OmniSound, shared-kmp-libs, Confetti-main, nav3-recipes, nowinandroid, WakeTheCave, WakeTheCave_clean, WakeTheCave_ref, FileKit-main, PeopleInSpace-main.

3. **3 GREEN** — full sweep through orchestrator + JDK auto-select + tests passing: android-challenge, TaskFlow, kotlinconf-app-main.

4. **0 RED-orchestrator (other)** — every non-cascade orchestrator path is healthy post-PR6 path migration.

5. **PR6 surface verified**: `.kmp-test-runner/` subdir creation, dual-read fallback for legacy `.kmp-test-runner-cache/`, project-config loading, cache schema 6 → 7 — none introduced a regression.

## Bucket counts

| Bucket | Pass-7 baseline | Pass-8 actual | Δ |
|---|---|---|---|
| GREEN | 3 | 3 | 0 |
| SKIP | 14 | 14 | 0 |
| RED-repo | 13 | 13 | 0 |
...
|---|---|---|---|---|---|---|
| android-challenge | PR3 | GREEN | 43s | 0 | – | 1 testcases ran |
| DawSync | PR3 | RED-repo | 13m 23s | 1 | module_failed×48, task_not_found | cascade-isolation retry fired on all cascade legs [common, desktop, androidUnit, androidInstrumented] — modules independently broken at evaluation phase (not orchestrator bug). 48 module_failed, 16564 testcases ran in OTHER legs. |
| dipatternsdemo | PR3 | RED-repo | 43s | 1 | module_failed×3, task_not_found | cascade-isolation retry fired on all cascade legs [androidUnit, androidInstrumented] — modules independently broken at evaluation phase (not orchestrator bug). 3 module_failed, 68 testcases ran in OTHER legs. |
| dokka-markdown-plugin | PR3 | SKIP | 1s | 3 | no_test_modules | all errors are no_test_modules (legitimately empty) |
| gyg | PR3 | RED-repo | 1m 52s | 3 | no_test_modules×2, module_failed×2 | module_failed discriminator (2 module(s), 30 testcases ran) |
| OmniSound | PR3 | RED-repo | 3m 44s | 1 | module_failed×14 | cascade-isolation retry fired on all cascade legs [common, desktop] — modules independently broken at evaluation phase (not orchestrator bug). 14 module_failed, 3630 testcases ran in OTHER legs. |
| shared-kmp-libs | PR3 | RED-repo | 10m 13s | 1 | module_failed×66, task_not_found | MIXED: cascade in [androidUnit] + real failures in [androidInstrumented] (66 module_failed, 7750 testcases ran) |
| TaskFlow | PR3 | GREEN | 26s | 0 | – | 1 testcases ran |
| Confetti-main | INTERESTING | RED-repo | 1m 3s | 1 | module_failed, unsupported_class_version | module_failed discriminator (1 module(s), 133 testcases ran) |
...
| nav3-recipes | INTERESTING | RED-repo | 54s | 1 | module_failed | cascade-isolation retry fired on all cascade legs [androidUnit] — modules independently broken at evaluation phase (not orchestrator bug). 1 module_failed, 0 testcases ran in OTHER legs. |
...
| WakeTheCave | NEW | RED-repo | 1m 43s | 1 | module_failed×118 | cascade-isolation retry fired on all cascade legs [common, desktop, androidUnit, androidInstrumented] — modules independently broken at evaluation phase (not orchestrator bug). 118 module_failed, 0 testcases ran in OTHER legs. |
...
| WakeTheCave_ref | NEW | RED-repo | 29s | 3 | no_test_modules×2, module_failed×38 | cascade-isolation retry fired on all cascade legs [androidUnit, androidInstrumented] — modules independently broken at evaluation phase (not orchestrator bug). 38 module_failed, 0 testcases ran in OTHER legs. |
| FileKit-main | NEW | RED-repo | 35s | 1 | module_failed×4, task_not_found | cascade-isolation retry fired on all cascade legs [androidUnit, androidInstrumented] — modules independently broken at evaluation phase (not orchestrator bug). 4 module_failed, 56 testcases ran in OTHER legs. |
...
| kotlinconf-app-main | NEW | GREEN | 22s | 0 | – | 14 testcases ran |
| Nav3Guide-master | NEW | SKIP | 1s | 3 | no_test_modules | all errors are no_test_modules (legitimately empty) |
| PeopleInSpace-main | NEW | RED-repo | 49s | 1 | module_failed×2 | module_failed discriminator (2 module(s), 24 testcases ran) |
...
## Retrospective vs pass-7 baseline

Per-project regression check on the 8 PR3-validated projects. Pass-7 results frozen 2026-05-03; pass-8 must match.

| Project | Pass-7 bucket | Pass-8 bucket | Notes |
|---|---|---|---|
| android-challenge | GREEN | GREEN | see `.smoke/pass-8/android-challenge.err` |
| DawSync | RED-repo | RED-repo | see `.smoke/pass-8/DawSync.err` |
| dipatternsdemo | RED-repo | RED-repo | see `.smoke/pass-8/dipatternsdemo.err` |
| dokka-markdown-plugin | SKIP | SKIP | see `.smoke/pass-8/dokka-markdown-plugin.err` |
| gyg | RED-repo | RED-repo | see `.smoke/pass-8/gyg.err` |
| OmniSound | RED-repo | RED-repo | see `.smoke/pass-8/OmniSound.err` |
| shared-kmp-libs | RED-repo | RED-repo | see `.smoke/pass-8/shared-kmp-libs.err` |
| TaskFlow | GREEN | GREEN | see `.smoke/pass-8/TaskFlow.err` |

## Per-project artifacts

Forensic captures live in `.smoke/pass-8/` (gitignored — same `.smoke/` rule as pass-7):
