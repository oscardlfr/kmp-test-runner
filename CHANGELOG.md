...
- **Live verification:** `kmp-test android --list-only --module-filter=:benchmark --project-root <a multi-module DI sample>` → `instrumented_modules:["benchmark"]` (pre-fix: all 43 modules); `kmp-test describe --module-filter=:core-result --project-root <a reference KMP composite project>` → 1 module.
...
The Windows-side audit (parallel to the iOS wet audit closed in PRs #169–#174) found that `cli.js#translateFlagForPowerShell` PascalCased the entire flag token, including any value attached via the POSIX `--name=value` form. So `--module-filter=:core-result` translated to `-ModuleFilter=:coreResult` (mangling `core-result` → `coreResult`) and `--gradle-args=--rerun-tasks` to `-GradleArgs=RerunTasks` (losing the value's leading `--`). PR #167 partially closed the rollout for `parallel`/`coverage` parsers but didn't reach the PowerShell-bridge translator.
...
- **`tests/vitest/cli.test.js`** — 2 new regression tests covering the two repro inputs from the audit. Both fail pre-fix (`-ModuleFilter=:coreResult`, `-GradleArgs=RerunTasks`) and pass post-fix.
...
`kmp-test parallel --list-only` emitted `modules: [{name, type, coverage_plugin, ...}]` (object array) since v0.9 step 9.8 (Bug #7), but the wet-run path still emitted `modules: ["core-result"]` (bare string array) — agents reading the same `modules[]` field across the two execution paths had to branch on shape.
...
- **Live verification:** `parallel --test-type common --module-filter ":core-result" --project-root <a reference KMP composite project>` → `modules[0]:{name:"core-result", type:"kmp", coverage_plugin:"kover", test_build_type:null, has_flavor:false, android_dsl:true, android_dsl_variant:"kmpAndroidLibrary"}` ✓.
...
- **Live verification:** `describe :core-result --json` against a reference KMP composite project returns 1 module (`:core-result`) — identical to the explicit `--module-filter` form. Extra positional emits warning on stderr.
...
- **Live verification** (S22 <device-serial> attached): a reference KMP composite project `parallel --list-only --module-filter "core-result,core-common"` → `modules: [{name:"core-common"}, {name:"core-result"}]`, `parallel.list_only:true`, `coverage.modules_with_kover_plugin: ["core-common","core-result"]`, exit 0. a multi-module DI sample `parallel --list-only --module-filter ":benchmark"` → 1 module matched (composes correctly with Bug #3 filter normalization).
...
- **Live verification** (S22 <device-serial> attached): a reference KMP composite project `:core-result` describe envelope flips `platforms: ["android"]` → `["jvm", "android", "ios", "macos"]`. test_tasks block (already correct pre-fix) consistent: `unit:'desktopTest'`, `device:'androidConnectedCheck'`, `ios:'iosSimulatorArm64Test'`, `macos:'macosArm64Test'`.
...
- **`lib/describe-orchestrator.js`** keeps its regex semantic for power users but adds a colon-stripped dual-test fallback so `^:core-result$` and `^core-result$` both match a module whose `m.name` is `:core-result`.
...
### Documentation
- **README — token-cost provenance footnote.** Detailed cross-model tables now stamped as v0.4.0 baseline (2026-04-27, a reference KMP composite project `:core-result*`). Fresh v0.8.0 cl100k_base re-measurement against the same module added inline (parallel A:C 41× vs 127×, coverage 1170× vs 1218×, changed 7,821× vs 127× — same order of magnitude, narrative unchanged). Captures live under [`tools/runs/<feature>/`](tools/runs/).
...
- **Plugin detection**: `lib/project-model.js#analyzeModule` recognizes `kotlin { androidLibrary {} }` blocks (the `com.android.kotlin.multiplatform.library` plugin opt-in introduced in AGP 8.2 / KMP 2.0+) as the Android variant of a KMP module. Pre-fix, modules using this opt-in DSL fell through to "no Android target" because the analyzer only looked for the legacy `kotlin("android")` plugin id. a reference KMP composite project `:core-firebase-native` was the regression case (silent skip → `task_not_found` post fix-PR-A's stricter source-set gate).
...
- **+12 vitest cases** in `project-model.test.js` covering 5 DSL shapes + the new task-name candidate + `resolveTasksFor` integration. Baseline 732 → 744 passing. Closes `:core-firebase-native task_not_found` + 3 silent false-skips on a reference KMP composite project. PR #126 / `873ed44`.
...
- **+7 vitest cases** covering the gate per leg + regression guards + integration with `--test-type all`. Baseline 725 → 732 passing. Live result on a reference KMP composite project: 66 false-positive errors → 1 error + 68 skipped (the 1 remaining error was `:core-firebase-native`, closed by fix-PR-D). PR #125 / `e8fd69c`.
...
- **Closes v0.8.0 release-blocker BACKLOG entry.** Unblocks PR4 wide-smoke pass-7 across the affected projects (a private KMP application, a private KMP application, a multi-module DI sample, gyg, nav3-recipes, FileKit, a private KMP application).
