...
- **Live verification:** `kmp-test android --list-only --module-filter=:benchmark --project-root <dipatternsdemo>` → `instrumented_modules:["benchmark"]` (pre-fix: all 43 modules); `kmp-test describe --module-filter=:core-result --project-root <shared-kmp-libs>` → 1 module.
...
- **Live verification:** `parallel --test-type common --module-filter ":core-result" --project-root <shared-kmp-libs>` → `modules[0]:{name:"core-result", type:"kmp", coverage_plugin:"kover", test_build_type:null, has_flavor:false, android_dsl:true, android_dsl_variant:"kmpAndroidLibrary"}` ✓.
...
When `--module-filter` matched at least one module but the per-leg skip semantics (env `SKIP_*_MODULES`, no test-type target) routed every match to `skipped[]`, the orchestrator emitted `errors[{code:"no_test_modules"}]` and exited 3. Surfaced during the iOS-side wet audit (PR #168, cells C1/E1/E2 — KaMPKit `:shared` no JVM target, shared-kmp-libs without macos target). Agents reading the exit code branched into a real environmental-failure path instead of recognising the legitimate skip.
...
- **Live verification:** `parallel --test-type common --module-filter ":benchmark" --project-root <dipatternsdemo>` → exit 0, errors:[], skipped:[{module:"benchmark", reason:"no common target (--test-type=common)"}] ✓ (pre-fix: exit 3 + no_test_modules error).
...
- **Live verification:** `describe :core-result --json` against shared-kmp-libs returns 1 module (`:core-result`) — identical to the explicit `--module-filter` form. Extra positional emits warning on stderr.
...
- **Live verification** (S22 R3CT30KAMEH attached): shared-kmp-libs `parallel --list-only --module-filter "core-result,core-common"` → `modules: [{name:"core-common"}, {name:"core-result"}]`, `parallel.list_only:true`, `coverage.modules_with_kover_plugin: ["core-common","core-result"]`, exit 0. dipatternsdemo `parallel --list-only --module-filter ":benchmark"` → 1 module matched (composes correctly with Bug #3 filter normalization).
...
- **Live verification** (S22 R3CT30KAMEH attached): shared-kmp-libs `kmp-test coverage` → `coverage.modules_with_kover_plugin` populated with 62 modules (the full kover surface). Pre-fix parallel envelope omitted the fields; post-fix shape parity across `parallel`/`android`/`coverage`.
...
- **Live verification** (S22 R3CT30KAMEH attached): shared-kmp-libs `:core-result` describe envelope flips `platforms: ["android"]` → `["jvm", "android", "ios", "macos"]`. test_tasks block (already correct pre-fix) consistent: `unit:'desktopTest'`, `device:'androidConnectedCheck'`, `ios:'iosSimulatorArm64Test'`, `macos:'macosArm64Test'`.
...
- **Live verification** (S22 R3CT30KAMEH attached): dipatternsdemo `android --list-only --module-filter ":benchmark"` → exit 0, `instrumented_modules: ['benchmark']` (was: exit 3, `no_test_modules`). Cross-product matrix confirms identical results across `parallel` / `android` / `benchmark` / `describe` for equivalent filter inputs.
...
- **Live verification.** NIA `:core:data` describe envelope flips `type: 'unknown'` → `type: 'android'`; `parallel --test-type androidUnit --module-filter ":core:data"` dry-run dispatches correctly (no more `no_test_modules`). `shared-kmp-libs :core-result` regression guard: `type: 'kmp'`, `androidDsl: 'androidLibrary'`, `androidDslVariant: 'kmpAndroidLibrary'`, `coveragePlugin: 'kover'` — all unchanged. S22 Ultra (R3CT30KAMEH) E2E confirmed.
- **Vitest 1080 → 1087** (+7 regression tests covering NIA `apply(plugin=...)` form, NIA `apply<JacocoPlugin>()` form, shared-kmp-libs `pluginManager.apply` form, `.asProvider()` parent-alias resolution, precompiled-script `plugins { id(...) }`, descriptor without `.kt` source backwards-compat, and the analyzeModule alias-expansion E2E).
