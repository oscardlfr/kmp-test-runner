...
The full CLI wet audit (pre-tag) surfaced a pre-existing inconsistency: `envErrorJson` hardcoded `exit_code: EXIT.ENV_ERROR (3)` in every envelope it returned, but two callers wanted different process exit codes — `describe.no_project` returns `EXIT.CONFIG_ERROR (2)` (the project root has no settings.gradle.kts — bad CLI input, not env failure), and `changed.no_changed_modules` returns `EXIT.SUCCESS (0)` (clean working tree is a legitimate exit-0 signal per BACKLOG line 105). describe just disagreed silently; changed had a manual workaround (`envelope.exit_code = EXIT.SUCCESS;` post-call override).

- **`lib/cli.js`** — `envErrorJson` accepts an optional `exitCode` parameter, defaulting to `EXIT.ENV_ERROR` for backward-compat (every other call site stays consistent without a code change).
- **`lib/describe-orchestrator.js`** — `no_project` site passes `exitCode: EXIT.CONFIG_ERROR` (matches the orchestrator's `return { envelope, exitCode: EXIT.CONFIG_ERROR }`).
- **`lib/changed-orchestrator.js`** — `no_changed_modules` (post-changes-detection) site passes `exitCode: EXIT.SUCCESS` directly, replacing the pre-existing manual `envelope.exit_code = EXIT.SUCCESS` override.
- **`tests/vitest/describe-orchestrator.test.js`** — F-1 regression test: `runDescribe` against an empty tmpdir asserts `envelope.exit_code === exitCode === EXIT.CONFIG_ERROR`. Pre-fix `envelope.exit_code` was `3` while `exitCode` was `2`.
- **Live verification:** `kmp-test describe --json --skip-probe --project-root <empty-tmpdir>` now reports both `envelope.exit_code: 2` AND process exit 2 (pre-fix: envelope reported 3 while process exited 2).
- **Vitest 1224 → 1225** (+1 regression test).
...
- **`lib/orchestrator-utils.js`** — new `validateEnum(flag, value, allowed, errors)` and `validateNonNegativeInt(flag, value, errors)` helpers. Both push a discriminated `invalid_flag_value` error into the caller-provided `errors[]` array on miss; `value === undefined` is treated as missing (not invalid) so unsupplied flags don't double-error.
- **`lib/cli.js`** — new `buildInvalidArgsEnvelope` envelope builder (mirrors `envErrorJson` shape but exits CONFIG_ERROR / 2). Pre-spawn validation in `main` rejects bad input BEFORE the ps1/sh wrapper spawn so the user gets a canonical `invalid_flag_value` JSON envelope instead of either ps1's PowerShell `[ValidateSet]` error (which the parse-gap fallback surfaced as `no_summary` — confusing) or silent fall-through to NaN.
- **`lib/parallel-orchestrator.js` + `lib/changed-orchestrator.js` + `lib/coverage-orchestrator.js` + `lib/benchmark-orchestrator.js`** — `parseArgs` populates `out.errors[]` via the new validators; `runX` exits CONFIG_ERROR with the `buildInvalidArgsEnvelope` shape when any `invalid_*` code is present. Direct `node lib/runner.js <sub>` invocations also gated.
- **`lib/describe-orchestrator.js`** — `runDescribe` compiles the `--module-filter` regex eagerly; on `SyntaxError` emits `errors[{code:'invalid_regex', flag:'--module-filter', value, message}]` + exit 2. Catches the positional shorthand path too.
- **`tests/vitest/input-validation.test.js`** (new) — 25 cases: 9 unit (validateEnum + validateNonNegativeInt), 13 integration parameterized over (subcommand, flag, badValue) × ENUM_CASES + NUMERIC_CASES, 3 Bug-C describe-regex variants, 1 envelope shape sanity.
- **Live verification:** `kmp-test parallel --test-type bogus --json` returns `exit_code:2, errors[0]:{code:"invalid_flag_value",flag:"--test-type",value:"bogus",allowed:[...]}` (pre-fix: `exit_code:1, errors[0].code:"no_summary"`); `kmp-test describe --module-filter "[unclosed" --json` returns `exit_code:2, errors[0]:{code:"invalid_regex",flag:"--module-filter",value:"[unclosed",message:"...Invalid regular expression..."}` (pre-fix: returned full module list silently).
- **Vitest 1193 → 1218** (+25 regression tests).

### Added — v0.9 win-audit Bug-K: top-level `schema_version: 1` field on every JSON envelope (2026-05-08)

The Windows-side audit found that only `describe.schema_version` (a project-model field, sourced from `buildProjectModel`) carried a version handle. The cli.js envelope itself — emitted by `parallel`, `changed`, `android`, `coverage`, `benchmark`, `info`, `describe`, `update`, `doctor` — had no top-level version, so agents reading the JSON couldn't branch on shape changes between releases.

- **`lib/cli.js`** — new `ENVELOPE_SCHEMA_VERSION = 1` constant near `EXIT`. Documented bump policy: bumped on breaking shape changes (field rename, type change, exit-code/error-code semantic shift); additive fields do NOT bump. Inserted into `buildJsonReport`, `envErrorJson`, `buildDryRunReport`, and the doctor inline emitter so EVERY envelope carries the field.
- **`tests/vitest/parity.test.js`** — explicit `envelope.schema_version === 1` assertion per subcommand (parameterized across the existing 9-entry CASES table). Snapshot tests under the same describe block also updated to include the new field — diff is exactly the `+ "schema_version": 1` line addition, no other shape drift. The explicit assertion guards against future regressions that the snapshot would silently absorb.
- **Live verification:** `kmp-test info --json --no-adb` envelope reports `"schema_version":1, "tool":"kmp-test", "subcommand":"info"` (pre-fix: schema_version absent).
- **Vitest 1184 → 1193** (+9 explicit assertions, snapshot churn absorbed).

### Fixed — v0.9 win-audit Bug-E + Bug-F: documented flag implications now actually fire (2026-05-08)

Both flags claimed implications in the `parallel --help` text that the parser didn't honor. Pre-fix, `parallel --coverage-only` didn't imply `--skip-tests` (so `dispatchLeg` still ran the test suite before the coverage-only short-circuit, and the spawn command line was missing `-SkipTests`). Pre-fix, `parallel --isolated-no-lock` alone didn't imply `--isolated` (so the envelope returned the nonsensical `{enabled:false, locked:false}` shape — locked:false but no isolation actually applied).

- **`lib/cli.js`** — `peekIsolatedFlags` now sets `enabled = true` when `--isolated-no-lock` is present (mirrors the existing `--isolated-cache-dir` branch). Also added a small `cleanedArgs` shim in `main` so `--coverage-only` adds `--skip-tests` BEFORE the wrapper spawn — without it the ps1/sh wrapper's spawn command line lacked `-SkipTests`, so the orchestrator-level fix alone wouldn't have closed Bug-E for `kmp-test parallel` invocations.
- **`lib/orchestrator-utils.js`** — `parseIsolatedArgs` mirrors the cli.js fix so direct `node lib/runner.js android --isolated-no-lock` invocations also flip enabled.
- **`lib/parallel-orchestrator.js`** — `parseArgs` `--coverage-only` case now sets both `out.coverageOnly = true; out.skipTests = true;`. The `if (opts.coverageOnly && opts.coverageModules)` filter at line ~1331 is now unreachable when coverage-only is set (skipTests routes to coverage-orchestrator first); left in place for direct calls without `--coverage-only`.
- **`tests/vitest/parallel-orchestrator.test.js`** — 2 new Bug-E parser tests + repurposed the pre-existing `--coverage-only filters modules to those listed in --coverage-modules` test (which asserted the buggy pre-fix behavior of running tests then filtering) to verify the new routing: `runCoverage` stub IS invoked and receives `--coverage-modules` forwarded.
- **`tests/vitest/cli.test.js`** — 2 new `peekIsolatedFlags` tests: `--isolated-no-lock` alone implies enabled (Bug-F); regression guard on `--isolated-cache-dir` not flipping noLock.
- **`tests/vitest/orchestrator-utils.test.js`** — 1 new `parseIsolatedArgs` test mirroring the Bug-F fix.
- **Live verification:** `parallel --coverage-only --dry-run --json` plan.spawn_args now contains `-SkipTests` (pre-fix: missing); `parallel --isolated-no-lock --dry-run --json` envelope now reports `isolated:{enabled:true, locked:false, ...}` (pre-fix: enabled:false).
- **Vitest 1179 → 1184** (+5 regression tests, repurposed +0).

### Fixed — v0.9 win-audit Bug-A.2: POSIX `--name=value` form normalized at cli.js + 4 orchestrators (2026-05-08)

The audit found that 4 of 6 orchestrators (`android`, `benchmark`, `changed`, `describe`) didn't accept the POSIX `--name=value` form. PR #167 partially closed the rollout for `parallel` + `coverage` (each had a local POSIX-split arm inside `expandNoCoverageAlias`) but didn't touch the other parsers. Wet investigation also surfaced a second mangling site: PowerShell's `-File` invocation parses `--name=:value` as a parameter binding, eats the `=` token, and strips the leading colon on the value — so `kmp-test android --module-filter=:benchmark` (cli.js → `run-android-tests.ps1` → `runner.js`) reached the orchestrator parser as `["--module-filter=", "benchmark"]`. The orchestrator-level helper alone wouldn't catch this; cli.js must split BEFORE the spawn.

- **`lib/orchestrator-utils.js`** — new `expandPosixEqualsForm(args)` exported helper. Splits each `--<name>=<value>` token (with the same `eq > 2` + no-whitespace guards as the historical inline copies) into `[--<name>, <value>]`, preserving every other token (positionals, space-form pairs, multi-`=` values like `-Pfoo=bar`).
- **`lib/cli.js`** — imported the helper and call it on `cleanedArgs` immediately after `expandNoCoverageAlias`. Defense in depth: cli.js fixes the cli → wrapper → orchestrator path; orchestrator-level calls fix direct `node lib/runner.js <sub>` invocations.
- **`lib/parallel-orchestrator.js` + `lib/coverage-orchestrator.js`** — refactored each local `expandNoCoverageAlias` to delegate the POSIX split to the shared helper. Alias-then-split shape preserved (so `--no-coverage=foo` still emits `[--no-coverage, foo]` rather than re-aliasing).
- **`lib/android-orchestrator.js` + `lib/benchmark-orchestrator.js` + `lib/changed-orchestrator.js` + `lib/describe-orchestrator.js`** — call `expandPosixEqualsForm(argv)` at the top of each `parseArgs` so direct invocation paths (and the parameterized vitest) see the canonical shape.
- **`tests/vitest/posix-flags.test.js`** (new) — 8 unit tests on `expandPosixEqualsForm` (passthrough non-flag, no-split when `eq <= 2`, multi-`=` preserved, mixed list, non-string defensive) + 4 parameterized parser tests asserting `parseArgs([flag, value])` and `parseArgs([flag+'='+value])` produce identical state across android, benchmark, changed, describe.
- **Live verification:** `kmp-test android --list-only --module-filter=:benchmark --project-root <a multi-module DI sample>` → `instrumented_modules:["benchmark"]` (pre-fix: all 43 modules); `kmp-test describe --module-filter=:sample-result --project-root <a reference KMP composite project>` → 1 module.
- **Vitest 1167 → 1179** (+12 regression tests).

### Fixed — v0.9 win-audit Bug-A.1: `translateFlagForPowerShell` preserves POSIX `--name=value` value verbatim (2026-05-08)

The Windows-side audit (parallel to the iOS wet audit closed in PRs #169–#174) found that `cli.js#translateFlagForPowerShell` PascalCased the entire flag token, including any value attached via the POSIX `--name=value` form. So `--module-filter=:sample-result` translated to `-ModuleFilter=:sampleResult` (mangling `sample-result` → `sampleResult`) and `--gradle-args=--rerun-tasks` to `-GradleArgs=RerunTasks` (losing the value's leading `--`). PR #167 partially closed the rollout for `parallel`/`coverage` parsers but didn't reach the PowerShell-bridge translator.

- **`lib/cli.js`** — `translateFlagForPowerShell` now splits on the first `=` (with `eq > 2` guard so `--=foo` is left alone), translates only the flag head through the dash-segment PascalCase walk, and re-appends the value (including `=`) verbatim.
- **`tests/vitest/cli.test.js`** — 2 new regression tests covering the two repro inputs from the audit. Both fail pre-fix (`-ModuleFilter=:sampleResult`, `-GradleArgs=RerunTasks`) and pass post-fix.
- **Vitest 1165 → 1167** (+2 regression tests).

### Fixed — v0.9 wet-audit drift #4: `_errors.json#testFailures[]` populates from non-AssertionError throws (2026-05-08)

Pre-fix the testFailures regex `/AssertionError[:\s].+/g` matched only `AssertionError` lines, so non-AssertionError throws (`IllegalStateException`, `NullPointerException`, `RuntimeException`, etc.) emitted by androidx-test runners produced an empty `testFailures[]` even when the top-level envelope correctly counted `tests.failed`. Surfaced during the iOS-side wet audit (PR #168, cells J2 + J2-watch — Galaxy Watch SM_L705F: `IllegalStateException: No compose hierarchies found in the app` from a Compose UI test fell through the regex).

- **`lib/android-orchestrator.js`** — new `parseTestFailures(stdout)` helper. Captures the canonical AGP/AndroidJUnitRunner failure shape (`<ClassFQN> > <testName>[device-id] FAILED\n    <ExceptionFQN>: <message>`) into structured `{test, cause}` entries. Falls back to a wide `*Exception/Error` scan (`{test: null, cause: <line>}`) when no canonical match fires. Dedupes repeated FQNs (gradle re-emits failures during the build summary). The `[device-id]` suffix is optional — host JVM tests omit it.
- **`tests/vitest/android-orchestrator.test.js`** — 7 new regression tests: canonical IllegalStateException, multiple-failure mix, host JVM (no device suffix), fallback path, empty input, canonical-preempts-fallback, FQN dedupe.
- **Vitest 1158 → 1165** (+7 regression tests).

### Fixed — v0.9 wet-audit drift #3: `parallel --test-type androidInstrumented` surfaces top-level `android:{}` block (2026-05-08)

`kmp-test android` populates a top-level `android:{device_serial, device_task, flavor, ...}` block on every wet run, but `kmp-test parallel --test-type androidInstrumented` (which dispatches the same gradle task internally) did not — agents reading the device serial / device task had to branch on subcommand. Surfaced during the iOS-side wet audit (PR #168, cells K4 + L3b — `parallel --test-type androidInstrumented` parity claim with `kmp-test android`).

- **`lib/parallel-orchestrator.js`** — when at least one leg is `androidInstrumented`, populate `parsed.android = {device_serial, device_task, flavor}` from the already-resolved values (`resolvedDeviceSerial` from the adb-probe block at line 1498, `opts.deviceTaskOverride`, `opts.flavor`). `cli.js#buildJsonReport` already propagates `parsed.android` to top-level. `tests.individual_total` is unaffected — WS-8's junit-XML walk has populated it through `state.tests.individual_total` since v0.5; it surfaces unchanged via `parsed.tests`.
- **`tests/vitest/parallel-orchestrator.test.js`** — 3 new regression tests: (1) android leg → top-level `android:{}` populated; (2) non-android leg → no `android:{}` field; (3) `individual_total` regression guard.
- **Vitest 1155 → 1158** (+3 regression tests).

### Fixed — v0.9 wet-audit drift #2: `modules[]` envelope shape parity between `--list-only` and wet runs (2026-05-08)

`kmp-test parallel --list-only` emitted `modules: [{name, type, coverage_plugin, ...}]` (object array) since v0.9 step 9.8 (Bug #7), but the wet-run path still emitted `modules: ["sample-result"]` (bare string array) — agents reading the same `modules[]` field across the two execution paths had to branch on shape.

- **`lib/parallel-orchestrator.js`** — extracted the `--list-only` short-circuit's inline shape into a `canonicalModuleEntry(mod)` helper. Both the short-circuit and `executeLeg`'s WS-9 `state.modules.push` site now build entries through the same helper. Object shape: `{name, type, coverage_plugin, test_build_type, has_flavor, android_dsl, android_dsl_variant}`.
- **`tests/vitest/parallel-orchestrator.test.js`** — added drift-#2 regression test asserting the wet-run and list-only paths emit identical object key sets. Updated 3 pre-existing tests (WS-9 + 2 `--coverage-only`/cascade-isolation cases) to use `.map(m => m.name)` for the string-comparison assertions.
- **Live verification:** `parallel --test-type common --module-filter ":sample-result" --project-root <a reference KMP composite project>` → `modules[0]:{name:"sample-result", type:"kmp", coverage_plugin:"kover", test_build_type:null, has_flavor:false, android_dsl:true, android_dsl_variant:"kmpAndroidLibrary"}` ✓.
- **Vitest 1154 → 1155** (+1 regression + 0 net from refactor).

### Fixed — v0.9 wet-audit drift #1: legit-skip in single-leg path no longer surfaces `no_test_modules` + exit 3 (2026-05-08)

When `--module-filter` matched at least one module but the per-leg skip semantics (env `SKIP_*_MODULES`, no test-type target) routed every match to `skipped[]`, the orchestrator emitted `errors[{code:"no_test_modules"}]` and exited 3. Surfaced during the iOS-side wet audit (PR #168, cells C1/E1/E2 — KaMPKit `:shared` no JVM target, a reference KMP composite project without macos target). Agents reading the exit code branched into a real environmental-failure path instead of recognising the legitimate skip.

- **`lib/parallel-orchestrator.js`** — `executeLeg` step 3 now distinguishes "filter matched nothing" (modules.length === 0 → emit `no_test_modules`, exit 3 — preserved) from "filter matched modules but every match was skipped for this leg" (modules.length > 0 + single-leg path → no error, exit 0). The `skipped[]` entries already carry the diagnostic. Multi-leg `--test-type all` keeps the legacy emission so the F2 demotion logic at line ~1565 can convert per-leg empties to `no_test_modules_for_leg` warnings when at least one other leg passes.
- **`tests/vitest/parallel-orchestrator.test.js`** — 3 new regression tests: filter+leg legit skip → exit 0 + skipped[] reason; regression guard for filter-no-match → exit 3 + `no_test_modules`; `--module-filter=*` + `--test-type common` against Android-only modules → exit 0.
- **Live verification:** `parallel --test-type common --module-filter ":benchmark" --project-root <a multi-module DI sample>` → exit 0, errors:[], skipped:[{module:"benchmark", reason:"no common target (--test-type=common)"}] ✓ (pre-fix: exit 3 + no_test_modules error).
- **Vitest 1151 → 1154** (+3 regression tests).

### Fixed — v0.9 wet-audit drift #5: `kmp-test describe` accepts positional `--module-filter` shorthand (2026-05-08)

`kmp-test describe :sample-result --json` previously dropped the positional argument silently and returned the full module set. Surfaced during the iOS-side wet audit (PR #168, cell B4 first attempt). The `parseArgs` switch in `describe-orchestrator.js` had no case for non-flag tokens, so positional args fell through to the `default: drop` arm — hiding the usage mistake from the user.

- **`lib/describe-orchestrator.js`** — `parseArgs` now binds the first non-flag token to `moduleFilter`, mirroring `kmp-test describe --module-filter <X>`. Subsequent positionals warn on stderr and are ignored. Globals consumed via `cli.js` lookup (not splice) — `--project-root <path>`, `--java-home <path>`, `--ignore-jdk-mismatch`, `--no-adb` — are now explicitly skipped so their values don't bind as positionals.
- **`tests/vitest/describe-orchestrator.test.js`** — 7 new regression tests: single positional, explicit-flag-wins-over-later-positional, first-positional-wins-over-later, `--project-root` value not bound, `--java-home` value not bound, boolean globals skipped, end-to-end runDescribe with positional shape.
- **Live verification:** `describe :sample-result --json` against a reference KMP composite project returns 1 module (`:sample-result`) — identical to the explicit `--module-filter` form. Extra positional emits warning on stderr.
- **Vitest 1143 → 1150** (+7 regression tests).

### Fixed — v0.9 step 9.8: `parallel --list-only` short-circuits before gradle dispatch (Bug #7) (2026-05-08)

`kmp-test parallel --list-only` was silently ignored — the flag was documented for `kmp-test android` only, and `parallel`'s orchestrator never parsed it. Users passing the flag against `parallel` got gradle dispatch (slow, requires JDK + adb) followed by `no_summary` error in the envelope. Surfaced during the v0.9 pre-release validation matrix (Block B cross-product). Out-of-scope for the original sweep; user requested fix before the v0.9.0 tag (2026-05-08 follow-up).

- **`lib/parallel-orchestrator.js`** — `parseArgs` now recognises `--list` / `--list-only` (mirrors android's flag aliases). New short-circuit between the empty-module check and the leg-determination loop emits `modules[]` (post-filter), `skipped[]` (filter-time skips), populated `coverage:{}` block, populated `isolated:{}` field, plus a new `parallel.list_only:true` marker, then exits 0 — no gradle dispatch.
- **`scripts/ps1/run-parallel-coverage-suite.ps1`** — added `[switch]$ListOnly` to the param block + `if ($ListOnly) { $kmpArgv += @('--list-only') }` passthrough. Required because the wrapper has `passthrough:false` (cli.js translates kebab→PascalCase upstream); without the param-block whitelist the flag was dropped.
- **`scripts/sh/run-parallel-coverage-suite.sh`** unchanged (sh wrapper passes flags verbatim).
- **`lib/cli.js`** SUBCOMMAND_HELP[parallel] documents the flag with comparison to `--dry-run`: dry-run shows the spawn command shape, list-only shows the module set the spawn would iterate.
- **Differs from `--dry-run`:** dry-run surfaces resolved spawn args (the `gradle` invocation cmdline) but doesn't compute the post-filter module set (`modules:[]` in dry-run); list-only computes the full module set + coverage shape but doesn't expose the spawn cmdline. Designed to be complementary.
- **Live verification** (S22 <device-serial> attached): a reference KMP composite project `parallel --list-only --module-filter "sample-result,core-common"` → `modules: [{name:"core-common"}, {name:"sample-result"}]`, `parallel.list_only:true`, `coverage.modules_with_kover_plugin: ["core-common","sample-result"]`, exit 0. a multi-module DI sample `parallel --list-only --module-filter ":benchmark"` → 1 module matched (composes correctly with Bug #3 filter normalization).
- **Vitest 1124 → 1130** (+6 regression tests in `parallel-orchestrator.test.js`: short-circuit + module-filter respect + empty-result error consistency + coverage-shape parity + `--list` alias acceptance + isolated-field passthrough).
...
- **Live verification** (S22 <device-serial> attached, dry-run only — no gradle dispatch on dry-run by definition):
  ```
...
- **`lib/parallel-orchestrator.js`** — `state.coverage` now includes both fields, populated from `modules.filter(m => m.coveragePlugin === 'kover'/'jacoco').map(m => m.name)` at state-init time. Inherited by changed/coverage subcommands via the standard envelope.
- **`lib/android-orchestrator.js`** — `discoverAndroidModules` now surfaces `coveragePlugin` per module. New `buildCoveragePluginLists` helper derives `kover`/`jacoco` arrays. All three sites that emit `coverage:{}` (`--list-only`, KMP_TEST_SKIP_ADB short-circuit, main dispatch state) now populate the lists from discovered modules.
- **`lib/coverage-orchestrator.js`** unchanged (real-run path was already populating; dry-run and `--coverage-tool none` paths emit empty lists by design — no project model = no signal).
- **Live verification** (S22 <device-serial> attached): a reference KMP composite project `kmp-test coverage` → `coverage.modules_with_kover_plugin` populated with 62 modules (the full kover surface). Pre-fix parallel envelope omitted the fields; post-fix shape parity across `parallel`/`android`/`coverage`.
- **Vitest 1113 → 1118** (+5 regression tests: 3 in `android-orchestrator.test.js` covering kover-only / jacoco-only / mixed populations across `--list-only`; 2 in `parallel-orchestrator.test.js` locking the shape always-present invariant + real-run population).

### Fixed — v0.9 step 9.4: `platforms[]` augmented from gradle probe (Bug #4) (2026-05-07)

`a reference KMP composite project :sample-result` reported `platforms: ["android"]` in `kmp-test describe` envelopes despite gradle exposing `desktopTest` / `iosSimulatorArm64Test` / `macosArm64Test` / `testAndroidHostTest` tasks (4 platforms). Root cause: `platformsFromAnalysis` only consulted source-set names + plugin/DSL evidence on disk; convention plugins that emit targets without leaving visible source-set dirs (KaMPKit-style hierarchy templates, Compose Multiplatform fixtures) were undercounted.

- **`lib/describe-orchestrator.js#platformsFromAnalysis`** gained an optional `gradleTasks: string[]` parameter. Static signals stay as fast-path; probed task names map back to platforms via canonical regex (`(jvm|desktop)Test` → jvm, `test(Debug|Release|...)?(Unit|AndroidHost|AndroidDevice)Test` / `connected*Test` / `androidConnectedCheck` → android, `ios(X64|Arm64|SimulatorArm64)?Test` → ios, `macos(X64|Arm64)?Test` → macos, `jsTest` → js, `wasmJsTest` → wasmJs). Static + probed are merged + deduped; output preserves canonical ordering (`jvm/android/ios/macos/js/wasmJs`).
- **`buildModuleEntry`** passes `entry.gradleTasks` from the project model (already populated by `probeGradleTasksCached` for non-`--skip-probe` runs). Null/undefined gradleTasks falls through to pre-fix static-only behavior — preserves `--skip-probe` semantics + test fixtures with no gradle integration.
- **Live verification** (S22 <device-serial> attached): a reference KMP composite project `:sample-result` describe envelope flips `platforms: ["android"]` → `["jvm", "android", "ios", "macos"]`. test_tasks block (already correct pre-fix) consistent: `unit:'desktopTest'`, `device:'androidConnectedCheck'`, `ios:'iosSimulatorArm64Test'`, `macos:'macosArm64Test'`.
- **Vitest 1104 → 1113** (+9 regression tests covering jvm augmentation, full KMP target set, jsTest/wasmJsTest/Android task mapping, null-gradleTasks fallback, static+probed merge dedup, ignored-non-canonical task names, canonical ordering).

### Fixed — v0.9 step 9.3: `--module-filter` normalization across orchestrators (Bug #3) (2026-05-07)

Pre-release validation against a multi-module DI sample surfaced a filter-matching inconsistency: `kmp-test android --list-only --module-filter ":benchmark"` returned 0 modules with `no_test_modules`, while the same filter shape worked on `parallel`/`describe`. Root cause was four divergent filter implementations across orchestrators (parallel: glob+CSV+dual-test, android: substring on stripped name, benchmark: substring on colon-preserved name, describe: regex on colon-preserved name) — bare `:`-prefix input failed on `android`'s `m.name.includes(filter)` against the colon-stripped `m.name`.

- **`lib/orchestrator-utils.js`** gained `splitCsv`, `globToRegex`, `matchModuleFilter` helpers — lifted from parallel-orchestrator. `matchModuleFilter` is the unified entry point with two semantics per pattern: bare strings (no `*`/`?`) do substring contains (preserves android/benchmark legacy contract); patterns with glob metacharacters do anchored glob match (preserves parallel legacy contract). Both modes dual-test against bare + `:`-prefixed forms + the short suffix after the last colon.
- **`lib/android-orchestrator.js#applyModuleFilter`** now delegates to `matchModuleFilter`. `:benchmark` against `:benchmark` module → match (was: 0 modules / `no_test_modules`).
- **`lib/benchmark-orchestrator.js#moduleFilterMatches`** now delegates to `matchModuleFilter`. Aligns with android + parallel.
- **`lib/describe-orchestrator.js`** keeps its regex semantic for power users but adds a colon-stripped dual-test fallback so `^:sample-result$` and `^sample-result$` both match a module whose `m.name` is `:sample-result`.
- **`lib/parallel-orchestrator.js`** drops the local `splitCsv` / `globToRegex` / `matchAnyGlob` definitions; `matchAnyGlob` retained as a backwards-compat alias to the shared `matchModuleFilter`. `applyModuleFilters` simplified — the redundant local short-suffix branch is now handled by the shared helper.
- **Live verification** (S22 <device-serial> attached): a multi-module DI sample `android --list-only --module-filter ":benchmark"` → exit 0, `instrumented_modules: ['benchmark']` (was: exit 3, `no_test_modules`). Cross-product matrix confirms identical results across `parallel` / `android` / `benchmark` / `describe` for equivalent filter inputs.
- **Vitest 1087 → 1104** (+17 regression tests across `orchestrator-utils.test.js` and `android-orchestrator.test.js` covering substring vs glob semantics, `:`-prefix dual-test, comma-CSV, mixed-pattern CSV, glob anchoring).

### Fixed — v0.9 step 9.2: convention plugin alias resolution (Bug #2) (2026-05-07)

Pre-release validation against NowInAndroid `:core:data` surfaced a broken type-classification path: modules that apply plugins ONLY via custom convention plugin aliases (e.g. `alias(libs.plugins.nowinandroid.android.library)`) were classified as `type: 'unknown'` because the alias resolves through the version catalog to a non-canonical literal id (`"nowinandroid.android.library"`), not the underlying `com.android.library`. The convention plugin's `apply(plugin = "com.android.library")` body is invisible to regex without source-scan, so `analyzeModule`'s canonical-id checks never fired and the parallel orchestrator's androidUnit gate rejected the module with `no_test_modules`.

- **`lib/project-model.js#parseBuildLogicPluginDescriptors`** extended to walk `*.kt` files under `build-logic/<X>/src/main/kotlin/`. New helper `extractAppliedPluginsFromConventionSource` recognizes five Kotlin source patterns: `apply(plugin = "...")` (NIA pattern), positional `apply("...")` (covers `pluginManager.apply(...)` + `target.apply(...)` + `with(pluginManager) { apply(...) }` — the a reference KMP composite project idiom), `apply<XxxPlugin>()` Kotlin generic form (Jacoco / Kover canonical mapping), `plugins { id("...") }` and `apply { plugin("...") }` precompiled-script forms.
- **Descriptor shape** gains `appliedPlugins: string[]` — the transitive plugin ids the convention plugin's `apply()` body activates. Resolved post-walk by basename match between `register("X") { implementationClass = "Y" }` blocks and `Y.kt` files. `addsCoverage` is now refined from `appliedPlugins` when the class-name heuristic missed (so neutrally-named convention plugins like `KmpLibraryConventionPlugin` that internally `pluginManager.apply("org.jetbrains.kotlinx.kover")` correctly surface `addsCoverage: 'kover'`).
- **`resolveDescriptorIdExpr` regex** extended to accept the optional `.asProvider()` step before `.get().pluginId`. NIA's parent aliases (e.g. `nowinandroid-android-library` coexisting with children `nowinandroid-android-library-{compose,jacoco}`) force `.asProvider()` in the Kotlin DSL to disambiguate. Without this, NIA's `androidLibrary` register block was silently dropped from the descriptor list, leaving consumers unresolvable.
- **`analyzeModule`** computes `expandedPluginIds = aliasPluginIds ∪ descriptor.appliedPlugins` and runs the canonical-id checks (`hasAndroidViaAlias`, `hasKmpViaAlias`, `hasKotlinAndroidViaAlias`, `hasNewKmpAndroidViaAlias`, `hasJvmKotlinPluginViaAlias`) against the expanded set. Modules whose alias resolves to a non-canonical literal now correctly inherit the underlying canonical plugin's type signal.
- **Live verification.** NIA `:core:data` describe envelope flips `type: 'unknown'` → `type: 'android'`; `parallel --test-type androidUnit --module-filter ":core:data"` dry-run dispatches correctly (no more `no_test_modules`). `a reference KMP composite project :sample-result` regression guard: `type: 'kmp'`, `androidDsl: 'androidLibrary'`, `androidDslVariant: 'kmpAndroidLibrary'`, `coveragePlugin: 'kover'` — all unchanged. S22 Ultra (<device-serial>) E2E confirmed.
...
- **README hero token-cost narrative** re-framed against a reference KMP composite project — different reference project, dual-track gradle-vs-`--json` story, 413-overflow callout. Cross-feature summary table + per-feature drill-down tables (parallel / coverage / changed / benchmark) + A:C savings ratio rebuilt from the v0.9 measurement notes + `tools/runs/cross-model-results-*.txt`. Coverage-A 413-overflow promoted to lead position (single most striking data point: the 74 MB raw kover capture cannot even be tokenised against an Anthropic model in one request — `kmp-test coverage --json` renders the same signal in 372 cl100k tokens).
- **Hero practical-impact paragraph** updated with a reference KMP composite project numbers — 5-iteration agent loop now ~6.6 M / ~5.6 M / ~1.2 M / ~143 M tokens for parallel / changed / benchmark / coverage (vs sub-5K each on `--json`).
...
### Performance — v0.9 step 8: token-cost re-measurement on a reference KMP composite project (2026-05-07)
...
- **Full matrix re-run** against a reference KMP composite project — 4 gradle-backed features × 3 approaches × 4 tokenizers + 2 agent-query subcommands × 2 approaches × 4 tokenizers (info / describe have no raw-gradle equivalent). Per-feature single capture; agent-query 3-run captures for determinism.
...
- **Load-bearing finding: coverage A overflows Anthropic's `count_tokens` endpoint** (413 `request_too_large`) on every Claude family — even Anthropic's tokenizer cannot process raw gradle on a real KMP project's coverage capture. Documented in the v0.9 measurement notes and surfaced in the README hero + coverage drill-down narrative.
...
- **Vitest 1068 → 1078** (+10). Evidence: the v0.9 measurement notes (top-level) + `tools/runs/cross-model-results-{parallel,coverage,changed,benchmark,info,describe}.txt`. README prose deferred to step 9 (re-framing required, not a one-line tweak).

### Fixed + Docs — v0.9 step 7 close-out: wet validation + 3 inline bug fixes + BACKLOG marked DONE (2026-05-06)

Closes the v0.9 macOS validation gate work end-to-end. The wet pass (real gradle invocations against the in-repo fixture + the reference KMP composite project + `KaMPKit`, with the S22 attached for instrumented cells) surfaced 3 genuine bugs — all fixed inline in this PR per `feedback_close_in_one_session.md`, none deferred to a follow-up session.
...
End-state: `node tools/macos-validation-gate.mjs --mode probe` against the 3 KMP projects (in-repo fixture, the reference KMP composite project, `KaMPKit`) reports **44 PASS / 0 DRIFT / 1 SKIP** — the only skip is the in-repo fixture's android cell which legitimately has no instrumented target. Step 7 closure ready: the gate can be re-run anytime in `--mode probe` (disk-safe) or `--mode scoped` (with `--module-filter`) to validate envelope shape + dispatch end-to-end.
...
- **`.gitignore`** — adds the macOS gate summary so iteration noise doesn't leak into commits. Revisited at step-7 close-out (Phase E) when the final summary is committed.
...
Closes the BACKLOG L1480 entry "Opt-in isolation". Previously, two simultaneous `kmp-test` runs against the same project root collided on Gradle's per-project `.gradle/` (configuration cache + build outputs); Tier 1's advisory lock serialized them. `--isolated` injects `--project-cache-dir <tmp>` into every gradle spawn so each run gets its own disposable cache dir — true parallel multi-agent fan-out. ~+41 vitest cases (922 → 963 passing). Live-validated end-to-end against a multi-module DI sample + S22 Ultra (<device-serial>) on Windows: 7 smoke scenarios covering dry-run / real spawn / user-supplied dir / KMP_TEST_KEEP_ISOLATED=1 / concurrent fan-out / kmp-test android passthrough, all green.
...
### Documentation
- **README — token-cost provenance footnote.** Detailed cross-model tables now stamped as v0.4.0 baseline (2026-04-27, a reference KMP composite project `:sample-result*`). Fresh v0.8.0 cl100k_base re-measurement against the same module added inline (parallel A:C 41× vs 127×, coverage 1170× vs 1218×, changed 7,821× vs 127× — same order of magnitude, narrative unchanged). Captures live under [`tools/runs/<feature>/`](tools/runs/).
...
- **Live verified** on a multi-module DI sample: pre-fix `:benchmark:connectedReleaseAndroidTest` fails immediately with `Unknown command-line option '--tests'`, post-fix gradle accepts the filter and `[PASS] benchmark` end-to-end with output written to `connected/release/`.
...
- **Root cause**: fix-PR-F (PR #131) added `--variant` honoring + `testBuildType` resolution but only in the AGP **fallback** branch of `pickGradleTaskFor('androidInstrumented')` — i.e. when `r.deviceTestTask` was null. Whenever `gradleTasks` contained a probed task name (the normal case), the early-return at `parallel-orchestrator.js:391` returned that name verbatim, bypassing the variant logic. Mac-side regression surfaced 2026-05-05 against a multi-module DI sample `:benchmark` post commit `058a520` (which enabled `connectedDebugAndroidTest` alongside the canonical `connectedReleaseAndroidTest` via `androidComponents.beforeVariants` for sweep compat): `resolveTasksFor`'s candidate chain matched Debug first → early-return → the orchestrator dispatched `connectedDebugAndroidTest` even though `testBuildType="release"` and the parser resolved it correctly into the cache.
...
- **+7 vitest cases**. Baseline 768 → 775 passing. Live verified on a multi-module DI sample: pre-fix `:benchmark task_not_found`, post-fix correct dispatch to `:benchmark:connectedReleaseAndroidTest`. PR #131 / `efacc82`.
...
- **+6 vitest cases** in `parallel-orchestrator.test.js`. Baseline 762 → 768 passing. Live S22 Ultra validated a private benchmark module's `androidConnectedCheck` runtime failure as `execution.failed: 0 → 1` (previously under-reported). PR #130 / `f95b57c`.
...
- **+12 vitest cases**. Baseline 744 → 756 passing. Live verified on a private KMP application (exit_code: 0, JDK 21 preserved). PR #127 / `049828a`.
...
- **Plugin detection**: `lib/project-model.js#analyzeModule` recognizes `kotlin { androidLibrary {} }` blocks (the `com.android.kotlin.multiplatform.library` plugin opt-in introduced in AGP 8.2 / KMP 2.0+) as the Android variant of a KMP module. Pre-fix, modules using this opt-in DSL fell through to "no Android target" because the analyzer only looked for the legacy `kotlin("android")` plugin id. a reference KMP composite project `:sample-firebase-mod` was the regression case (silent skip → `task_not_found` post fix-PR-A's stricter source-set gate).
...
- **+12 vitest cases** in `project-model.test.js` covering 5 DSL shapes + the new task-name candidate + `resolveTasksFor` integration. Baseline 732 → 744 passing. Closes `:sample-firebase-mod task_not_found` + 3 silent false-skips on a reference KMP composite project. PR #126 / `873ed44`.

### Fixed — v0.8.0 fix-PR-A — source-set gate prevents AGP modules from over-dispatching (2026-05-04)
- **Root cause**: `parallel-orchestrator.js#legsForAll` dispatched the `androidUnit` and `androidInstrumented` legs to any module exposing an Android-flavored task name in `gradleTasks`, regardless of whether `sourceSets` actually declared `androidUnitTest` / `androidInstrumentedTest`. Result on the reference project's KMP modules that apply AGP for `compileOnly` Android resources but have no test source sets: 66 false-positive errors with `task_not_found` per module.
...
- **+7 vitest cases** covering the gate per leg + regression guards + integration with `--test-type all`. Baseline 725 → 732 passing. Live result on a reference KMP composite project: 66 false-positive errors → 1 error + 68 skipped (the 1 remaining error was `:sample-firebase-mod`, closed by fix-PR-D). PR #125 / `e8fd69c`.
...
- **Wide-smoke pass-7 baseline locked**: 30-project regression matrix sweeping `kmp-test parallel --test-type all` against every KMP / Android project under `<workspace>/<project> Bucket counts: 3 GREEN / 14 SKIP / 13 RED-repo / 0 cascade. RED-orchestrator-cascade bucket count is the release gate: must be 0 to ship.
...
- **`preflightJdkCheck` returns `agpVersion`** so the auto-select `[NOTICE]` banner can attribute the JDK bump to AGP when applicable. Inlines the existing `aggregateJdkSignals` call instead of `findRequiredJdkVersion`; legacy thin wrapper kept for the 7 existing vitest cases. Banner on a private KMP application now reads `[NOTICE] auto-selecting JDK 17 from <Adoptium path> (Eclipse Adoptium; host default is JDK 23 — project applies AGP 8.8.2)` — makes WHY the JDK was bumped explicit.
...
- **Closes v0.8.0 release-blocker BACKLOG entry.** Unblocks PR4 wide-smoke pass-7 across the affected projects (a private KMP application, a private KMP application, a multi-module DI sample, local-app-1, nav3-recipes, FileKit, a private KMP application).

### Fixed — v0.8.0 PR2 — F3: `tests.individual_total:0` on UP-TO-DATE / FROM-CACHE runs (2026-05-03)
- **Root cause** (confirmed via Windows-side repro on a multi-module DI sample): the stale-XML `mtime` guard added in PR #116 to `lib/parallel-orchestrator.js#junitTestCountFor` filters out `TEST-*.xml` files older than `state.runStartMs`. When gradle marked a test task UP-TO-DATE, AGP did NOT rewrite the XMLs — their `mtime` lagged the run start by hours/days, and the guard discarded all of them. Repro: a multi-module DI sample had 4 valid XMLs containing 68 testcases at the canonical path, walker reported 0; touching the files (advancing mtime) → walker reported 68.
...
- **Verified live** post-fix: a multi-module DI sample `individual_total: 0 → 68` (matches expected count from the 4 XMLs); execution telemetry shows `up_to_date:1, no_source:1, no_evidence:1` for the 3 module run, walker correctly counts the up_to_date module's 68 testcases and skips the others.
...
- **`detectBuildLogicCoverageHints` now distinguishes CONVENTION vs SELF kover/jacoco signals (v0.6 Bug 6).** The v0.5.2 Gap A pre-work added a naive `\bjacoco\b` (and `\bkover\b`) scan over every file under `build-logic/`. nowinandroid surfaced the false positive: `build-logic/convention/build.gradle.kts` only NAMES jacoco-related convention plugins via `gradlePlugin { plugins { register("androidApplicationJacoco") { id = libs.plugins.<...>.jacoco.get().pluginId; implementationClass = "AndroidApplicationJacocoConventionPlugin" } } }` — the substring "jacoco" appears multiple times but jacoco is never APPLIED to build-logic itself or to consumer modules from this file. Pre-fix, `analyzeModule` propagated `coveragePlugin: 'jacoco'` to all 35 nowinandroid modules; the actual `kmp-test parallel --coverage-tool jacoco` run reported `modules_contributing: 0` because no module had jacoco apply for its own tests. Discrimination now uses two signals:
  ...
  Real-world surface: a reference KMP composite project continues to inherit `coveragePlugin: 'kover'` for all 71 modules (its convention plugin lives under `build-logic/src/main/kotlin/` — the CONVENTION path); nowinandroid no longer false-positives.
...
- **Bug Z (v0.5.1) — `--json` mode hung indefinitely on Windows with no output.** `pwsh.exe` inherited Node's pipe handles to its grandchildren (`gradlew.bat` → gradle java daemon). The daemon survived the script exit by hours and held the pipes open, so `spawnSync` waited for stdout EOF that never arrived. Real-world repro: `kmp-test android --json --module-filter '*sample-encryption*'` against a reference KMP composite project hung 15+ min with 0 bytes stdout (the actual gradle work completed in 4s — the rest was wait-for-pipe-close). Fix: on Windows + jsonMode, redirect child stdout/stderr to temp file descriptors via `fs.openSync` instead of buffered pipes — the daemon may keep the FD open, but `spawnSync` only waits for the script process to exit, not for FD close. POSIX path keeps default `encoding: 'utf8'` (no inheritance issue). Production validation: `kmp-test parallel --json` against a reference KMP composite project now completes in 50s with full envelope (3/3 tests passed, `no_coverage_data` warning surfaced).
...
- **`run-changed-modules-tests.ps1`: 3 Windows-blocking bugs** that made
  ...
  - Leading colon: `Get-ModuleFromFile` returns `':sample-result'` (gradle
    style) but `-ModuleFilter` expects `'sample-result'`. Now uses
...
- Conventional Commits enforcement on PR titles via
  ...
  private-toolkit/reusable-commit-lint.yml so the repo stays standalone (per
